...
原保存模式完整梯度摘要与R2一致。长控制累计保存冻结权重/view113,127,841,792
字节，是14,140,980,224字节绑定存储的8倍；prefill激活73,998,897,016字节，KV
5,607,096,320字节，其他decode激活117,296,536字节。保存张量响应后全部释放。
本次RSS/HWM观测峰值323,191,660,544字节，不与上轮一次RSS观测混同。

直接view引用将长控制RSS/HWM降为108,694,700,032字节，但梯度最大差0.0026191857，
未过冻结容差，R3整体BLOCKED。logP相等不替代梯度对照。代码检查发现原CPU保存
创建连续布局，而view保留原stride；后继每个绑定view只持有一份原保存布局的连续
GPU副本，不逐事件复制，额外显存单列。布局解释仍须实际对照，不提前断言根因。

## R4执行前冻结

三个阶段串行且各用新进程：共享连续布局对照既有2+8token完整梯度；同点分段
回放对照完整梯度；22528+2048固定合成压力、后续短响应释放和非空Adam接线。
原完整梯度使用R3保存的真实向量，不重跑原资源分项。logP和完整梯度均沿用
atol1e-6/rtol1e-5，不放宽失败准入。固定压力token不是金融采样，无新增generate。

分段只允许当前Qwen的append-only DynamicLayer，保存一份完整KV，边界用其
精确前缀；反向传回所有边界伴随及prefill导数。纯层重算使用显式VJP和新局部
缓存，不重复修改历史；始终在虚拟参数作用域和eval模式，不切train模式绕检查。

非空Adam控制在GPU参数副本上执行200次明确标记的诊断步，比较虚拟201步和
另一副本的真实201步、pullback有限性与原模型隔离。这不是原Student训练，也
不冒充实际epoch5；正式运行到epoch5仍核对真实step200及当时矩。资源门通过后
还需冻结完整闭环脚本，方可启动已登记Full seed11，不能把门通过当成360反馈完成。
