...
The exact 13,092-byte external review is bound at SHA-256
`cc2740cbe68233493f3cd93a09298f7ee870042b1d6d4c28939118af34d8dce0`. It retains v26.214
only as `VALID_SCOPED_DEDICATED_EXCEPTION_SUBCLASS_CONTROLS` but fails its full terminal
authority at `ACTUAL_V209_TYPED_FAILURE_SOURCE_SURFACE_TOTALITY`: the four actual v26.209
throws use the base `TypedTransportFailure` with an instance terminal, while v26.214 admitted
eight dedicated subclasses through a bare-class-name-to-terminal map. The actual base class was
absent from that map, so actual v26.209 failures could escape during observation validation. The
exact 42-byte operator directive `参照审计报告开展后续实验修订`, SHA-256
`e3adc8d65f07c54893d36828d8c12bdca9e83ab8a07fb94e40a259a2a18bcf73`, authorizes only
this credential-free repair. Provider calls and credential lookups are zero.

All 63 v26.214 formal files and 1,535,767 bytes revalidate, including all 62 self-excluding
Manifest members and 1,523,563 bound bytes. Its generic evidence shape, removal of
caller-selected evidence-subclass authority, eight dedicated subclass controls, and four
authority attacks remain retained at their scoped meanings. Its eight-pass historical Gate,
Decision, and Transition remain immutable; their actual-source-totality interpretation is
superseded.

The exact v26.209 source commit/tree and source-file SHA-256 are:

```text
commit  5809e9782515e55ee797b43730584d5d860aaa5c
tree    b2272bc1766a2d9b8c6562cb0b9f2f47151ad7cf
SHA-256 4529523fc737f26801118cc5cf78b682f2e510c5f887ed0d14a60a5bd26d9b35
```

One new `TypedFailureSourceContract` binds all four actual raise callsites, the exact exception
type ID as module plus qualified name, the instance terminal, and an origin partition:

```text
ScriptedTransport.send / invalid dispatch chain  transport_send     instrument_failure
ScriptedTransport.send / empty queue             transport_send     instrument_failure
public projection / reasoning-like key           public_projection  privacy_rejection
public projection / non-object response          public_projection  instrument_failure
```

Bare class names are not authority, and a class is not required to imply one terminal. The
admitted exact type is the v26.209
`...final_request_contract_continuity_repair_preflight:TypedTransportFailure`; terminal
admission requires agreement among exact type ID, actual `error.terminal`, catch-site origin,
v26.195 Registry, and `ExecutableInvocationRecord.typed_terminal`.

`ActualSourceAuthenticRunner` preserves the exact v26.209 request and Runtime path but separates
the transport call and public projection into two origin-specific catches. It constructs the
observation from the actual caught instance, hashes the actual reason, binds the complete
invocation record, and appends it to a no-replace Runner authority only after strict validation.
The dispatcher remains `dispatch(evidence)` with no terminal or exception-type selector.
Persistence reparses the evidence, requires byte equality with the Runner authority, and
independently rederives the decision before Raw.

One source-bound `ActualSourceFailureConsumer.execute_preflight` entry executes:
...
exact authorization guard -> durable isolated preflight receipt
  ...
  -> actual v26.209 typed-failure source -> origin-specific Runner catch
  -> Runner-owned observation -> consumer terminal branch
  -> source/Registry dispatch -> Raw -> Result -> Trace -> Outcome -> checkpoint
```

Four distinct exact v26.209 Manifest Jobs exercise the four actual throws:

```text
actual source callsites / base exception instances              4 / 4
Runner observations / consumer terminal branches                4 / 4
exact origin-terminal matches                                   4 / 4
transport / projection origins                                  2 / 2
instrument / privacy terminals                                  3 / 1
persisted Raw/Result/Trace/Outcome/checkpoint files                  20
exception escapes / empirical rows / Provider calls          0 / 0 / 0
```

Two new admission controls reject the exact base class with an unregistered terminal and a
different class with the same bare name. Four retained full-rehash controls reject instrument to
provider-identity reclassification, privacy to transport reclassification, reason-hash
replacement, and cross-Job substitution. The retained attacks create twenty prospective layer
identities before rejection:

```text
source-admission / retained authority attacks rejected          2 / 2, 4 / 4
fully rehashed downstream layer identities                              20
accepted attacks / attack Raw writes                             0 / 0
```

The exact noncompensatory Gate partition is:

```text
R0 external scope and exact v26.214 Freeze                              PASS
R1 exact v26.209 typed-failure Source Contract                          PASS
R2 exact type + instance terminal + origin observation                  PASS
R3 durable ingress mechanics retained                                   PASS
R4 four actual v26.209 source controls terminalize                      PASS
R5 authentic source terminal -> five-layer persistence                  PASS
R6 source admission + retained authority attacks reject                 PASS
R7 zero-Provider/credential/empirical boundary                          PASS
passed / failed                                                          8 / 0
```
...
- external revision authorization / v26.214 Freeze:
  `finance_v26_215_external_revision_authorization:25938853f34cf5855c7c8a7ad8f4c4a9bb505598ec9bda2a8b531e837b3dabcc` /
  `finance_v26_215_v214_freeze:3bebc6b6af92fc32ea43f392c88bbb5b997d7c009af0f81b49c46cf12cb7a083`;
- implementation / actual-source Contract:
  `fresh_repaired_actual_typed_failure_source_totality_implementation_binding:0a7358a0c4871f0943e7a834a90fb261f7ff1d4cbb9ca46f6212cac593e33cdd` /
  `fresh_repaired_actual_v209_typed_failure_source_contract:93af74abff1f7a646f86ea2d143da693e93324df589364138feb3429f1230403`;
- Runner observation / dispatcher / persistence:
  `fresh_repaired_actual_source_runner_observation_binding:bb77e63f604c9a4fbb0ed096790f595794954df56c0395cfae5f27eac63c5d1d` /
  `fresh_repaired_actual_source_typed_failure_dispatcher_binding:ac34c98a34d14d44d17e23cb8b7b68e0ee0a808b1d447d61b458041d2716a914` /
  `fresh_repaired_actual_source_typed_failure_persistence_binding:3df9c00763c8b96df29bb7311518fbe4fb86e86eea384329294a300edda1043a`;
- consumer / composition:
  `fresh_repaired_actual_source_failure_consumer_binding:611a80554500277a97471b6740b8998b9f2e5ec81a355f4297516d2850571629` /
  `fresh_repaired_actual_source_failure_composition_contract:0cfe95aa031b46790cb0c505d32772616fe6e3faf8101a202c04ba11d9cfdc8c`;
- execution / negative-control Audits:
  `finance_v26_215_actual_source_surface_execution_audit:5ed58c72692fb3176b3200d479cfea94eb2847b1eeb22a1ca8c93c77c392e039` /
  `finance_v26_215_source_totality_negative_control_audit:c4813c3c7ec97515153b528ef142bfebe8849e86072d54e90ac211aac241f1a6`;
- Gate / Decision / Transition:
  `finance_v26_215_gate_evaluation:9c9062c68fecf0cc06c2aa3ba22f3d9e91c2d69083fc3096338d91ed58f3b8c3` /
...
The exact source commit/tree are `bae3bafbedeb3c18dd04b21ebabf6c4138c14ced` /
`0740f9c5a56e107e5deb07fed7637b29c8a91ad9`. The 44-file, 785,750-byte formal
directory rebuilds byte for byte; its Manifest binds 43 members and 777,701 bytes. Focused tests
...
as `VALID_SCOPED_DEDICATED_EXCEPTION_SUBCLASS_CONTROLS` and fails it at
`ACTUAL_V209_TYPED_FAILURE_SOURCE_SURFACE_TOTALITY`. Its one generic evidence shape, removal of
caller-selected subtype authority, eight dedicated subclass controls, and four authority attacks
remain valid at their scopes. The four actual v26.209 failure sources use the base
`TypedTransportFailure` with instance terminals, which v26.214's bare-class-name map did not
admit. Its actual source-totality and one-consumer terminalization claims therefore fail.
...
The later operator directive consumed only the zero-Provider v26.215 repair above. The current
transition is v26.215's credential-free independent audit only; the v26.211 authorization
remains unconsumed.
...
The immutable historical v26.214 decision was:
...
only an exact response-grammar and fresh-identity credential-free preflight. v26.112 now compiles
that Grammar from one strong v2 Schema and closes a fresh 24-TaskPackage, 48-Path, 32-Job identity
chain. v26.113 credential-free preflights the exact future Runner and authorizes only its fresh
32-Job engineering calibration. v26.114 has now executed that exact denominator: 54/81 public
payloads crossed the exact ABI, 30 reached reversible Stage 2 Commit and public Observation, and
all 32 Jobs remained model-invalid. v26.115 independently audits the complete run and authorizes
only a fresh Host-bound fixed-`stage` metadata protocol preflight. v26.116 then performs the
...
combined ABI-plus-semantic-recovery control pass with zero real Provider calls. v26.120 then
attempted that exact online denominator. All 32 Jobs were exposed, but the run failed before
aggregation with 31 complete Instrument Raw Executions and one Provider-orphan Job. v26.121 now
independently audits the failed lineage, 256 persisted Provider artifacts, 188 complete-Raw
semantic choices, Final response-interface mismatch, and privacy-first persistence defect.
v26.122 has now compiled one shared exact two-field Final Grammar, preserved all 324 action states
and Candidate presentations, qualified fresh 24-TaskPackage, 48-Path, and 32-Job identities, and
...
calls. The historical failed Gate and unidentified rejection cause remain unchanged. v26.138 has
now consumed the exact online authorization. All 32 Jobs complete in 191 exact-model, Thinking-
complete, Usage-complete HTTP-success calls with 191 privacy-first artifact pairs and zero Stage 2
Provider calls. Thirty-one Jobs cross the exact first-action interface, all twelve Mechanism x
Path cells are covered, and privacy, Instrument, model, Thinking, and Usage failure counts are
zero. The pre-registered noncompensatory Qualification Gate therefore passes under the exact v2
repeated-engineering condition. v26.139 independently replays 4,525 files, reconstructs all 32 Raw
Executions, 605 bound Raw artifacts, 34 report aggregates, and all 173 reached online v2 Action
...
v26.180 has now consumed that exact independent-audit-only transition with zero Provider calls.
It freezes and independently rebuilds all eighteen v26.179 files byte for byte, retains the exact
192-Job index, multicomponent correction, Job-level estimand, and accepted-prefix results, and
narrows the strongest estimator claim to an exact Job-key-set and wrapper-parent Gate. Six
separate fully rehashed 192-row controls then cross Job Outcomes, duplicate Raw IDs, duplicate
Result IDs, rotate Raw/Result parents, break Result-to-Outcome/Final parentage, or reuse one
attempt trace. The unchanged estimator accepts all six and returns synthetic `192/192` first and
bounded fractions; no control row enters an empirical denominator. A Final-ABI-false Qualified
payload also validates, while a real malformed-Final Runtime path and one ABI-valid unknown first
Action each escape as exceptions with zero typed Outcome rows. A completed payload accepts a fake
`first_failed_component_key`, and six registered outer failure classes have zero exact rows. All
11 defect controls reproduce and all 25 audit-construction Gates pass, but the noncompensatory
online Gate fails exactly as required. Provider, Development outcome, empirical row or estimate,
Mapper, State, frequency, Contribution, and VTDO counts remain zero. The current transition
...
All eighteen authoritative v26.179 files independently rebuild byte for byte. Its exact 32 x 6
prospective Job index, 192 unique Raw and Result namespaces, ordered multicomponent attempt
language, complete-Job `q_first` and `q_bounded_correction` definitions, 4,632 accepted-prefix
local trajectories, 41,124 Candidate evaluations, and eleven scripted branch controls remain
immutable. The strongest estimator interpretation is narrowed to
...
```text
Control  Attack                                      Raw IDs  Result IDs  Trace IDs
1        cross-Job Outcome reassignment                  192         192        192
2        one Raw ID reused by all Jobs                     1         192        192
3        one Result ID reused by all Jobs                192           1        192
4        Raw and Result parents rotated across Jobs      192         192        192
5        Result parent unrelated to Outcome/Final        192         192        192
6        one attempt trace reused by all Jobs            192         192          1
```

Every attack retains 192 unique Job and row IDs and returns fixture-only
`q_first=192/192` and `q_bounded_correction=192/192`. These are diagnostic outputs, not empirical
Capability estimates. `RawExecutionDescriptor`, `JobResultDescriptor`, authoritative Job-bound
trace, formal empirical row, and formal estimate counts all remain zero.

Controls 7 and 8 show that a fully rehashed completed Qualified payload with
`final_response_abi_valid=false` passes the frozen validator, while a real Runtime trace that
reaches production finalization and submits an exact malformed Final propagates a Pydantic
`ValidationError`. `final_response_abi_invalid` has no registered endpoint; typed Outcome,
exact-row, Verifier-null-policy, and Qualified-false-policy evidence counts are zero. Control 9
uses the current State, exact four-field Grammar, and unknown 24-hex Action
`753cf4b44888c3d513d5877a`; the Runner raises
`ABI-valid first response references an absent current Action` and emits zero typed rows. The
first-Action correction policy remains intentionally unfrozen.

Control 10 fully rehashes a completed all-committed Outcome with
`first_failed_component_key=audit.fake.completed.component`; the old validator accepts it. The
old Runtime also uses that field as a mechanism-failure fallback, while separate
`first_uncommitted_component_key` and `first_mechanism_failed_component_key` fields are absent.
Control 11 registers Provider no-payload, Transport, Privacy, Resource, Instrument, and Provider
...
The 343-file transitive source Root has zero unresolved imports. The formal Root contains 14 files
and 132,421 bytes; report SHA-256 is
`521772cfa330867fec456d282664f6e31efc03a6aac0d3e71322a8c58a53b375`. Five fast tests pass in
3.30 seconds with the rebuild test deselected. The complete warning-as-error empty-directory
rebuild passes in 667.08 seconds and matches 14/14 files. The v26.179-v26.180 adjacent
non-rebuild regression passes 12/12 in 3.47 seconds. Focused PyCompile, Ruff, format, and Mypy
pass; package-wide Ruff passes. Package-wide Mypy checks 571 source files, retains six historical
diagnostics in four files, and has zero v26.180 diagnostics.
...
A successor may introduce only prospective content-addressed RawExecution and JobResult
descriptors, a Job-bound AttemptTrace parent, constructor-only empirical Outcome rows, an exact
evidence-set estimator, typed first-Action-reference-invalid and Final-ABI-invalid endpoints,
strictly separated failure-localization fields, and exact outer-terminal rows. It must preflight
them with zero Provider calls and zero Development outcomes. Provider execution, source, Task,
Component, Candidate, Schedule, presentation, model, Thinking, Grammar, Policy, resource,
correction-bound, threshold, validity, Manifest Job-set, Confirmation, historical rewrite,
...
That independent-audit transition has now been consumed by v26.180 with zero Provider calls.
Every v26.179 byte and narrow local result remains immutable, while its empirical parent-
authenticity and terminal-totality claims fail under the eleven exact controls recorded above.
The current transition is the credential-free authoritative parent-binding and terminal-totality
repair preflight authorized by v26.180.
...
Finance v26.138 consumed only the exact 32-Job privacy-safe S1 engineering execution authorized by
v26.137. Before credential lookup or client construction it replayed 3,901/3,901 files: all 3,884
v26.137 source bindings, all sixteen v26.137 outputs, and its exact committed implementation. It
independently rebuilt all predecessor outputs byte for byte. The formal prepare-only directory
contained thirteen frozen-input files and zero Raw, Provider, Transport, or checkpoint rows, so the
online denominator started at 0/32 with zero recovery and zero orphan artifacts.

All 32 Jobs completed in 191 Stage 1 Provider calls and 191 transport-inclusive invocations. Every
call was HTTP success, requested/selected/returned exact `deepseek-v4-flash`, retained positive
Thinking telemetry and complete Usage, and had no fallback, model discovery, Provider-native tool,
Transport replacement, or Stage 2 Provider call. All 191 privacy-first Envelope/Projection pairs
contain validated public payloads; Privacy rejection and Provider-failure no-payload counts are
zero.

Artifact-backed Usage is 1,455,568 tokens: 589,102 Prompt, 866,466 Completion, and 840,279
Reasoning tokens. The Reasoning/Completion fraction is `0.969777233036`; estimated cost telemetry
is USD `0.32318810720000002837`. The maximum actual Prompt is 14,027 bytes and the maximum Job
Usage is 88,765 tokens, within the unchanged 60K and 1.12M per-Job bounds. These engineering
observations do not replace the role resource Contract.
...
combined integrity failures                      0
~~~

The 31/32 Entry count exceeds the frozen 24-Job minimum, every one of the twelve cells has an
empirical Entry, and the zero-tolerance Privacy/Instrument/model/Thinking/Usage conjunction has no
failure. The exact v2 repeated-engineering S1 representation Qualification Gate therefore passes.
No v26.134 Entry row is pooled into this denominator, and the historical v1 Gate remains failed.

The one Entry failure is Failure Recovery `search_then_structured` Job
`finance_v26_privacy_safe_s1_qualification_job:4bb4c50a51a1708f73445ba1f0c0ffb98b1b6825d40d81cbc8b597df2ea35975`.
Its Primary and ABI Rescue are both exact-model HTTP successes with complete Thinking and Usage.
Both public payloads pass privacy, have the exact four-field Key set, exact protocol, current State,
and a visible Action. Their Decision kinds are `select` and `query_fully_qualified`, not the
Decision kind bound by the selected Candidate. Both therefore fail the unchanged Action Grammar.
The Job is model-invalid, not privacy- or Instrument-invalid, and receives no Host repair.

Across all Jobs there are 164 Primary requests, 27 ABI Rescues, zero Semantic Recoveries, 132 exact
Action payloads and Choices, 132 reversible Commits, and 115 Observations. The Observations contain
108 successes and seven failures. Progress Vector change occurs 104 times, three successful no-
immediate-progress actions match the reference baseline, and one nonreference action consumes the
single Ordinary Detour. The Job Detour partition is 31/1/0; support-exit terminals are zero.

The complete terminal partition is fifteen `model_valid_trajectory` and seventeen
`model_invalid_trajectory`. Twenty-one Programs close and complete their terminal node, seventeen
complete post-terminal verification, and fifteen cross the exact Final ABI and are independently
valid. These repeated-engineering-source values remain descriptive and contribute zero Capability,
Reachability, State Mapping, training, release, or production rows.

Finance v26.139 independently replayed 4,525/4,525 files: all 3,901 v26.138 source bindings, all
623 execution files, and its exact implementation. It reparsed 32 Results, 32 checkpoint rows, 32
Raw Executions, 191 Envelopes, 191 Projections, 191 Transport certificates, and 132 Choice
diagnostics. All 605 Raw descriptors, all 32 Verifier v3 Replays, and all 34 independently
reconstructed report aggregates match.

It also independently rebuilds every reached Action Prompt: 147 Primary and 26 ABI Rescue Prompts.
All 173 match persisted request hashes and byte counts, use the v2 protocol, reconstruct exact
State and Candidate set/order, and contain zero classifier-sensitive or predecessor-sensitive
Keys. This confirms Prompt/output structured-Key noninterference on the reached online surface; it
does not establish behavior equivalence to v1 or privacy of unconstrained free text.

Private-reasoning payload, invalid-payload persistence, Raw HTTP body, Raw request body, role-source
Job, role-class-external opportunity, and Stage 2 Provider-call counts are zero. Sixteen destructive
mutations fail closed. The audit made zero Provider calls and used zero GPU jobs.

The authoritative identities are:

- v26.138 report:
  `finance_v26_privacy_safe_s1_qualification_execution_report:f9b1df0929685b8245814ac190c88c822468015680770692b249c935d2838ff9`;
- v26.138 source replay:
  `finance_v26_privacy_safe_s1_execution_source_replay:95158b5c06a23946fb4f777c11618dd05f42ae88b19d7a9e9b09d5bdf536b267`;
- v26.138 Raw Lineage:
  `finance_v26_privacy_safe_s1_qualification_raw_lineage:9768e142debfe9586efa3c7feaa52f7c24005cf976f70b105cb157f025e734f3`;
- v26.139 report:
  `finance_v26_privacy_safe_s1_postrun_audit_report:4dca82657f009642423a416cd1da3553f2327fd1ecdf468bdc1b1b169eba497a`;
- independent Raw reconstruction:
  `finance_v26_privacy_safe_s1_independent_raw_reconstruction:98726cca664605b49824947a12d99755d2d2c148c746c8290172ed0d5933eeaa`;
- online Prompt noninterference:
  `finance_v26_online_prompt_noninterference_audit:2eb9cae6c88c375042bcdf0a65addc5f42282cfcfc3bc0c56790ed84df7886cd`;
- Entry boundary:
  `finance_v26_privacy_safe_s1_entry_boundary_audit:ae4d0cc4136a5544b54f27b0e0156f580e96f6c22ca5efa838e20204e1badc12`;
- Qualification Gate:
  `finance_v26_privacy_safe_s1_qualification_gate_audit:10206b746caef04c550b1b9ba389f0f3a14203f1f5594a9a26ca584ae82488cc`;
- outcome interpretation:
  `finance_v26_privacy_safe_s1_outcome_interpretation:68ece29b1996545706c44a12fd69cd334e7c19839ab0427e457304dfcf6fff11`;
- transition:
  `finance_v26_privacy_safe_s1_postrun_transition:97def7ae5bbfdb49edf6f8854cb41a86dbd997731be84dbe00462a14192d0cbf`.

A credential-free completed-run replay resumes at 32/32, constructs no client, makes zero calls,
and preserves both the 623-file denominator and report SHA-256.

Focused v26.138 Pytest passes 3/3 in 179.31 seconds; focused v26.139 Pytest passes 2/2 in
42.67 seconds. The adjacent v26.137-v26.139 regression passes 7/7 in 319.54 seconds. Focused Ruff
and Mypy pass. Package-wide Mypy checks 456 source files and retains only the three pre-existing
v26.70/v26.129 diagnostics, with zero v26.138 or v26.139 diagnostics.
...
The successor may materialize only a fresh Capability-only TaskPackage, Path, Contract, Manifest,
Job, Runner, execution, and report identity chain under the exact frozen role Population, tasks,
Tiers, S1, Candidate authority, v2 Prompt metadata, Grammars, classifier, model/Thinking profile,
resource and bounded-Detour Contracts, recovery channels, and zero-Provider Stage 2. It must pass a
complete credential-free Runner preflight before any role call. Provider calls, Capability
execution, Reachability materialization or execution, State Mapping, role-source exposure,
historical rerun or pooling, protocol/Grammar/Candidate/model/resource changes, Host repair,
training, release, and production Contribution remain forbidden. See
...
not reclassify the historical row or establish that the lexical hazard caused it. That exact online transition was then consumed by v26.138 and audited by v26.139. The current
transition is the fresh Capability-only identity-chain and credential-free Runner preflight
recorded above.
...
Finance v26.129 consumed only the credential-free fresh-role preflight transition authorized by
v26.128. It replayed 3,154/3,154 files before source construction: all 3,134 v26.128 transitive
bindings, all nine v26.128 outputs, nine additional historical role identity inputs, the exact
source Evidence Snapshot, and its implementation. It looked up no credential, constructed no
model client, made zero Provider or Stage 2 Provider calls, used zero GPU jobs, and created zero
empirical rows.

The historical census covers 29 selected task-record, task-package, and
Development/Confirmation Population files. It finds 155 historical source tasks, maps 154 back
to immutable broad source Populations, and retains the unmapped task's direct identities. The
26,290 Evidence exclusions in the v26.29 receipt and 961 historically selected Evidence
identities form a 27,173-item union. A freshly regenerated 70-task Sampling Frame uses 401
Evidence identities and has zero overlap with that union.

Before loading any Kernel or resource value, a role-neutral fixed-salt ranking froze separate
12-task Capability and Reachability source Populations. Each role contains one Easy, one
Frontier, and one Hard task for each of the four mechanisms. Historical and cross-role overlap is
zero on task ID, source-task ID, Evidence ID, Evidence Version ID, core semantic signature, task
signature, mechanism-instance signature, and source-record ID. Both roles contain 75 Evidence
identities and share none.

Only after those identities were persisted did the audit load the exact v26.128 Kernel and
v26.123 privacy-first Runner Contract. The fixed first Gate covers six Context sources and twelve
paths: three Capability `structured_direct` paths and nine Reachability paths across
`structured_direct`, `search_then_structured`, and `search_then_open`. The Prompt-only
control reaches 132 public states, independently matches complete Candidate authority, makes
Stage 2 Commits with zero Provider calls, closes every diagnostic Program, and reaches Final
Commit.

Four Easy paths fit. Eight paths are incompatible: both Frontier direct paths already exceed
400,000 tokens; both Frontier search paths also exceed the request and Provider-call bounds; and
all four Hard paths exceed the rollout, request, and Provider-call bounds as well as the
60,000-byte Prompt ceiling. The exact failure counts are 4 Prompt, 6 Primary-request, 6
Provider-call, and 8 rollout-bound failures. The maximum Prompt is 86,161 bytes, maximum Primary
request count is 20, maximum conservative path bound is 1,276,468 tokens, and minimum headroom is
-876,468.

The fixed first Gate therefore closes as `role_population_kernel_incompatible`.
Semantic Reconciliation, Failure Recovery, and State-dependent Stopping remain unmeasured after
fail-fast; they are not classified as passing or failing. The six Context operational packages
are diagnostic fixtures only. Future role TaskPackage, Contract, Manifest, Job, and Runner counts
are zero. Post-hoc source deletion, source substitution, tier change, threshold relaxation, and
role Provider execution remain forbidden.

All twelve destructive mutations failed with zero Provider calls. Formal and independent builds
regenerated all ten outputs byte for byte. Focused Pytest passed 2/2 in 22.76 seconds; focused
Ruff and Mypy passed. The selected v26.122-v26.129 adjacent regression passed 7/7 in 83.75
seconds.
...
The successor must preserve the exact two source Populations and their eight-channel separation.
It may design a fresh scalability protocol for Prompt projection, Candidate representation,
interaction staging, request accounting, or resource qualification. Role TaskPackage, Contract,
...
That credential-free transition has now been consumed by v26.129. The v26.128 Kernel and endpoint
results remain immutable. The current transition is the frozen role-Population Kernel
scalability design recorded above.
...
Finance v26.122 consumed only the credential-free repair transition authorized by v26.121. It
replayed 2,534/2,534 files before Grammar or identity construction: all 2,523 v26.121 transitive
bindings, all nine v26.121 outputs, and two exact implementation files. It looked up no
credential, constructed no model client, made zero Provider or Stage 2 Provider calls, used zero
GPU jobs, and produced zero empirical rows.

One strong Schema now compiles the exact Final model payload, Primary and Rescue Prompt contracts,
parser, and typed rejection. The model returns exactly `answer` and `rationale_summary`. Fixed
`stage`, response `protocol`, terminal public-state identity, and terminal Commit identity are
bound in a content-addressed Host Envelope before Provider invocation. The Host cannot insert or
repair the answer or rationale. Both Primary and Rescue contain the Provider JSON-mode lexical
cue, share the same Grammar, and bind the same Host Envelope.

Across all 48 terminal Compiler states, the Prompt-only control parsed 48/48 Primary and 48/48
Rescue Prompts, preserved 48/48 semantic projections, and matched 96/96 Compiler answers. All
missing, extra, wrapper, and private-reasoning-field mutations failed. Forty-eight schema-valid
wrong answers crossed only the ABI, showing that exact serialization and answer validity remain
separate. Maximum Final Primary and Rescue sizes are 5,152 and 5,286 bytes.

The Final repair preserves 324/324 exact Semantic Action Prompt hashes, 324/324 Candidate
presentation orders, and 324/324 Prompt-only reference Commits. Source, Path, assignment, and seed
projections pass 24/24, 48/48, and 32/32. All 104 TaskPackage/Path/Job rows retain the exact
Semantic Action protocol, complete Candidate authority, Flash Thinking profile, Stage 2 profile,
and resource bounds. v26.120 outcomes were not used for selection.

v26.122 materialized 24 TaskPackages, 48 Paths, one resource Contract, one execution Contract,
one Manifest, and 32 Jobs under fresh identities. TaskPackage, Path, and Job overlap with v26.118
is zero. The preserved 16,384-token request and 400,000-token rollout bounds contain every
complete path, including Final Primary and Rescue: the maximum conservative bound is 366,799 and
minimum headroom is 33,201 tokens. All 20 static mutations failed closed.

Finance v26.123 replayed 2,549/2,549 files before profile parsing, credential lookup, or client
construction: all 2,534 v26.122 transitive bindings, all thirteen v26.122 outputs, and two exact
Runner implementation files. Every Provider response is now split into a privacy-redacted
Envelope and a separate public payload Projection. The Envelope is atomically persisted before
payload privacy or response-Schema validation. It retains exact parent certificates, response
model, finish/status telemetry, Usage, Thinking telemetry, and public content hash/length, while
retaining no response payload, private reasoning content/hash, Raw HTTP body, or Raw request body.

After Envelope persistence, the Projection contains either one validated public payload, one
generic privacy rejection with no payload or rejected key, or one generic no-payload Provider
failure. A privacy rejection produces a complete model-result Raw Execution and preserves its call
and resource denominator. Complete Raw recovery reparses both artifact sets with zero calls; any
Envelope or Projection without Raw blocks retry as an orphan.

The 32-Job direct control made 256 scripted Stage 1 calls: 224 exact four-field Semantic Action
payloads, 224 accepted choices and reversible Commits, 192 public Observations, and 32 exact
two-field Final payloads. It persisted 256 Envelopes before 256 Projections and passed 32/32
Program closure, terminal verification, independent validity, and mechanism checks. These are
local fixtures, not Provider calls or empirical rows.

One malformed Final Primary used the one global ABI Rescue, retained the same Host Envelope and
JSON cue, and completed with independent validity. One exact-schema wrong answer crossed the
parser and independently failed validity. One injected reasoning-key payload retained model,
finish reason, Usage, and content hash/length in its Envelope, persisted neither payload nor exact
key, produced a complete model-result Raw terminal, and recovered with zero calls. A combined
malformed-ABI plus unknown-action control retained the first semantic failure and completed only
through the separate Semantic Recovery.

Completion Usage 16,384 and 16,385 was admitted, with 16,385 fully charged; 16,386 produced
Instrument failure and blocked later calls. Oversized Prompt, insufficient budget, reused
preparation, parent/certificate changes, private payload/hash persistence, Host answer insertion,
resource/model/recovery changes, and Stage 2 Provider routing failed closed. All 20 Runner
mutations were rejected with zero Provider or Stage 2 Provider calls.

Formal and independent v26.122 builds produced all thirteen outputs byte for byte; formal and
independent v26.123 builds produced all twelve outputs byte for byte. Focused Pytest passed 3/3 in
51.33 seconds; the selected v26.117-v26.123 adjacent regression passed 11/11 in 186.66 seconds;
focused Ruff and Mypy passed. Both stages made zero real Provider calls, zero Stage 2 Provider
calls, zero GPU jobs, and zero empirical rows.
...
Finance v26.120 consumed exactly the 32-Job Semantic Action engineering-calibration authorization
frozen by v26.119. Its final `--prepare-only` replay passed 2,228/2,228 files before credential
lookup or client construction: all 2,217 v26.119 transitive bindings, all ten v26.119 outputs, and
the exact v26.120 implementation. A 32-Job scripted control made 256 local Stage 1 calls, accepted
224 exact four-field choices, made 224 reversible same-action Commits and 192 Observations, and
passed 32/32 Replay, independent validity, and mechanism controls with zero real Provider calls.

The unchanged online attempt exposed all 32 Jobs but failed before aggregation. Thirty-one Jobs
have complete Raw Executions and five of those have checkpoint rows. One Semantic Reconciliation
`structured_direct` Job has six persisted Provider artifacts and no Raw Execution. The process
observed one additional parsed response whose `RawActionProviderCall` privacy validation failed
before artifact persistence. That response payload, exact rejected key, telemetry, Usage, and hash
were not retained. All 32 Job identities are retired. A completed v26.120 report does not exist,
and an exact 32-Job empirical terminal denominator is unavailable.

The 31 complete Raw Jobs all end Instrument failure. They bind 250 Provider artifacts; the
incomplete Job binds six, giving 256 artifact-backed calls and an operator-observed lower bound of
257 invocations. Persisted artifacts contain 225 HTTP successes and 31 HTTP 400 responses. All 256
requested and selected exact `deepseek-v4-flash`; all successful rows retain exact response model,
positive Thinking telemetry, complete Usage, native-tool absence, zero fallback, and zero
discovery. Persisted lower-bound Usage is 1,068,881 tokens: 727,617 Prompt, 341,264 Completion,
and 314,039 Reasoning. Estimated cost telemetry is a lower bound of USD
`0.19742030000000001688`. Private reasoning content and hashes, Raw HTTP bodies, and Raw request
bodies remain zero.

v26.121 independently rebuilt every current public state and exact presented Candidate order for
the 31 complete Raw Jobs. Their 188 semantic choices contain 187 visible-action and Decision-kind
matches, 184 accepted Primary choices, 186 reversible Commits, 155 Observations, 138 public-
progress choices, and 45 Program-node-progress choices. Thirty of 31 first actions were visible
and accepted; 15 made immediate progress. One stale-state and one unknown-action rejection each
used Semantic Recovery; both changed action and committed, and one made immediate progress.
Seventeen legal no-progress choices all continued through ordinary replanning and later progress.

The reached Candidate partition is 61 Singleton and 127 Multi-Candidate choices. Candidate counts
1/2/3/4/5/6/7/8 occurred in 61/21/44/19/9/24/1/9 choices. The model selected the Prompt-only
reference 116 times, a legal non-reference 71 times, and one invisible action once. Selected
positions 0/1/2/3/4/5/6/invisible occurred 119/25/23/10/7/2/1/1 times; selected description sizes
were 546-1,031 bytes with median 821. These are descriptive reached-state associations, not a
Candidate-load or position-causality estimate.

All 31 complete Raw Jobs closed the Program, completed the terminal node, selected and succeeded
at terminal verification, and committed Final. No Final answer crossed the parser. The exact
Primary Prompt requires exactly `rationale_summary` and `answer`, and all 31 public payloads use
that key set; the frozen parser instead requires exactly `stage`, `answer`, and `protocol`. All 31
are therefore rejected as `final_answer_not_exact_object_contract`. The Final Rescue exposes only
`answer`, remains bound to the same three-field parser, and omits a case-insensitive JSON lexical
cue under `response_format.type=json_object`; all 31 Rescue calls returned HTTP 400. Because HTTP
error bodies were not persisted, the missing cue is a strong prospective localization, not a
unique causal claim.

The incomplete Job localizes a separate persistence defect. Parsed public payload privacy
validation occurs before artifact write and before the telemetry ledger append. A response with a
reasoning-classified public key can therefore fail without a telemetry-only Raw artifact. The
future repair must retain the private-reasoning prohibition while persisting redacted telemetry
and public content hash/length before payload projection; rejected content itself must remain
unpersisted.

v26.121 replayed 2,523/2,523 files: all 2,228 v26.120 bindings, all 294 failed execution files,
and its exact implementation. It reparsed five checkpoint rows, 31 Raw Executions, 256 Provider
artifacts, 293 canonical JSON files, and 250 Raw descriptors. Formal and independent builds
produced all nine outputs byte for byte. Twelve destructive mutations failed with zero Provider or
Stage 2 Provider calls. Focused Pytest passed 2/2; focused Ruff and Mypy passed.

The authoritative identities are:

- report:
  `finance_v26_semantic_action_failure_audit_report:62abced772e0b281912749f041e56ab1038b2cc76489da4157bfcb6265efcf5a`;
- failed lineage:
  `finance_v26_semantic_action_failed_lineage:fa1849ebf76c4c336b74bf289da9be6f4969a775315c5bdd274b4eeb3b523593`;
- Provider telemetry:
  `finance_v26_semantic_action_failed_provider_telemetry:65ecbad99e33a13fcaad3aeff7f8d5e69fe9b409ae92402f28b90d9adda436cd`;
- public action outcomes:
  `finance_v26_semantic_action_public_outcome_audit:42ca2423b076d1a86e89c5bb9bf794098b1d3aa5ae9360c6903c2bad3ddcf907`;
- Final interface:
  `finance_v26_semantic_action_final_interface_audit:40d9413df66b0ea4c6c13f6b8d3078ae73b5d6d32c916b16f5579118edd0437e`;
- privacy persistence:
  `finance_v26_semantic_action_privacy_persistence_failure:42a3834d45b952ac6a914517e63be74e0be24c7f32633665823ec1de99154855`;
- transition:
  `finance_v26_semantic_action_failure_transition:4f893fe1d7a4e9250e9685b55661c8cca3b9a8e2189ee6fdbe69cc514543ec9f`.

At the v26.121 audit freeze, the only permitted transition was:

~~~text
fresh_privacy_first_capture_and_exact_final_response_grammar_taskpackage_contract_manifest_runner_preflight_only
~~~

The successor must preserve the exact Semantic Action protocol, complete Candidate authority,
model, Thinking profile, Completion and resource bounds, separate one-use recovery channels, and
zero-Provider Stage 2. It requires a shared exact Final Grammar across Primary, Rescue, and parser;
the Provider JSON-mode lexical cue; privacy-first telemetry persistence; fresh response-protocol,
TaskPackage, Contract, Manifest, Job, Runner, execution, and report identities; and a complete
credential-free Runner preflight before any Provider call. Historical v26.120 rerun, recovery,
continuation, or reclassification; Host semantic repair; model/resource/recovery changes; role,
State Mapping, training, release, and production Contribution remain forbidden. See
...
At the v26.119 preflight freeze, the only permitted transition was:
...
Finance v26.116 consumed no online authorization. It performed the credential-free diagnostic
requested after review of v26.114-v26.115. Before constructing a Proposal row it replayed
2,181/2,181 files: all 2,172 v26.115 transitive bindings, all eight v26.115 outputs, and the exact
v26.116 implementation. It looked up no credential, constructed no client, made zero Provider or
Stage 2 Provider calls, used zero GPU jobs, and reclassified zero empirical rows.

For each of the 54 historically ABI-accepted payloads, the audit independently rebuilt the exact
`PublicActionState` from the frozen Task, Environment, and public Observation prefix; matched its
identity to the dynamic pre-call certificate; reparsed the ten-field payload; and independently
compiled the resulting Semantic Proposal. All 54 state reconstructions and bindings passed. All
30 persisted Commits matched independent Compilation. Duplicate detection required exact public
Tool and Arguments equality against a prior failed Observation, including failures earlier than
the latest Observation.

The accepted Decision distribution is 42 `acquire_public_input` and twelve
`execute_public_operation`; none selected terminal verification or final answer. Twenty-nine
acquisitions and one Operation committed, giving Commit fractions `0.690476190476` and
`0.083333333333`. The complete 24-row accepted-without-Commit partition is ten Tool-argument
grammar failures, seven unresolved-Operation frontier selections, four operand-source grounding
failures, and three repeated failed public calls.

All ten Tool-grammar rows omitted the four exact top-level `query_structured_fact` fields and
instead supplied an alternate or wrapped argument shape. All seven frontier rows selected the
registered `operation_stage_01` Node with the correct Tool and available Operator before its
public symbols were resolved. All four operand rows used resolved Evidence identities instead of
the registered ordered source symbols. The three duplicates followed one
`structured_query_no_match` and two `typed_selector_requires_refinement` failures. No
accepted-without-Commit row used an unregistered Tool, nonexistent ready Node, unavailable
Operator, or unavailable Evidence.

Thirty of 32 Jobs produced an accepted Proposal, 19 produced a Commit, 15 produced a successful
Observation, and eight reached two or more Commits. The Commit-count partition is 13/11/5/3 Jobs
with 0/1/2/3 Commits. Of 29 acquisition Commits, 22 succeeded and 20 reduced unresolved public
symbols. The sole Operation Commit was a successful normalization. No verification or final
Proposal occurred, Program closure and independent validity remained zero, and all historical
...
- Action Selection failure audit:
...
Finance v26.114 consumed exactly the online transition authorized by v26.113. Its final
`--prepare-only` replay passed 2,049/2,049 files before credential lookup or client
construction: all 2,041 v26.113 transitive bindings, all seven v26.113 outputs, and the exact
v26.114 implementation. The exact Runner Contract, 32-Job denominator, response Grammar, model,
Thinking profile, Completion and rollout bounds, PublicActionState, task assignments, seeds, one
global Rescue, and zero-Provider Stage 2 remained unchanged.

The online run started at 0/32 with eight workers and zero Raw recovery. All 32 exact Jobs
completed after 81 HTTP-success Stage 1 calls: 62 Primary and 19 Rescue. Every call requested,
selected, and returned exact `deepseek-v4-flash`, used `thinking.type=enabled`, bound
`max_tokens=16384`, and had dynamic public-state, exact request-body, and cumulative resource
certificates before invocation. Fallback, discovery, Provider-native tools, transport failure,
typed no-call, Completion-unusable, Instrument failure, and Stage 2 Provider calls were zero.

The run used 577,078 Provider-reported tokens: 147,995 Prompt, 429,083 Completion, and 410,643
Reasoning tokens. Aggregate Reasoning/Completion fraction was `0.957024631598`; estimated cost
telemetry was USD `0.1398790904000000131`. Private reasoning content and hashes, Raw HTTP
bodies, and Raw request bodies were not persisted. Per-Job Usage was 7,287-42,008 tokens, leaving
217,992-252,713 tokens under the frozen 260,000 ceiling.

All 32 formal terminals are `model_invalid_trajectory`. Unlike v26.110, the exact response
interface was empirically crossed. The independently reproduced Funnel is 81 public JSON
payloads, 81 exact ten-field Key Sets, 81 exact protocols, 81 exact state bindings, 81 registered
Decision/conditional-field passes, 54 exact stage/Schema/Parser passes, 30 reversible Stage 2
Commits, 30 public Observations, zero Program closures, and zero independently valid
trajectories.

The 27 mechanical ABI failures are localized entirely to the fixed `stage` constant. Fifty-four
payloads used exact `semantic_decision_proposal`; the remaining values were sixteen `stage_1`,
six numeric `1`, three `stage_one`, one `stage_01`, and one response-protocol string. No
payload failed its field set, `protocol`, `state_id`, Decision enum, conditional semantic
fields, or null/empty defaults. ABI failures occurred at logical positions 0/1/2/3 with counts
9/12/3/3.

The 54 ABI-accepted Proposals produced 30 committed actions and 24 uncommitted semantic outcomes.
Twenty-one Jobs ended in semantic compile rejection: ten public calls violated the exposed Tool
grammar, seven selected an unresolved public Operation, and four changed registered public
operand sources. Three Jobs repeated a failed semantic Proposal. All 30 Commits independently
decompiled to the exact model Proposal with zero Host semantic insertion. Their 30 Observations
were 23 successes and seven typed failures. Verifier v3 Replay passed 32/32; there were two local
mechanism successes and five requested-path adherences, but no final answer or valid trajectory.

Thirteen Jobs used no Rescue. Nineteen used the one global Rescue; twelve recovered an
ABI-accepted Proposal and seven repeated the fixed-`stage` failure. One recovered Job later
encountered a second rescuable ABI failure after Rescue was consumed. This authorizes neither
additional Rescue nor a resource-bound change.

Finance v26.115 independently replayed 2,172/2,172 files: all 2,049 v26.114 transitive bindings,
all 122 v26.114 execution files, and its exact implementation. It reparsed 121 canonical JSON
files, one canonical 32-row checkpoint JSONL, 32 Job results, 32 Raw Executions, 81 Provider
artifacts, and 113 Raw descriptors. All hashes, parents, certificate triples, Usage, privacy
fields, terminals, and Stage 2 zero-call counts reproduced. Formal and independent builds
produced all eight outputs byte for byte. All 20 computed destructive mutations failed with zero
...
The successor may move only fixed `stage` metadata into a Host-bound immutable envelope. Host
alias normalization, missing semantic-field insertion, Tool/Node/Operator/Operand/Evidence
selection or repair, v26.114 rerun, profile/model/Completion/rollout change, additional Rescue,
and every Provider call remain forbidden until a fresh identity chain and exact credential-free
Runner preflight pass. Role experiments, State Mapping, release, and production Contribution
remain forbidden. This is now the historical v26.115 transition: v26.116 leaves the evidence and
Contract immutable but supersedes the current priority with a computed Semantic Action Selection
failure distribution and design-only transition. See
`docs/finance_v26_114_v26_115_exact_response_grammar_calibration_execution_and_audit.md`.

## v26.112-v26.113 Exact Response Grammar And Runner Preflight Decision

Finance v26.112 consumed only the credential-free transition authorized by v26.111. Before
Grammar compilation it replayed 2,029/2,029 files: all 2,017 v26.111 transitive bindings, all ten
v26.111 outputs, and two exact implementation files. It looked up no credential, constructed no
client, made zero Provider calls, used zero GPU jobs, and produced zero empirical rows.

The new strong `ExactStageOneSemanticProposalPayload` and the model-visible Grammar compiler share
one registered ten-field Schema and one conditional-rule table. Every response must contain exact
top-level fields `stage`, `state_id`, `decision_kind`, `tool_id`, `node_id`, `operator_id`,
`operand_sources`, `direct_arguments`, `evidence_ids`, and `protocol`. All fields are always
present; inapplicable values use the frozen `null`, `{}`, or `[]` form. Extra fields, wrappers,
multiple Proposals, Aliases, stale state, wrong protocol, and Decision/field mismatch fail closed.
The exact v2 response protocol is
`prospective_two_stage_stage_one_exact_response.v2`; the content-addressed Grammar is
`prospective_stage_one_response_grammar:641ea2e9b4391cc46026f7b0d187ba5f7a674fe28e271a4b58e37aeed87a6b52`.

Primary and Rescue expose the same exact Grammar. Primary retains the unchanged complete
`PublicActionState`. Rescue deterministically projects every currently selectable public semantic
affordance, exact state binding, and typed response failure while dropping low-level wire details,
the previous response, and private reasoning. It cannot choose a semantic field. A fixture that
reads only the final serialized Prompt, never the parser Schema or an internal Proposal, passed
324/324 Primary and 324/324 Rescue parses, 648/648 exact state bindings, and 324/324 identical
Primary/Rescue semantic projections. Rescue was shorter in 324/324 states. Maximum Primary and
Rescue sizes were 7,724 and 4,996 bytes.

v26.112 rematerialized 24 TaskPackages, 48 Paths, one Contract, one Manifest, and 32 Jobs under
fresh identities. Task, Path, and Job overlap with v26.108 is zero. Computed preservation checks
pass 24/24 source projections, 48/48 Compiler paths, and 32/32 Job assignments and seeds; all 104
Task/Path/Job rows retain the exact model, Thinking, Stage 2, and resource bindings. The unchanged
260,000-token resource ceiling still contains all complete paths: bounds are 157,409-258,646 with
minimum headroom 1,354. All sixteen Grammar and identity mutations failed closed.

Finance v26.113 then replayed 2,041/2,041 files before profile parsing, credential lookup, or
client construction: all 2,029 v26.112 transitive bindings, all ten v26.112 outputs, and two exact
Runner files. The future Runner retains dynamic public-state, exact request-body, and cumulative
resource certificates, one single-use authorization, Raw privacy-redacted telemetry before
projection, raw-only recovery, orphan rejection, one global Rescue, and zero Stage 2 Provider
calls.

Its 32-Job direct control made 256 scripted Stage 1 calls, including 224 exact ten-field semantic
payloads, 224 reversible Stage 2 Commits, and 192 public Observations. All 32 Jobs preserve the
Compiler semantic projection and final answer and pass Verifier v3 Replay, independently computed
validity, and mechanism scoring. A separate actual Runner control recovered from one invalid
Primary serialization through one exact-Grammar Rescue. The Rescue used the same exact v2
protocol.

All 256 Raw Provider fixtures bind their dynamic, exact request, and resource certificates and
pass privacy controls. Usage fixtures admit 16,384 and 16,385 Completion tokens, charge the latter
without clipping, reject 16,386 as Instrument failure, and block Rescue afterward. Complete Raw
recovery is byte-identical with zero calls; orphan artifacts, oversized Prompts, reused requests,
and insufficient budget fail before unauthorized behavior. Ten actual object-revalidation Runner
mutations failed with zero Provider and Stage 2 Provider calls.
...
- response Grammar:
  `prospective_stage_one_response_grammar:641ea2e9b4391cc46026f7b0d187ba5f7a674fe28e271a4b58e37aeed87a6b52`;
- v26.112 report:
  `finance_v26_exact_grammar_rematerialization_report:88b4a2cac6d43e46d52a45a0af7956787b78805893daf86af2ac01abde5e2f8f`;
- v26.112 Contract:
  `finance_v26_two_stage_execution_contract:a162af1661dd51e253cf77dff5628a73df1c92e2d335cc6aeadc000dfc952329`;
- v26.112 Manifest:
  `finance_v26_two_stage_manifest:3b9377e0d41e9af761df93a2009402c6ac3f7efa84d41695f7b192dc85d67416`;
- v26.113 report:
  `finance_v26_exact_grammar_runner_preflight_report:da5f397ce5082137ef0f343e240db57ce88e6f89178ac4a2b486118cdc5d24cf`;
- Runner Contract:
  `finance_v26_two_stage_runner_contract:02340ee6c14e53831b6057e92c8d2e694572cfbfb9e7432835cb7e6f28053506`;
- Runner fixture:
  `finance_v26_exact_grammar_runner_fixture:469544b4415e7ce773836374ed4ec82fd66b318e457066acc437500b5c7ce750`.

At the v26.113 preflight freeze, the only permitted transition was:

```text
exact_response_grammar_calibration_execution_only
```

This was positive response-constructibility and Runner-Instrument evidence, not empirical Flash
semantic-Proposal usability. v26.114 has now consumed its exact fresh 32-Job authorization, and
v26.115 independently audits the measured negative semantic result. The historical v26.113
transition is no longer current. No v26.110 rerun, role experiment, State Mapping, release, or
production Contribution is authorized. See
`docs/finance_v26_112_v26_113_exact_response_grammar_and_runner_preflight.md`.
...
At the v26.111 audit freeze, the only permitted transition was:

```text
fresh_exact_response_grammar_taskpackage_contract_manifest_and_runner_preflight_only
```

That transition has now been consumed by v26.112-v26.113. The successor exposed the exact
response field names, state binding, conditional rules, null/empty defaults, protocol field, and
one-proposal shape in both Primary and Rescue. It used fresh response-protocol, Prompt,
TaskPackage, Contract, Manifest, Job, Runner, execution, and report identities and completed the
exact credential-free Runner preflight before any Provider call. Host alias normalization, Host
semantic choice, v26.110 rerun, profile/model/Completion/
rollout changes, and Stage 2 Provider calls remain forbidden under the current v26.113
authorization. See
...
| v26.122 Semantic Action preservation | 324/324 exact action Prompt hashes, Candidate presentation orders, and reference Commits; 24/24 source, 48/48 Path, and 32/32 assignment/seed projections; v26.120 outcome selection reads 0 |
| v26.122 resource and identity chain | fresh 24 TaskPackages, 48 Paths, 1 Contract, 1 Manifest, 32 Jobs; zero v26.118 Task/Path/Job overlap; exact 16,384/400,000 bounds; maximum complete path 366,799 and minimum headroom 33,201 tokens |
| v26.123 source replay and binding | 2,549/2,549 files before profile/credential/client; exact model, Thinking, Action/Final Grammars, Candidate authority, 16K/400K resources, separate one-use recoveries, 32 Jobs, and zero-Provider Stage 2 bound |
| v26.123 direct Runner control | 32 Jobs, 256 scripted Stage 1 calls, 224 exact action payloads/accepted choices/reversible Commits, 192 Observations, 32 exact Final payloads; 32/32 closure/verification/validity/mechanism; zero real Provider/GPU |
| v26.123 privacy-first capture | 256 Envelopes persisted before 256 Projections; injected privacy rejection retained model/finish/Usage/content hash and length, persisted no payload/key/private reasoning, produced complete model-result Raw and zero-call recovery; orphan blocked |
...
| v26.120 Usage lower bound | 1,068,881 persisted tokens: 727,617 Prompt, 341,264 Completion, 314,039 Reasoning; USD 0.19742030000000001688 estimated cost; one unjournaled response Usage unknown |
| v26.121 Semantic Action outcomes | 31 complete Raw, 188 choices, 187 visible/Decision matches, 184 accepted Primary, 186 reversible Commits, 155 Observations, 138 public-progress and 45 node-progress choices; 31 Program closures and terminal verifications |
| v26.121 first choice and recovery | first action visible/accepted/progress in 30/30/15 of 31 Jobs; 1 stale-state and 1 unknown-action rejection; 2 separate recoveries changed action and committed, 1 made immediate progress; all 17 legal no-progress choices replanned to later progress |
| v26.121 Candidate diagnostics | 61 Singleton and 127 Multi choices; count distribution 61/21/44/19/9/24/1/9 for 1-8 Candidates; 116 reference, 71 non-reference, 1 invisible; selected position 0 in 119/188; selected length 546-1,031 bytes, median 821; descriptive only |
| v26.121 Final interface | 31/31 Primary payloads matched visible `answer,rationale_summary` but parser required `stage,answer,protocol`; 31 parser rejects; all 31 Rescue Prompts lacked JSON lexical cue and returned HTTP 400; error-body absence prevents unique Provider-cause claim |
| v26.121 privacy persistence | incomplete Job has 6 exact persisted payloads plus 1 operator-observed unjournaled parsed response; payload privacy validation preceded artifact write and telemetry append; rejected payload/key/hash/Usage absent; orphan retry denied with zero calls |
| v26.121 independent audit | 2,523/2,523 files; 5 checkpoint rows, 31 Raw, 256 Provider, 293 canonical JSON, and 250 descriptors replayed; formal/independent 9 outputs byte-identical; 12/12 mutations rejected; zero Provider/Stage 2/GPU |
| v26.120-v26.121 focused validation | v26.120 focused 2 passed in 53.50 seconds and selected adjacent 4 passed in 106.86 seconds; v26.121 focused 2 passed in 16.24 seconds; focused Ruff and Mypy passed |
| v26.121 transition | fresh privacy-first capture and exact Final-response Grammar TaskPackage/Contract/Manifest/Runner preflight only; no Provider call, historical recovery, semantic protocol/Candidate/model/resource change, role, State, training, release, or production authorization |
...
| v26.119 historical transition | exact fresh 32-Job Semantic Action engineering calibration authorization consumed by failed v26.120; immutable preflight retained; v26.121 repair priority consumed by v26.122-v26.123 |
...
| v26.116 Action Selection failures | 24 accepted without Commit: 10 Tool-argument grammar, 7 unresolved frontier, 4 operand grounding, 3 duplicate failed calls; 0 unregistered Tool, nonexistent ready Node, unavailable Operator, or Evidence |
| v26.116 trajectory progression | 30 Jobs with accepted Proposal, 19 with Commit, 15 with successful Observation, 8 with at least 2 Commits, maximum 3; 20 acquisitions reduced unresolved symbols; 0 Program closure and validity |
...
| v26.114 residual mechanical ABI | 27/81 failures, all fixed `stage` constant only; logical positions 0/1/2/3 = 9/12/3/3; zero field-set/protocol/state/conditional/default failures |
| v26.114 semantic Proposal result | 21 compile rejections: 10 Tool-grammar, 7 unresolved-Operation, 4 operand-source; 3 duplicate failed Proposals; 0 Program closures and valid trajectories |
| v26.114 Rescue and resources | 13 no-Rescue Jobs; 19 Rescue Jobs, 12 recovered and 7 failed; 1 later second rescuable failure; 217,992-252,713 observed rollout headroom |
| v26.114 Provider/authority Gates | exact model, Thinking, Usage, native-tool absence, fallback absence, dynamic/exact/resource binding, Budget Adequacy, 30/30 reversible Commits, privacy, and Stage 2 zero-call passed |
| v26.114 completed-run replay | resumed 32/32 without credential or client; zero new Provider calls; execution report `7b531b18...a5ca3` unchanged |
| v26.115 source and lineage replay | 2,172/2,172 files; 121 canonical JSON plus 1 checkpoint JSONL; 32 results, 32 Raw, 81 Provider, 113 descriptors; all parents/certificates/Usage/privacy reproduced |
| v26.115 independent audit | formal and independent 8-file builds byte-identical; independent ABI diagnosis agreed 81/81; 30/30 Commit decompiles and 32/32 Verifier v3 Replay passed |
| v26.115 destructive controls | 20/20 computed source/lineage/ABI/Commit/privacy/authorization mutations rejected with zero Provider and Stage 2 Provider calls |
| v26.112-v26.115 focused validation | 8 passed; focused Ruff format/check and Mypy passed for both v26.114-v26.115 implementation modules |
| v26.103-v26.115 adjacent regression | 63 passed against the canonical complete immutable artifact root |
| v26.115 historical transition | fresh Host-bound fixed-`stage` metadata semantic-Proposal protocol and credential-free preflight only; current priority superseded by v26.116 without changing the immutable v26.115 Contract |
| v26.112 source replay | 2,029/2,029 files: 2,017 v26.111 transitive bindings, 10 v26.111 outputs, and 2 exact implementation files; zero credential/client/API/GPU |
| v26.112 exact response Grammar | one strong ten-field v2 Schema; exact field names, protocol/stage constants, state copy, types, defaults, four conditional rules, one-object/no-extra/no-wrapper rules all model-visible |
| v26.112 Prompt-only constructibility | 324 Primary plus 324 Rescue parses; 648/648 state bindings; 324/324 semantic matches and shorter Rescues; maximum 7,724/4,996 bytes |
| v26.112 fresh identity and resources | 24 TaskPackages, 48 Paths, 1 Contract, 1 Manifest, 32 Jobs; zero corresponding v26.108 identity overlap; 157,409-258,646-token bounds and 1,354 minimum headroom |
| v26.112 dual build and destructive controls | all 10 outputs byte-identical; 16/16 exact Grammar/identity mutations rejected; zero credential/client/API/GPU and empirical rows |
| v26.113 source replay | 2,041/2,041 files: 2,029 v26.112 transitive bindings, 10 v26.112 outputs, and 2 exact Runner implementation files |
| v26.113 exact Runner control | 32 Jobs, 256 scripted Stage 1 calls/certificate triples, 224 exact semantic payloads and reversible Stage 2 Commits, 192 Observations; 32/32 Replay, validity, mechanism, final-answer, and path passes |
...
| v26.113 historical transition | exact fresh 32-Job response-Grammar calibration authorization consumed by v26.114; no v26.110 rerun, bound/profile change, role experiment, State Mapping, release, or production authorization |
...
response-grammar omission. v26.112 has now consumed that credential-free transition by exposing
the exact ten-field response shape, state binding, conditional requirements, null/empty defaults,
protocol, and one-proposal rule from one strong Schema in both Primary and Rescue. It
rematerialized every affected identity while preserving the profile, 16K Completion request,
260K rollout ceiling, semantic authority boundary, all task/path/seed projections, and Stage 2
zero-Provider route.

v26.113 preflighted the exact future Runner with 32/32 scripted Jobs, 224/224 exact semantic
payloads and reversible Commits, 32/32 Replay/validity/mechanism passes, complete certificate,
Usage, privacy, Rescue, recovery, and destructive controls, and zero real Provider calls.
v26.114 has consumed that exact 32-Job authorization. Its 54 ABI-accepted Proposals and 30
reversible Commits establish measured negative semantic behavior, while its 27 ABI failures are
localized solely to the fixed `stage` constant. v26.115 independently reproduced the full
execution. Its Host-bound fixed-`stage` metadata preflight remains an immutable historical
transition, but v26.116 supersedes the current priority after independently reconstructing the 54
accepted Proposals and all 24 uncommitted outcomes.

At its historical freeze, v26.116 permitted only credential-free Semantic Action Selection
protocol design. The design could
consume the measured Tool-argument grammar, unresolved-frontier, operand-grounding, and
duplicate-failed-call distributions, but it may not normalize a historical response or turn any
diagnostic category into a Host-selected Tool, Node, Operator, Operand, Evidence item, or repair.
Response-Grammar optimization, prioritizing fixed-`stage` metadata work, v26.114 rerun,
model/Profile/Completion/rollout changes, additional Rescue, every Provider and Stage 2 Provider
call, role experiments, State Mapping, training, and release remain forbidden.
...
calibration, and v26.111 is its zero-generation independent post-run audit. v26.112-v26.119 close
the exact response and Semantic Action chain, v26.120 is the failed online attempt, v26.121 is its
independent failure audit, v26.122 is the fresh exact Final-Grammar rematerialization, and v26.123
is the zero-generation privacy-first Runner preflight. None freezes a Thinking role protocol. The
only online experiment currently authorized is the exact fresh v26.124 32-Job engineering
calibration; no role or downstream experiment is authorized. Task depth and capability
informativeness remain unresolved independently of the response channel.
...
- `src/trusted_synthesis/experiments/vtdo_experiment/phase1_v26_final_grammar_privacy_rematerialization.py`
- `src/trusted_synthesis/experiments/vtdo_experiment/phase1_v26_privacy_first_exact_final_execution.py`
- `src/trusted_synthesis/experiments/vtdo_experiment/phase1_v26_privacy_first_exact_final_runner_preflight.py`
- `tests/test_v26_exact_final_grammar_and_privacy_first_runner.py`
- `artifacts/vtdo_experiment/finance_v26_122_final_grammar_privacy_rematerialization_v1_20260823/`
- `artifacts/vtdo_experiment/finance_v26_123_privacy_first_exact_final_runner_preflight_v1_20260823/`
...
## v26.194 Authoritative Execution Kernel Parent Binding Preflight
