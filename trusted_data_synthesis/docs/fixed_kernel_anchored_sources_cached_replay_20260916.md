...
运行中对PID4185998的一次只读进程观测记录RSS=264,079,816KiB（约251.85GiB），
当时全机可用内存仍约712GiB。这是一次RSS观测，不是严格测得的全程主存峰值；
19.289GiB的CUDA reserved峰值不能当作全部内存开销。210.09秒也不是360条完整
反馈的实测工期，不能按8token简单线性外推。此实现采用通用saved-tensor CPU
offload，存在重复保存冻结权重/历史张量的潜在开销；尚未通过分项分析确定占比。

下一阶段应在不改变随机策略、完整前缀导数和容差的前提下优化保存张量与缓存图
管理，再补齐每轮360条生成封存、独立评分、同点gJ反传与真实训练日程。单个反例
已修复，但这条完整生产路径仍未完成；当前没有自适应训练或金融反馈会话在跑。
