...
源码提交`203e6fbebb8756328026d28859ed99045b99eae2`已快进合入并推送远端main。
合入前后核对表述阶段冻结的1,840个既有源码/测试依赖，全部SHA一致；旧产物没有改写。

持续入口于2026-09-12 15:49:20.157771 UTC实际启动，PID726630；
启动记录为`follow_started:9e18b242e3162db514f6992201a02ee073b049fdd55cac156a487e197ea65956`。
原Teacher进程PID644313保持运行，未再次调用collect。
15:49:50.194421 UTC的后续观察为完成3,182、待执行21,450、运行8；
共同保守扣账49,345,745，余额950,654,255，用量未知Teacher请求3，无全局停止。
该观察与启动记录保存在独立runtime目录，不能代替原固定批次最终manifest。

启动后实际检查：follow_failed不存在，Student科学运行目录尚不存在。
因此现在的真实状态是“原采集运行中，后继闭环已启动等待”，不是“已物化/训练/确认完成”。
达到原批次和材料终局后，后台入口才按前述固定路径继续；不会由聊天结束触发另一套采集。
