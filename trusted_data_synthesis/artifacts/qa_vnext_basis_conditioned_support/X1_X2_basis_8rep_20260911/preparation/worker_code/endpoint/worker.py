...
from model_contract import REQUESTED_MODEL, response_model_matches  # noqa: E402
...
def generate(document, output, credential=None, scripted=None, model=REQUESTED_MODEL):
    messages = [
    ...
    for index in range(LIMITS["model_responses"]):
        ...
        try:
            ...
            if (
                ...
                or (scripted is None and not response_model_matches(model, envelope.get("model")))
            ):
...
def main():
    ...
    parser.add_argument("--model", choices=[REQUESTED_MODEL], default=REQUESTED_MODEL)
