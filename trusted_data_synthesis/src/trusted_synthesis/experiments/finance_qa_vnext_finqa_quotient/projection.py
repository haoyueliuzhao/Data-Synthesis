...
def _project(session, review):
    ...
    for position, turn in enumerate(turns):
        ...
        if model["kind"] == "action":
            ...
            if operation not in {"read", "multiply", "subtract"} or model["parameters"]:
            ...
            for slot, ref in enumerate(model["inputs"]):
                ...
                port = "factor" if operation == "multiply" else f"input:{slot}"
    ...
    for source, target, role in graph["edges"]:
        if target in selected and (
            role.startswith(("input:", "factor:")) or role == "answer_claim"
        ):
