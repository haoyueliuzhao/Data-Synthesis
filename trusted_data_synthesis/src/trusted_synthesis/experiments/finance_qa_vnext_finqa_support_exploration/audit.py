...
from trusted_synthesis.experiments.finance_qa_vnext_finqa_reason_policy.runtime import (
    VERSION as REASON_POLICY_VERSION,
)
from trusted_synthesis.experiments.finance_qa_vnext_finqa_reason_policy.runtime import (
    parse,
...
def verify_session(panel, registration, directory, *, model_required=True):
    ...
    config = configuration(registration["exploration_stratum"])
    ...
    for index in range(result["submissions"]):
        ...
        verify_binding_view(request, events, registration["view_condition"])
        events.append(event)
    ...
    binding_view_http_exposures = nonempty_binding_view_http_exposures = (
    ...
    for attempt in ledger["attempts"]:
        ...
        require(
            json.loads(body)["messages"][0] == {"role": "system", "content": config.system_prompt},
            "audit.exact_N_D_prompt",
        )
        require(
            http["model_configuration_id"] == config.as_record()["id"],
            "audit.exact_N_D_configuration",
        )
        ...
        verify_binding_view(
            request, events[: request["state"]["submission_count"]], registration["view_condition"]
        )
        if VIEW_KEY in request:
            binding_view_http_exposures += 1
            nonempty_binding_view_http_exposures += bool(request[VIEW_KEY]["results"])
            binding_view_content_bytes += len(canonical_json_bytes(request[VIEW_KEY]))
    ...
    return record(
        "finqa_support_exploration_session_audit",
        exploration_stratum=registration["exploration_stratum"],
        exploration_prompt_http_exposures=ledger["provider_attempt_count"]
        if registration["exploration_stratum"] == "D"
        else 0,
        presentation_version=VERSION,
        view_condition=registration["view_condition"],
        binding_view_http_exposures=binding_view_http_exposures,
        nonempty_binding_view_http_exposures=nonempty_binding_view_http_exposures,
        binding_view_content_bytes=binding_view_content_bytes,
        expression_contract_version=REASON_POLICY_VERSION,
