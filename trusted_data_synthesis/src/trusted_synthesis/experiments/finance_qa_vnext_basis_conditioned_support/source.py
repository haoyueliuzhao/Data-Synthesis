"""Read-only original interactions from the new basis-conditioned batch only."""
...
def batch_path(label):
    require(label in LABELS, "source.registered_candidate_only")
    return OUTPUT
...
def bind(root, row):
    ...
    require(row["task_key"] in TASKS and row["arm"] in SYSTEMS, "source.new_condition_and_task")
    require(
        type(row.get("replicate")) is int
        and label == f"{row['arm']}_{row['task_key']}_{row['replicate']:02d}",
        "source.registered_label_metadata_binding",
    )
    require(row["condition_id"] == condition(row["arm"])["id"], "source.exact_frozen_condition")
    public = read_json(base / f"preparation/public/{row['task_key']}.json")
    ...
    for index, path in enumerate(sorted((directory / "turns").glob("*_event.json"))):
        ...
        binding = record(
            ...
            source_batch="new_basis_conditioned_32",
