"""New-condition accounting with inherited Flash rates, never a current quote or invoice."""
...
from .plan import GENERATION_CONDITIONS, LABELS, MODEL, TASKS, read_json, registrations, require


def _registrations(online):
    """Bind the exact new 32 rows; guidance never becomes an old N/E condition."""
    preparation = online.parent / "preparation"
    public = {task: read_json(preparation / f"public/{task}.json") for task in TASKS}
    frozen = read_json(preparation / "registrations.json")
    observed = read_json(online / "launch.json")["registrations"]
    require(
        observed == frozen == registrations(public)
        and len(observed) == 32
        and [row["label"] for row in observed] == list(LABELS),
        "cost.exact_frozen_new_32_registration_identity",
    )
    return observed
...
def cost_summary(online_directory):
    ...
    online_directory = Path(online_directory)
    rows = []
    for registration in _registrations(online_directory):
        require(registration["requested_model"] == MODEL, "cost.fixed_flash_model")
        require(
            registration["arm"] in GENERATION_CONDITIONS, "cost.registered_instruction_condition"
        )
    ...
    return {
        ...
        "price_checked_date_role": "inherited historical lookup date, not rechecked for this study",
...
def costs(online):
    """Apply the frozen hourly assumptions to actual UTC reservation timestamps."""
    online = Path(online)
