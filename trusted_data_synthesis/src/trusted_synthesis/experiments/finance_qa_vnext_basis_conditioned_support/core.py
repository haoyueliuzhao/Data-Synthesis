...
class Store:
    ...
    def seal(self, **fields):
        ...
        value = record("basis_stage_manifest", members=members, **fields)
...
def history_guard(root):
    paths = [
        ...
        *[":(exclude)" + p for p in (PACKAGE, OUTPUT, DOCUMENT, *TESTS)],
    ]
    for command in (
        ...
        require(
            not subprocess.check_output(command, cwd=root), "history.only_new_basis_study_changes"
        )
...
def implementation(root):
    paths = subprocess.check_output(
        ["git", "ls-files", "--", "trusted_data_synthesis/src/**/*.py", *TESTS], cwd=root, text=True
    ).splitlines()
    require(paths and set(TESTS) <= set(paths), "implementation.tracked_sources_and_test")
    require(
        not subprocess.check_output(
            ["git", "status", "--porcelain", "--", "trusted_data_synthesis/src", *TESTS, DOCUMENT],
            cwd=root,
        ),
    ...
    return record(
        "basis_frozen_implementation", commit=commit, references=[reference(root, p) for p in paths]
    )
