...
def generate(document, output, credential=None, scripted=None):
    messages = [
    ...
    for index in range(LIMITS["model_responses"]):
        body = encode(request_body(messages))
        ...
        reservation = record(
            ...
            reserved_token_allowance=107520,
        )
        ...
        write(output, prefix + "_http_response.body", reply["body"])
        outcome = {
            "index": index,
        ...
        try:
            ...
            envelope = strict_json(reply["body"])
            ...
            if (
                ...
                or (
                    scripted is None
                    and envelope.get("model") not in {"deepseek-v4-pro", "deepseek-v4-pro-0813"}
                )
...
def main():
    ...
    result = generate(document, args.output, credential, scripted)
    print(
        json.dumps(
            {
