"""Collect twenty-four fixed Flash/high A/T sessions, then evaluate only after generation."""
...
from .instructions import SYSTEMS, capsule_files
...
def prepare(root):
    ...
    store.json(
        "private/review_ids.json", {label: f"R{i + 1:03d}" for i, label in enumerate(shuffled)}
    )
    store.write(
    ...
    review = Path(AUDIT_PATH).read_bytes()
    require(sha(review) == AUDIT_SHA256, "stage.exact_external_review")
    store.write("external_review.txt", review)
    bundles = {}
    for arm in CONDITIONS:
        members = []
        for name, data in capsule_files(root, arm).items():
            store.write(f"worker_code/{arm}/{name}", data)
        ...
        bundles[arm] = {
            ...
            "common_only_T_system_append": arm == "T",
        }
...
def launch_worker(root, registration, credential):
    ...
    require(registration["requested_model"] == MODEL, "stage.flash_only")
    require(registration["arm"] in CONDITIONS, "stage.known_instruction")
    code = prep / "worker_code" / registration["arm"]
...
def collect(root):
    ...
    for replicate in (1, 2):
        with ThreadPoolExecutor(max_workers=12) as pool:
            futures = {
                pool.submit(launch_worker, root, r, credential): r
                for r in rows
    ...
    require(known_count <= 768, "stage.attempt_cap")
    result = record(
        "collection_summary",
        registrations=24,
        rows=ordered,
        ...
        no_resampling=True,
        task_weights_apply_separately_per_condition=True,
        no_student=True,
    )
