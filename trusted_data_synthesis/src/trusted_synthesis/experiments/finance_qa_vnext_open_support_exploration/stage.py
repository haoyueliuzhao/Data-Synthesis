...
def prepare(root):
    ...
    require(len(rows) == 24 and len({r["label"] for r in rows}) == 24, "prepare.all_registrations")
...
def collect(root):
    with execution_guard(online=True) as counts:
        ...
        require(
            all(n <= 32 for n in attempts.values()) and sum(attempts.values()) <= 768,
        ...
        store.json(
            "execution_guards.json", guard_report(counts, phase="fixed_24_new_open_sessions")
        )
