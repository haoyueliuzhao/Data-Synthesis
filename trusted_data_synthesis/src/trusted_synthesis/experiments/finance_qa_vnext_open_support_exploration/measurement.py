...
provenance and the exact full 24-row cohort are asserted by the stage caller;
...
def measure(projections, source_rows):
    """Keep m/4 validity yield separate from n/m conditional class mass.

    The stage supplies all 24 new registrations, including terminated/unknown rows
    without a usable result. This function permits smaller synthetic input sets but
    never increases the four-replicate task budget. It does not infer validity from
