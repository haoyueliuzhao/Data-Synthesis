...
def assess(root):
    ...
    with execution_guard(online=False) as counts:
        ...
        require(
            summary["all_workers_terminated"] and summary["registered"] == 24,
...
def finalize(root, review_path):
    ...
    with execution_guard(online=False) as counts:
        ...
        require(set(reviews) == set(LABELS), "finalize.all_24_reviews")
        ...
        require(
            len(rows) == 24 and all(sum(r["task_key"] == key for r in rows) == 4 for key in TASKS),
        ...
        for key in TASKS:
            ...
            by_task[key] = {
                ...
                "valid_count": n_valid,
                "valid_yield": str(Fraction(n_valid, 4)),
        ...
        report = record(
            ...
            valid_yield=str(Fraction(valid_count, 24)),
            by_task=by_task,
            costs=costs_report,
            ...
            observation_scope=(
                "24 new development-panel sessions; finite support reachability and original "
