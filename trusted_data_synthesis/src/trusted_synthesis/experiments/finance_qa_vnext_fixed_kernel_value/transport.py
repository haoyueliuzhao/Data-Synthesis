...
class FatalMaterialError(RuntimeError):
    global_fatal = True
...
class Provider:
    ...
    def __call__(self, messages, context):
        ...
        try:
            ...
            cancellation = self.ledger.cancel_unsent(request_id, reason)
            self.ledger.halt("before_HTTP:" + type(error).__name__)
            try:
                p.write_once(directory / "unsent_cancellation.json", cancellation)
            except OSError:
                # The immutable ledger remains the authoritative no-send record.
                pass
            raise FatalMaterialError("fixed_kernel_transport.unsent_preparation_failed") from error
        ...
        try:
            ...
            if response.status_code in {400, 401, 402, 403}:
                raise FatalMaterialError(
                    "fixed_kernel_transport.fixed_request_or_auth_billing_HTTP_"
            ...
            receipt = p.record(
                ...
                live_http_sender=self.live_http,
                freeze_id=self.ledger.freeze_id,
                purpose="fixed_empirical_material_kernel",
            )
            p.write_once(directory / "receipt.json", receipt)
            if echoed:
                raise FatalMaterialError("fixed_kernel_transport.credential_echo_not_original_data")
            if model != p.MODEL:
                raise FatalMaterialError("fixed_kernel_transport.fixed_model_identity")
            if not known:
                raise FatalMaterialError("fixed_kernel_transport.known_usage_within_reservation")
...
def verify_origin(session, ledger, output):
    ...
    for index, turn in enumerate(turns):
        ...
        request = p.checked(
            json.loads((directory / "request.json").read_bytes()), "material_public_request"
        )
        response = p.checked(
            json.loads((directory / "public_response.json").read_bytes()),
            "material_public_response",
        )
        receipt = p.checked(
            json.loads((directory / "receipt.json").read_bytes()), "material_request_receipt"
        )
