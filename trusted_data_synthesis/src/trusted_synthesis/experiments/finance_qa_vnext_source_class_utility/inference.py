...
def evaluate_model(root, variant, model, model_identity, phase):
    require(phase in {"dev", "confirm"}, "generation.registered_phase")
    output = root / OUTPUT
    directory = output / "generation" / phase / variant
    require(not directory.exists(), "generation.no_repeated_session_or_model")
    binding, _, tokenizer = load_bound_assets(root)
    config = read_json(output / "preparation/evaluation_configuration.json")
    require(config == evaluation_config(), "generation.frozen_configuration")
    registrations = read_json(output / "preparation/evaluation_registrations.json")[phase]
    require(len(registrations) == (12 if phase == "dev" else 24), "generation.fixed_panel")
    if phase == "confirm":
        authorization = read_json(output / "selection/confirmation_authorization.json")
        require(variant in authorization["variants"], "generation.only_sealed_confirm_selection")
        expected = authorization["training_report_ids"][variant]
        require(
            model_identity["training_report_id"] == expected,
            "generation.selected_checkpoint_identity",
        )
    model.eval()
    ...
    for registration in registrations:
        ...
        require(
            ...
            "generation.actual_sealed_public_question",
        )
        result = run_session(public, directory / "sessions" / key, decoder, model_identity)
        rows.append(
            {
                "phase": phase,
                "variant": variant,
        ...
        print("generation", phase, variant, key, "finished", flush=True)
    report = record(
    ...
    seal_directory(store, kind="source_class_generation_manifest", report_id=report["id"])
    return report
