"""New-batch open-history targets using the frozen exact tokenizer algorithm.
...
from .plan import TRAIN_SYSTEM as SYSTEM
from .plan import read_json, record, require, sha
...
def _policy(binding):
    require(MAX_SEQUENCE_LENGTH == 32_768, "support_tokens.existing_length_policy")
    require(
        ...
        "support_tokens.existing_assets_and_position_authority",
    )
    return record(
        "source_class_token_policy",
        ...
        input=(
            "same-interaction actual HTTP request.messages, including unchanged original N SYSTEM"
        ),
...
def load_bound_assets(root):
    ...
    for relative, byte_count, digest in (
        ...
        path = root / relative
        require(path.is_file() and not path.is_symlink(), "support_tokens.bound_file")
        raw = path.read_bytes()
        require(len(raw) == byte_count and sha(raw) == digest, "support_tokens.bound_file_bytes")
        objects.append(read_json(path))
        require(path.read_bytes() == raw, "support_tokens.bound_file_changed")
    ...
    require(
        ...
        "support_tokens.inherited_policy_binding",
    )
...
def _candidate(candidate):
    require(isinstance(candidate, dict), "support_tokens.candidate_shape")
    for key in ("id", "task_key", "session_label", "source_closeout_id"):
        require(
            isinstance(candidate.get(key), str) and bool(candidate[key]), "support_tokens.identity"
        )
    require(
        candidate.get("population") == "N"
        ...
        "support_tokens.population_and_response",
    )
    ...
    require(
        ...
        and messages[-1]["role"] == "user",
        "support_tokens.original_open_messages",
    )
    require(
        messages[0]["content"] == SYSTEM,
        "support_tokens.original_condition_system",
    )
    target = candidate.get("target_response")
    require(isinstance(target, str) and bool(target), "support_tokens.target_text")
    require(
        sha(target.encode("utf-8")) == candidate["raw_response_sha256"],
        "support_tokens.target_bytes",
    )
    require(
        candidate.get("response_kind")
        in {"message", "calculate", "read_source", "notebook", "Final"},
        "support_tokens.positive_response_kind",
    )
    ...
    content_identity(candidate, "support_tokens.original_candidate_content_identity")
...
def encode_candidate(candidate, binding, policy, tokenizer):
    ...
    require(policy == _policy(binding), "support_tokens.frozen_policy")
    require(tokenizer.chat_template == binding["chat_template"], "support_tokens.runtime_template")
    ...
    require(full == prefix + target + assets.CHAT_SUFFIX, "support_tokens.rendered_content_changed")
    ...
    for left, right in offsets[target_start:target_end]:
        require(
            ...
            "support_tokens.target_offset_coverage",
        )
        covered = max(covered, right)
    require(covered == end, "support_tokens.target_offset_coverage")
    target_ids = ids[target_start:target_end]
    require(
        not set(target_ids) & set(tokenizer.all_special_ids), "support_tokens.target_special_token"
    )
    decoded = tokenizer.decode(
        target_ids, skip_special_tokens=False, clean_up_tokenization_spaces=False
    )
    require(decoded.encode("utf-8") == target.encode("utf-8"), "support_tokens.target_decode")
    require(ids[target_end:] == assets.SUFFIX_TOKEN_IDS, "support_tokens.suffix_tokens")
    require(encoded["attention_mask"] == [1] * len(ids), "support_tokens.unexpected_padding")
    ...
    require(mask[0] == 0 and sum(mask[1:]) == len(target_ids), "support_tokens.causal_shift")
    fits = len(ids) <= MAX_SEQUENCE_LENGTH
    return record(
        "new_support_token_representation",
