"""Two frozen rounds; unchanged single-session execution, qualification and exporters.

Population-specific orchestration is intentionally separate from the immutable
twelve-session experiment. No old report denominators or calibration callback.
"""
...
from ..finance_qa_vnext_model_execution.models import TASK_GROUPS, read_json, record, require
from ..finance_qa_vnext_model_execution.plan import (
    seal_directory,
    verify_directory,
    verify_source_snapshot,
)
from ..finance_qa_vnext_model_execution.qualification import (
    compare_qualified_sessions,
    qualify_session,
)
from ..finance_qa_vnext_model_execution.representation import export_candidates, tokenize_candidates
from ..finance_qa_vnext_model_execution.runner import (
    _credential,
    _run_session,
    _session_start,
    _share_support,
...
from .measurement import progress, request_presentation
from .plan import STAGE, _prepared
...
def summarize(
        ...
        task_rows.append(
            {
                ...
                **{
                ...
                "complete_success_proportion": successes / 2 if decidable else None,
    ...
    return record(
        ...
        registered_session_denominator=6,
        fixed_task_denominator=3,
        equal_task_weight_mean=sum(item["success_numerator"] for item in task_rows) / 6
        if complete
        else None,
...
def analyze(
    ...
    require(len(pairs) <= 3, "analysis.same_task_pair_budget")
    store.json(
    ...
    require(total_attempts is None or total_attempts <= 192, "analysis.total_attempt_budget")
    actual_usage = {}
    for key in (
        "prompt_tokens",
        "completion_tokens",
    ...
    scientific = {
        ...
        "all_three_selected_tasks_have_complete_witness": summary[
            "selected_tasks_with_success_witness"
        ]
        == 3,
    }
    report = record(
        ...
        full_six_session_execution_complete=summary["complete_decidable_population"],
