"""Two fresh B sessions in one frozen concurrent round; unchanged execution and qualification."""
...
from .plan import STAGE, TASK_GROUPS, _prepared
...
def run(root: Path, preparation_directory: Path) -> dict[str, Any]:
    ...
    for round_number in range(1, 2):
        ...
        futures = []
        with ThreadPoolExecutor(max_workers=2, thread_name_prefix="qa-vnext") as executor:
...
def summarize(
    ...
    require(len(qualifications) == 2, "summary.registered_denominator")
    ...
    return record(
        ...
        registered_session_denominator=2,
        fixed_task_denominator=1,
        equal_task_weight_mean=sum(item["success_numerator"] for item in task_rows) / 2
        if complete
        else None,
...
def analyze(
    ...
    require(len(pairs) <= 1, "analysis.same_task_pair_budget")
    store.json(
    ...
    require(total_attempts is None or total_attempts <= 64, "analysis.total_attempt_budget")
    actual_usage = {}
    for key in (
        "prompt_tokens",
        "completion_tokens",
    ...
    scientific = {
        ...
        "branch_complete_reachability_witness": summary["selected_tasks_with_success_witness"] == 1,
    }
    report = record(
        "pilot_report",
        stage=STAGE,
        ...
        full_two_session_execution_complete=summary["complete_decidable_population"],
