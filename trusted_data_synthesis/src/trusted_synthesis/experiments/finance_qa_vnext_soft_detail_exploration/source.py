...
def bind_session(root, row, *, qualifying_only=True):
    """Bind only a joint-valid new session; missing/unknown sessions are not read as valid."""
    root = Path(root)
    require(
        not qualifying_only or row["formula_driven_trace_verified"] is True,
        "new_source.original_valid_only",
    )
    content_identity(row, "new_source.closeout_identity")
    require(
        row["arm"] in ARMS and row["label"].startswith(row["arm"] + "_" + row["task_key"] + "_"),
    ...
    expected = [
        {"role": "system", "content": SYSTEMS_NEW[row["arm"]]},
...
def bind_execution_prefix(root, row):
    """Retain verified actual executed events even if a later response never reaches Final.

    This is not a positive-package binder. Absent events/responses remain absent;
    only available, fully joined original request/response/event records enter.
    The caller has already checked the full online phase manifest.
    """
    root = Path(root)
    require(row["arm"] in ARMS, "prefix.registered_condition")
    directory = root / OUTPUT / "online/sessions" / row["label"]
    public = read_json(root / OUTPUT / f"preparation/public/{row['task_key']}.json")
    expected = [
        {"role": "system", "content": SYSTEMS_NEW[row["arm"]]},
        {"role": "user", "content": encode({"task": public}).decode()},
    ]
    ...
    if (directory / "manifest.json").exists():
        manifest(directory)
    turns = []
    for index, event_path in enumerate(sorted((directory / "turns").glob("*_event.json"))):
        require(event_path.name == f"{index:03d}_event.json", "prefix.contiguous_original_events")
        prefix = directory / f"turns/{index:03d}"
        ...
        if result is not None:
            require(
                ...
                and result["events"][index] == event,
                "prefix.original_session_event_and_source_join",
            )
        raw = file("_assistant.raw").read_bytes()
        ...
        content_identity(reservation, "prefix.original_reservation")
        require(
            ...
            "prefix.actual_public_execution_provenance",
        )
        final_content = (
            result["final"]
            if result is not None
        ...
        parsed = _event_target(event, raw, final_content)
        call = event["tool_call"]
        if call is not None:
            require(call["output"] == read_json(file("_tool.json")), "prefix.original_tool_output")
        binding = record(
            "diagnostic_original_interaction",
            population=row["arm"],
            task_key=row["task_key"],
            session_label=row["label"],
            response_index=index,
            ...
            raw_response_sha256=sha(raw),
            event_sha256=sha(event_path.read_bytes()),
            no_positive_qualification_implied=True,
        )
        turns.append(
            {
                "binding": binding,
    ...
    return {
        ...
        "result": result,
        "directory": directory,
        "diagnostic_observed_prefix_only": True,
        "absent_response_or_Final_fabricated": False,
    }
