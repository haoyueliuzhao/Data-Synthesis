...
VERSION = "public_quantity_interpretation.v1"
NUMBER = (
    r"[+-]?(?:(?:\d{1,3}(?:,\d{3})+|\d+)(?:\.\d*)?|\.\d+)"
    r"(?:[eE][+-]?\d+)?(?:\s*/\s*[+-]?\d+(?:\.\d+)?)?"
)
...
AMOUNT = re.compile(
    ...
    rf"\s*(?P<value>{NUMBER})(?:\s*(?P<unit>{UNIT_PHRASE}))?",
    re.IGNORECASE,
)
CURRENCIES = {
...
def amount_mentions(text, context):
    ...
    for match in AMOUNT.finditer(text):
        prefix, unit = match.group("prefix"), match.group("unit")
        if not prefix and not unit:
            continue
        combined = " ".join(part.strip() for part in (prefix, unit) if part)
        try:
        ...
        result.append(
            {
                ...
                "unit": combined,
                "interpretation": interpret_unit(combined, context),
