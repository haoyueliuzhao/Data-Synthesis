...
from .plan import ARMS, condition, encode, record, require, sha
...
def project_session(session, mapping, goal_scope):
    ...
    try:
        ...
        require(row["arm"] in ARMS, "projection.fixed_registered_population")
        ...
        signature = {
            "fixed_condition": condition(row["arm"])["id"],
            "task_version": public["task_id"],
            "source_document_id": public["id"],
            "goal_scope": goal_scope,
