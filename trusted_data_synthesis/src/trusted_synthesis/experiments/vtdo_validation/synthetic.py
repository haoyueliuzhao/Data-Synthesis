...
_METHODS: tuple[SyntheticMethod, ...] = (
    "random",
    "novelty_only",
    "contribution_only",
    "no_anchor",
...
def run_synthetic_experiment(
    ...
    *,
    ...
    for seed in config.seeds:
        ...
        contribution_oracle = _regularized_contribution_oracle(
            coverage,
            contribution,
            temperature=config.contribution_oracle_temperature,
        )
    ...
    summaries = tuple(_summarize_method(method, points, config.rounds) for method in _METHODS)
    sensitivity = tuple(
        _anchor_sensitivity(config, coverage_weight)
        for coverage_weight in config.anchor_sensitivity
    )
    ...
    report = SyntheticExperimentReport(
        ...
        reference_definitions={
            "regularized_contribution_oracle": (
                "p_C*(z|x) is proportional to r(z|x) * "
                "exp(true_contribution(z) / contribution_oracle_temperature), "
                "conditioned on Accepted states. This is an independent pure-contribution "
                "reference and is not the joint AEVTDR objective."
            ),
            "joint_utility": (
...
def _production_vtdo_update(
    ...
    *,
    ...
    manifest = estimate_centered_contributions(
        ...
        target_metric_id="oracle_expected_utility",
...
def _metric_point(
    ...
    coverage: Mapping[str, float],
    contribution: Mapping[str, float],
    contribution_oracle: Mapping[str, float],
    epsilon: float,
    *,
    ...
    return SyntheticMetricPoint(
        ...
        method=method,
        round_index=round_index,
        kl_to_regularized_contribution_oracle=_kl(current, contribution_oracle),
...
def _summarize_method(
    ...
    return SyntheticMethodSummary(
        method=method,
        run_count=len(final),
        final_kl_to_regularized_contribution_oracle=_aggregate(
            [item.kl_to_regularized_contribution_oracle for item in final]
        ),
...
def _anchor_sensitivity(
    ...
    tuned = config.model_copy(update={"coverage_kl_weight": coverage_weight})
    for seed in config.seeds:
        ...
        contribution_oracle = _regularized_contribution_oracle(
            coverage,
            contribution,
            temperature=tuned.contribution_oracle_temperature,
        )
        for round_index in range(1, tuned.rounds + 1):
            ...
            current, _ = _production_vtdo_update(
                ...
                tuned,
                round_index=round_index - 1,
                condition_suffix=f"anchor_{coverage_weight:g}",
            )
        metrics.append(
            _metric_point(
                seed,
    ...
    return AnchorSensitivityResult(
        coverage_kl_weight=coverage_weight,
        final_kl_to_regularized_contribution_oracle=_aggregate(
            [item.kl_to_regularized_contribution_oracle for item in metrics]
        ),
...
def _regularized_contribution_oracle(
    coverage: Mapping[str, float],
    contribution: Mapping[str, float],
    *,
    temperature: float,
) -> dict[str, float]:
    maximum = max(contribution.values())
    return _normalize(
        {
            key: coverage[key] * math.exp((contribution[key] - maximum) / temperature)
            for key in coverage
        }
    )
