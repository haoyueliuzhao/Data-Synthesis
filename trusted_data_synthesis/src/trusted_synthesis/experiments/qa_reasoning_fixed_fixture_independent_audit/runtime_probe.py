...
ordering for the ten fixed-Fixture Envelopes.  It does not import candidate
helpers.
"""
...
CANDIDATE_MODULE = "trusted_synthesis.experiments.qa_reasoning_fixed_fixture.preflight"
...
class Observer:
    ...
    def _special(relative: str | None) -> bool:
        return bool(
            relative
            and relative.startswith("runtime/")
            and relative.endswith(("_envelope.json", "_preaction_commit_receipt.json"))
        )

    @staticmethod
    def _runtime_directory(relative: str | None) -> bool:
        return bool(relative and relative.startswith("runtime/") and "." not in Path(relative).name)
    ...
    def profile(self, frame: FrameType, event: str, arg: Any) -> None:
        ...
        if (
            ...
            or frame.f_globals.get("__name__") not in {CANDIDATE_MODULE, "__main__"}
            or not frame.f_code.co_filename.endswith(
                "trusted_synthesis/experiments/qa_reasoning_fixed_fixture/preflight.py"
            )
        ):
            return
            ...
            _canonical(
                {
                    ...
                    "schema_version": "qa_reasoning_fixed_fixture_dynamic_probe.v1",
                }
            )
        )
