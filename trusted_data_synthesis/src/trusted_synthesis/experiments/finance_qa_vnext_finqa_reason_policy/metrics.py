...
from trusted_synthesis.experiments.finance_qa_vnext_finqa_final_recovery.metrics import (
    recovery_trace as original_recovery_trace,
)

from .runtime import action_schema, decode, parse
...
def proposed_core(value, request, task):
    ...
    if tree is not None:
        try:
            matches = [
                key
                for key, probe in alignment_probes(task).items()
...
def summarize_layers(rows):
    return {
        ...
        "raw_source_symbolic_aligned_core_proposals": sum(
            bool(r["proposed_core_alignment_probes"]) for r in rows
        ),
        "aligned_actual_executions": sum(bool(r["actual_alignment_probes"]) for r in rows),
        "aligned_accepted_observations": sum(
            bool(r["actual_alignment_probes"]) and r["observation_resolution"] == "accept"
            for r in rows
        ),
        "aligned_observations_with_later_action_consumer": sum(
            bool(r["actual_alignment_probes"]) and bool(r["later_direct_action_consumers"])
            for r in rows
        ),
        "aligned_consumed_by_valid_final": sum(
            bool(r["actual_alignment_probes"]) and r["consumed_by_valid_final"] for r in rows
        ),
...
def aggregate(rows):
    ...
    total["by_expression_condition_in_group"] = dict(
        Counter(r["expression_condition"] for r in rows)
    )
    return total
