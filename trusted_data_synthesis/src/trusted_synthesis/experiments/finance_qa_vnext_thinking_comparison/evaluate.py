...
from .plan import MODELS, OUTPUT, TASKS, history_guard, read_json, require
...
def closeout(root, reviews_path):
    ...
    for audit in assessments:
        ...
        if any(c is False for c in secondary_checks):
            score.update(task_answer_status="FAIL", reason="inconsistent_final_numeric_fields")
        ...
        row = record(
            ...
            **trace_checks(audit, review, score),
            ...
            reviewer_model_labels_masked=True,
    ...
    report = record(
        ...
        by_model={
            arm: {
                "registered": 12,
                "terminal_counts": dict(Counter(r["terminal"] for r in rows if r["arm"] == arm)),
                "answer_counts": dict(
                    Counter(r["answer"]["task_answer_status"] for r in rows if r["arm"] == arm)
                ),
                "formula_driven_verified": sum(
                    r["formula_driven_trace_verified"] for r in rows if r["arm"] == arm
                ),
                "by_task": {
                    key: dict(
                        Counter(
                            r["answer"]["task_answer_status"]
                            for r in rows
                            if r["arm"] == arm and r["task_key"] == key
                        )
                    )
                    for key in TASKS
                },
...
def historical_side_table(root):
    old = (
        root
        / "trusted_data_synthesis/artifacts/qa_vnext_autonomous_formula/six_tasks_2rep_20260909"
    )
    for name in ("preparation", "online", "assessment", "closeout"):
        manifest(old / name)
    before = read_json(old / "closeout/report.json")
    private = read_json(old / "preparation/private/evaluation_targets.json")
    rows = []
    for row in before["rows"]:
        review = row["semantic_review"]
        score = reviewed_answer_score(
            answer_score(
                review["published_value"], review["published_unit"], private[row["task_key"]]
            ),
            review,
        )
        rows.append(
            {
                ...
                "new_answer": score,
                "same_published_value_and_unit": True,
                "new_model_success": False,
            }
        )
    return record(
        "historical_evaluation_side_table",
        rows=rows,
        provider_calls=0,
        old_strict_counts=before["answer_counts"],
        revised_counts=dict(Counter(r["new_answer"]["task_answer_status"] for r in rows)),
        old_records_unchanged=True,
        known_data_measurement_revision_only=True,
    )
