...
def prepare(root):
    ...
    review = Path(
        "/home/zhuxinrui/.codex/attachments/d9c5a643-dda2-4507-b0ba-45ec99dfca78/pasted-text.txt"
    ).read_bytes()
    require(
        sha(review) == "94e08fbf14f90b8be27a628e30a592d5c707e125cc8d3a53ef9725ef64170e30",
        "stage.exact_external_review",
    )
    store.write("external_review.txt", review)
    from .evaluate import historical_side_table

    store.json("historical_evaluation_side_table.json", historical_side_table(root))
    bundle_members = []
...
def collect(root):
    ...
    result = record(
        ...
        no_resampling=True,
        task_weights_apply_separately_per_model=True,
        no_student=True,
    )
