...
from trusted_synthesis.experiments.vtdo_experiment.phase1_stopping_shape_redesign_protocol import (  # noqa: E501
    ALL_SHAPES,
...
STOPPING_SHAPE_REDESIGN_CONTRACT_VERSION = "finance_stopping_shape_redesign_contract.v1"
STOPPING_SHAPE_REDESIGN_RESULT_VERSION = "finance_stopping_shape_redesign_result.v1"
STOPPING_SHAPE_REDESIGN_POLICY_VERSION = "finance_stopping_difficulty_policy.v2"
STOPPING_SHAPE_REDESIGN_REPORT_VERSION = "finance_stopping_shape_redesign_report.v1"
STOPPING_SHAPE_REDESIGN_MANIFEST_VERSION = "finance_stopping_shape_redesign_manifest.v1"
...
class FinanceStoppingShapeRedesignContract(FrozenModel):
    ...
    next_permitted_stage: Literal["flash_stopping_shape_redesign_development"] = (
        "flash_stopping_shape_redesign_development"
    )
    schema_version: str = STOPPING_SHAPE_REDESIGN_CONTRACT_VERSION

    @model_validator(mode="after")
    def validate_contract(self) -> FinanceStoppingShapeRedesignContract:
        if self.stage != RuntimeResolutionStage.RESIDUAL_DEVELOPMENT:
            raise ValueError("Stopping Shape redesign must remain Development-only")
        ...
        if set(shape_stratum_indices) != expected_cells or any(
            ...
            raise ValueError("Stopping Shape redesign task pairing is incomplete")
        expected_tokens = {
        ...
        if self.implementation_manifest_hash != canonical_hash(
            ...
            prefix="finance_stopping_shape_redesign_implementation:",
        ):
            raise ValueError("Stopping shape execution implementation identity is invalid")
        if self.contract_id != stopping_shape_redesign_contract_id(self):
            raise ValueError("Stopping shape contract identity is invalid")
        return self


class StoppingShapeRedesignTaskResponse(FrozenModel):
    ...
    @model_validator(mode="after")
    def validate_response(self) -> StoppingShapeRedesignTaskResponse:
...
class StoppingShapeRedesignResult(FrozenModel):
    ...
    task_responses: tuple[StoppingShapeRedesignTaskResponse, ...] = Field(
    ...
    boundary_task_count: int = Field(ge=0, le=TASKS_PER_SHAPE)
    nonzero_information_task_count: int = Field(ge=0, le=TASKS_PER_SHAPE)
    total_fisher_information: float = Field(ge=0)
    effective_task_count: float = Field(ge=0, le=TASKS_PER_SHAPE)
    ...
    schema_version: str = STOPPING_SHAPE_REDESIGN_RESULT_VERSION

    @model_validator(mode="after")
    def validate_result(self) -> StoppingShapeRedesignResult:
        ...
        if observed != expected:
            raise ValueError("Stopping Shape redesign response pairing is incomplete")
...
class FrozenStoppingDifficultyPolicyV2(FrozenModel):
    ...
    positive_controls_frozen: tuple[str, ...] = tuple(sorted(FROZEN_POSITIVE_CONTROLS))
    ...
    schema_version: str = STOPPING_SHAPE_REDESIGN_POLICY_VERSION

    @model_validator(mode="after")
    def validate_policy(self) -> FrozenStoppingDifficultyPolicyV2:
        if set(self.shape_task_quotas) != ALL_SHAPES:
            raise ValueError("Stopping difficulty policy v2 lacks a Shape")
        if set(self.shape_task_quotas.values()) != {TASKS_PER_SHAPE}:
            raise ValueError("Stopping difficulty policy changed Shape quota")
        if self.policy_id != stopping_difficulty_policy_v2_id(self):
            raise ValueError("Stopping difficulty policy identity is invalid")
        return self


class FinanceStoppingShapeRedesignReport(FrozenModel):
    ...
    shape_results: tuple[StoppingShapeRedesignResult, ...] = Field(
        min_length=len(ALL_SHAPES), max_length=len(ALL_SHAPES)
    )
    positive_control_regression_count: int = Field(ge=0, le=3)
    redesigned_shape_admission_count: int = Field(ge=0, le=3)
    all_shapes_admitted: bool
    difficulty_policy: FrozenStoppingDifficultyPolicyV2 | None
    ...
    next_permitted_stage: Literal[
        "fresh_three_population_stable_support_preparation",
        "runtime_measurement_repair_only",
        "stopping_shape_redesign_only",
    ]
    schema_version: str = STOPPING_SHAPE_REDESIGN_REPORT_VERSION

    @model_validator(mode="after")
    def validate_report(self) -> FinanceStoppingShapeRedesignReport:
        by_shape = {item.shape_id: item for item in self.shape_results}
        if set(by_shape) != ALL_SHAPES:
            raise ValueError("Stopping Shape redesign report coverage is incomplete")
        regressions = sum(not by_shape[shape_id].admitted for shape_id in FROZEN_POSITIVE_CONTROLS)
        redesigns = sum(by_shape[shape_id].admitted for shape_id in REDESIGNED_FAILURE_SHAPES)
        if self.positive_control_regression_count != regressions:
            raise ValueError("Stopping Shape positive-control accounting is invalid")
        if self.redesigned_shape_admission_count != redesigns:
            raise ValueError("Stopping Shape redesign accounting is invalid")
        ...
        if self.fresh_three_population_preparation_authorized != admitted:
            raise ValueError("Stopping Shape redesign authorization is inconsistent")
        expected_stage = (
            "runtime_measurement_repair_only"
        ...
        if self.report_id != stopping_shape_redesign_report_id(self):
            raise ValueError("Stopping Shape report identity is invalid")
        return self


def stopping_shape_redesign_contract_id(value: FinanceStoppingShapeRedesignContract) -> str:
    return canonical_hash(
        value.model_dump(mode="json", exclude={"contract_id"}),
        prefix="finance_stopping_shape_redesign_contract:",
    )


def stopping_difficulty_policy_v2_id(value: FrozenStoppingDifficultyPolicyV2) -> str:
    return canonical_hash(
        value.model_dump(mode="json", exclude={"policy_id"}),
        prefix="finance_stopping_difficulty_policy_v2:",
    )


def stopping_shape_redesign_report_id(value: FinanceStoppingShapeRedesignReport) -> str:
    return canonical_hash(
        value.model_dump(mode="json", exclude={"report_id"}),
        prefix="finance_stopping_shape_redesign_report:",
    )


def prepare_stopping_shape_redesign_contract(
    *,
    protocol_path: Path,
    ...
    population = FinanceStoppingShapeRedesignPopulation.model_validate_json(
    ...
    task_instances = {
        task_id: canonical_hash(
            ...
            prefix="finance_stopping_shape_redesign_task_instance:",
        )
    ...
    rollout_tokens = {
        f"{binding.binding_id}|{replicate}": canonical_hash(
            ...
            prefix="finance_stopping_shape_redesign_rollout:",
        )
    ...
    provisional = FinanceStoppingShapeRedesignContract.model_construct(
        contract_id="pending", **values
    )
    contract = FinanceStoppingShapeRedesignContract(
        contract_id=stopping_shape_redesign_contract_id(provisional), **values
    )
...
def run_stopping_shape_redesign(
    *,
    contract_path: Path,
    output_dir: Path,
    workers: int,
) -> FinanceStoppingShapeRedesignReport:
    contract = FinanceStoppingShapeRedesignContract.model_validate_json(
        contract_path.read_text(encoding="utf-8")
    )
    _verify_contract_inputs(contract)
    prefix = "stopping_shape_redesign_development"
    ...
    report = make_stopping_shape_redesign_report(
        contract,
        records,
        outcomes,
        terminals,
    ...
    report_path = output_dir / "finance_stopping_shape_redesign_report.json"
    _write_json(report_path, report.model_dump(mode="json"))
    (output_dir / "finance_stopping_shape_redesign_report.md").write_text(
        _render_report(report), encoding="utf-8"
    )
    manifest = {
        "schema_version": STOPPING_SHAPE_REDESIGN_MANIFEST_VERSION,
    ...
    _write_json(output_dir / "finance_stopping_shape_redesign_manifest.json", manifest)
    return report


def make_stopping_shape_redesign_report(
    contract: FinanceStoppingShapeRedesignContract,
    ...
    *,
    ...
    positive_control_regressions = sum(
        not by_shape[shape_id].admitted for shape_id in FROZEN_POSITIVE_CONTROLS
    )
    redesigned_shape_admissions = sum(
        by_shape[shape_id].admitted for shape_id in REDESIGNED_FAILURE_SHAPES
    )
    ...
    values = {
        ...
        "primary_valid_success_rate": _rate(item.primary_valid_success for item in behaviors),
        ...
        "positive_control_regression_count": positive_control_regressions,
        "redesigned_shape_admission_count": redesigned_shape_admissions,
        ...
        "fresh_three_population_preparation_authorized": admitted,
    ...
    provisional = FinanceStoppingShapeRedesignReport.model_construct(report_id="pending", **values)
    return FinanceStoppingShapeRedesignReport(
        report_id=stopping_shape_redesign_report_id(provisional), **values
    )


def _make_redesign_shape_result(
    contract: FinanceStoppingShapeRedesignContract,
    shape_id: str,
    behaviors: Sequence[SubmechanismBehaviorObservation],
    ...
    responses: list[StoppingShapeRedesignTaskResponse] = []
    ...
    interval = _redesign_shape_information_bootstrap(
        frozen,
        contract.thresholds,
        shape_id=shape_id,
    )
...
def _redesign_shape_information_bootstrap(
    responses: Sequence[StoppingShapeRedesignTaskResponse],
    thresholds: StoppingShapeRedesignThresholds,
    *,
    shape_id: str,
) -> tuple[float, float]:
    seed = int(
        canonical_hash(
            ...
            prefix="finance_stopping_shape_redesign_bootstrap_seed:",
        ).rsplit(":", 1)[-1][:16],
        16,
    )
...
def _make_difficulty_policy(
    ...
    provisional = FrozenStoppingDifficultyPolicyV2.model_construct(policy_id="pending", **values)
    return FrozenStoppingDifficultyPolicyV2(
        policy_id=stopping_difficulty_policy_v2_id(provisional), **values
    )
...
def _verify_contract_inputs(contract: FinanceStoppingShapeRedesignContract) -> None:
    for reference in (
...
def _implementation_manifest() -> dict[str, str]:
    ...
    paths = (
        ...
        "src/trusted_synthesis/experiments/vtdo_experiment/phase1_stopping_shape_redesign.py",
    )
...
def _render_report(report: FinanceStoppingShapeRedesignReport) -> str:
    lines = [
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run v25.37 Stopping Shape Redesign Development")
...
def main() -> None:
    ...
    if args.command == "prepare":
        contract = prepare_stopping_shape_redesign_contract(
        ...
        report = run_stopping_shape_redesign(
