...
from trusted_synthesis.experiments.vtdo_experiment.phase1_v26_thinking_16k_completion_calibration_contracts import (  # noqa: E501
    EXECUTION_RUN_ID,
    EXPECTED_PROVIDER_USAGE_SEMANTICS_ID,
    EXPECTED_V26_103_CONTRACT_ID,
    EXPECTED_V26_103_CROSS_BINDING_ID,
    EXPECTED_V26_103_MANIFEST_ID,
    EXPECTED_V26_103_REPORT_ID,
    ...
    Exact16KProviderBudgetAudit,
    Exact16KProviderBudgetCertificate,
    Exact16KRawExecution,
    Exact16KRawLineageAudit,
    Exact16KRawProviderCall,
    Exact16KRequestAttempt,
    Exact16KRunnerPreflightReport,
    Exact16KRunnerSourceReplayAudit,
    Prepared16KRequest,
    canonical_bytes,
    exact_16k_execution_report_id,
    exact_16k_provider_budget_audit_id,
    exact_16k_provider_call_id,
    exact_16k_raw_execution_id,
    exact_16k_raw_lineage_audit_id,
    exact_16k_raw_provider_call_id,
    exact_16k_request_attempt_id,
    load_canonical_json,
    prepared_16k_request_id,
    sha256,
    sha256_text,
...
from trusted_synthesis.runtime.agent.prospective_thinking_16k_client import (
    EXACT_16K_MAX_TOKENS,
    EXACT_16K_MODEL_ID,
    EXACT_16K_PROFILE_SHA256,
    Exact16KProspectiveThinkingJsonClient,
    certify_exact_16k_request_pre_call,
    require_exact_16k_model_config,
)
...
class _StaticInputs(BaseModel):
    ...
    predecessor_report: Exact16KRematerializationReport
    predecessor_contract: Exact16KCompletionContract
    predecessor_manifest: Exact16KManifest
    profile_binding: Exact16KProfileBinding
    cross_binding: Exact16KCrossArtifactBindingAudit
    provider_usage_semantics: ProviderUsageSemanticsContract
    task_packages: tuple[Exact16KTaskPackage, ...]
    path_audits: tuple[Exact16KPathAudit, ...]
    predecessor_exact_8k_paths: tuple[Exact8KPathAudit, ...]
...
class PreparedInputs(BaseModel):
    ...
    preflight_report: Exact16KRunnerPreflightReport
    execution_contract: Exact16KExecutionContract
    interpretation_contract: Exact16KOutcomeInterpretationContract
...
class RuntimeBinding(BaseModel):
    ...
    task_package: Exact16KTaskPackage
...
def load_static_inputs(package_root: Path) -> _StaticInputs:
    ...
    report = Exact16KRematerializationReport.model_validate(
        load_canonical_json(predecessor_dir / "report.json")
    )
    contract = Exact16KCompletionContract.model_validate(
        load_canonical_json(predecessor_dir / "exact_16k_completion_contract.json")
    )
    manifest = Exact16KManifest.model_validate(
        load_canonical_json(predecessor_dir / "exact_16k_job_manifest.json")
    )
    profile_binding = Exact16KProfileBinding.model_validate(
        load_canonical_json(predecessor_dir / "exact_16k_profile_binding.json")
    )
    cross = Exact16KCrossArtifactBindingAudit.model_validate(
        load_canonical_json(predecessor_dir / "cross_artifact_binding_audit.json")
    )
    usage = ProviderUsageSemanticsContract.model_validate(
        load_canonical_json(predecessor_dir / "provider_usage_semantics_contract.json")
    )
    task_packages = cast(
        tuple[Exact16KTaskPackage, ...],
        _load_models(predecessor_dir / "exact_16k_task_packages.json", Exact16KTaskPackage),
    )
    paths = cast(
        tuple[Exact16KPathAudit, ...],
        _load_models(predecessor_dir / "exact_16k_path_audits.json", Exact16KPathAudit),
    )
    exact_8k_paths = cast(
    ...
    profile_path = implementation_root / profile_binding.profile_relative_path
    if sha256(profile_path) != EXACT_16K_PROFILE_SHA256:
        raise ValueError("v26.104 persisted exact 16K profile bytes changed")
    profile_payload = json.loads(profile_path.read_text(encoding="utf-8"))
    model_config = require_exact_16k_model_config(
    ...
    if (
        ...
        or contract.contract_id != EXPECTED_V26_103_CONTRACT_ID
        or manifest.manifest_id != EXPECTED_V26_103_MANIFEST_ID
        or cross.audit_id != EXPECTED_V26_103_CROSS_BINDING_ID
        or usage.contract_id != EXPECTED_PROVIDER_USAGE_SEMANTICS_ID
        ...
        or model_config.public_manifest_hash != EXPECTED_16K_MODEL_CONFIG_ID
        or bind_prospective_thinking(model_config).binding_id != EXPECTED_16K_THINKING_BINDING_ID
        or manifest.higher_bound_job_count != 0
        ...
        raise ValueError("v26.104 static exact 16K predecessor binding changed")
    static = _StaticInputs(
        ...
        cross_binding=cross,
        provider_usage_semantics=usage,
        task_packages=task_packages,
        path_audits=paths,
        predecessor_exact_8k_paths=exact_8k_paths,
...
def runtime_binding(inputs: _StaticInputs, job: Exact16KJob) -> RuntimeBinding:
    ...
    exact_8k_paths = {item.audit_id: item for item in inputs.predecessor_exact_8k_paths}
    ...
    exact_8k_path = exact_8k_paths[exact_path.predecessor_path_audit_id]
    bound_path = bound_paths[exact_8k_path.predecessor_path_audit_id]
    ...
    if (
        ...
        raise ValueError(f"v26.104 runtime binding changed for Job {job.job_id}")
...
def raw_provider_path(output_dir: Path, job: Exact16KJob, call_index: int) -> Path:
...
def raw_execution_path(output_dir: Path, job: Exact16KJob) -> Path:
    job_hash = hashlib.sha256(job.job_id.encode("utf-8")).hexdigest()[:20]
...
class JournaledExact16KClient:
    ...
    def __init__(
        ...
        *,
        ...
        config = require_exact_16k_model_config(delegate.config)
        if (
            ...
            or provider_usage_semantics.contract_id
            != execution_contract.provider_usage_semantics_contract_id
            or provider_usage_semantics.exact_request_completion_bound_tokens != 16384
            or provider_usage_semantics.maximum_accounting_admissible_completion_tokens != 16385
            or provider_usage_semantics.rollout_upper_bound_tokens != 240000
            or completion_protocol.protocol_id != execution_contract.completion_bound_protocol_id
            or job.candidate_id != completion_protocol.fallback_candidate_id
        ):
            raise ValueError("v26.104 journal client differs from the frozen exact 16K route")
        self._delegate = delegate
        self._execution_contract = execution_contract
        self._job = job
        self._provider_usage_semantics = provider_usage_semantics
        self._completion_protocol = completion_protocol
        self._output_dir = output_dir
        self._certificates: list[Exact16KProviderBudgetCertificate] = []
    ...
    def _reserves(
        ...
        *,
        ...
        rescue_accounting = (
            self._execution_contract.completion_rescue_accounting_reserve_tokens if rescue else 0
        )
        final = (
        ...
        final_accounting = (
            self._execution_contract.final_answer_accounting_reserve_tokens if final else 0
        )
        return rescue, rescue_accounting, final, final_accounting
    ...
    def _provider_certificate(
        ...
        *,
        ...
        contract = self._provider_usage_semantics
        prompt_bytes = len(prompt.encode("utf-8"))
        prompt_upper = prompt_bytes + contract.chat_envelope_tokens
        request_upper = prompt_upper + contract.maximum_accounting_admissible_completion_tokens
        rescue, rescue_accounting, final, final_accounting = self._reserves(
            phase=phase,
            request_kind=request_kind,
            rescue_available_before=rescue_available_before,
        )
        projected_without_reserve = self._cumulative_tokens + request_upper
        rescue_reserve = rescue + rescue_accounting
        final_reserve = final + final_accounting
        projected = projected_without_reserve + rescue_reserve + final_reserve
        denial: (
            Literal[
                "oversized_prompt",
        ...
        if prompt_bytes > contract.prompt_upper_bound_bytes:
            denial = "oversized_prompt"
        elif projected_without_reserve > contract.rollout_upper_bound_tokens:
            denial = "request_bound_exceeds_remaining_budget"
        elif projected > contract.rollout_upper_bound_tokens:
        ...
        values = {
            ...
            "completion_token_upper_bound": (
                contract.maximum_accounting_admissible_completion_tokens
            ),
            "exact_request_completion_upper_bound_tokens": (
                contract.exact_request_completion_bound_tokens
            ),
            "provider_reported_accounting_margin_tokens": (
                contract.provider_reported_accounting_margin_tokens
            ),
            "request_token_upper_bound": request_upper,
            "cumulative_provider_tokens_before": self._cumulative_tokens,
            "contract_repair_reserve_tokens": rescue_reserve,
            "final_answer_reserve_tokens": final_reserve,
            "exact_rescue_reserve_tokens": rescue,
            "rescue_accounting_reserve_tokens": rescue_accounting,
            "exact_final_answer_reserve_tokens": final,
            "final_answer_accounting_reserve_tokens": final_accounting,
            "required_reserve_tokens": rescue_reserve + final_reserve,
            "projected_upper_total": projected,
            "maximum_total_tokens": contract.rollout_upper_bound_tokens,
            "decision": "denied_no_call" if denial is not None else "allowed",
            "denial_reason": denial,
            "provider_call_permitted": denial is None,
        }
        provisional = Exact16KProviderBudgetCertificate.model_construct(
            certificate_id="pending",
            **values,
        )
        return Exact16KProviderBudgetCertificate(
    ...
    def prepare_request(
        ...
        *,
        ...
        if self._contract_failure_ids or self._no_call_terminal is not None:
            raise ValueError("v26.104 cannot prepare after a terminal budget state")
        request_binding = certify_exact_16k_request_pre_call(
            config=self.config,
            prompt=prompt,
        ...
        if provider_certificate.provider_call_permitted:
            ...
            if phase == "primary":
                ...
                if failure_type is None:
                    raise ValueError("v26.104 Rescue preparation lacks a failure type")
                ...
                if rendered != prompt:
                    raise ValueError("v26.104 prepared Rescue differs from the bounded renderer")
        values = {
        ...
        provisional = Prepared16KRequest.model_construct(preparation_id="pending", **values)
        return Prepared16KRequest(
            preparation_id=prepared_16k_request_id(provisional),
            **values,
        )
    ...
    def _persist_provider_call(
        ...
        *,
        ...
        prepared: Prepared16KRequest,
        payload: dict[str, Any] | None,
        telemetry: ModelCallTelemetry,
        failure_artifact: ProspectiveThinkingFailureArtifact | None,
    ) -> None:
        if prepared.dynamic_certificate is None:
            raise ValueError("v26.104 cannot persist an uncertified Provider call")
        call_index = len(self._telemetry)
        provider_call_id = exact_16k_provider_call_id(
            self._job.job_id,
            call_index,
            telemetry,
        ...
        provisional = Exact16KRawProviderCall.model_construct(artifact_id="pending", **values)
        artifact = Exact16KRawProviderCall(
            artifact_id=exact_16k_raw_provider_call_id(provisional),
            **values,
        )
    ...
    def _record_usage(
        ...
        checks: dict[str, bool] = {
            ...
            "requested_model_match": telemetry.model_requested == EXACT_16K_MODEL_ID,
            "selected_model_match": telemetry.model_selected == EXACT_16K_MODEL_ID,
            "fallback_absent": not telemetry.fallback_used,
            "request_binding_exact_16k": (
                prepared.request_binding_certificate.request_max_tokens == 16384
            ),
        }
        counted = 0
            ...
            checks.update(
                {
                    ...
                    "completion_accounting_margin_respected": (
                        completion_tokens is not None and completion_tokens <= 16385
                    ),
                    "two_or_more_excess_tokens_absent": (
                        completion_tokens is not None and completion_tokens < 16386
                    ),
                    ...
                    "rollout_ceiling_respected": (
                        total_tokens is not None
                        and self._cumulative_tokens + total_tokens <= 240000
                    ),
                }
            )
        ...
        values = {
            "contract_id": self._provider_usage_semantics.contract_id,
    ...
    def invoke(
        ...
        *,
        ...
        if prepared.preparation_id in self._used_preparations:
            raise LLMClientError("v26.104 prepared request cannot be invoked twice")
        ...
        if not prepared.provider_invocation_authorized or prepared.dynamic_certificate is None:
            raise LLMClientError("v26.104 Provider invocation lacks all pre-call certificates")
        try:
            ...
            if self._contract_failure_ids:
                raise LLMClientError(
                    "v26.104 Provider budget or binding Contract failed",
                    exc.telemetry,
                    failure_artifact=artifact,
                ) from exc
            raise
    ...
    def audit(self) -> Exact16KProviderBudgetAudit:
        failures = tuple(sorted(self._contract_failure_ids))
        one_token_margin_count = sum(
            item.http_success and item.completion_tokens == 16385 for item in self._usage_records
        )
        two_or_more_excess_count = sum(
            item.http_success
            and item.completion_tokens is not None
            and item.completion_tokens >= 16386
            for item in self._usage_records
        )
        values = {
            "contract_id": self._provider_usage_semantics.contract_id,
            ...
            "maximum_total_tokens": self._provider_usage_semantics.rollout_upper_bound_tokens,
            "one_token_accounting_margin_count": one_token_margin_count,
            "two_or_more_excess_token_count": two_or_more_excess_count,
            "actual_usage_charged_without_clipping": True,
        ...
        provisional = Exact16KProviderBudgetAudit.model_construct(audit_id="pending", **values)
        return Exact16KProviderBudgetAudit(
            audit_id=exact_16k_provider_budget_audit_id(provisional),
            **values,
        )
...
def _attempt_disposition_from_error(
    exc: LLMClientError,
    *,
    ledger: JournaledExact16KClient,
...
def request_attempt(
    *,
    ...
    prepared: Prepared16KRequest | None = None
    ...
    provisional = Exact16KRequestAttempt.model_construct(attempt_id="pending", **values)
    return Exact16KRequestAttempt(
        attempt_id=exact_16k_request_attempt_id(provisional),
        **values,
    )
...
def _logical_request(
    *,
    ...
    request_kind: CompletionRequestKind,
    source_prompt: str,
    primary: Exact16KRequestAttempt,
    rescue: Exact16KRequestAttempt | None,
) -> ThinkingRepairLogicalRequest:
...
def _terminal_disposition(attempt: Exact16KRequestAttempt) -> str:
    return {
...
def _load_raw_execution(path: Path) -> Exact16KRawExecution:
    return Exact16KRawExecution.model_validate(load_canonical_json(path))


def execute_exact_16k_job_raw(
    *,
    job: Exact16KJob,
    execution_contract: Exact16KExecutionContract,
    provider_usage_semantics: ProviderUsageSemanticsContract,
    ...
    if raw_path.exists():
        ...
        if raw.execution_contract_id != execution_contract.contract_id or raw.job != job:
            raise ValueError("v26.105 raw-only recovery crosses frozen identities")
        return raw
    provider_dir = raw_provider_path(output_dir, job, 0).parent
    if provider_dir.exists() and any(provider_dir.glob("call_*.json")):
        raise ValueError(
            "orphan v26.105 Provider Artifacts exist without a Raw Execution; "
            "automatic model retry is forbidden"
        )
    if client is None:
        raise ValueError("pending v26.105 Job has no model client")
    if (
        ...
        raise ValueError("v26.105 execution inputs differ from the frozen Job")

    ledger = JournaledExact16KClient(
        client,
        execution_contract=execution_contract,
        job=job,
        provider_usage_semantics=provider_usage_semantics,
    ...
    attempts: list[Exact16KRequestAttempt] = []
    ...
    for logical_index in range(maximum_logical_requests):
        ...
        rescue: Exact16KRequestAttempt | None = None
        active = primary
        if primary.disposition == "completion_failure" and not rescue_used:
            if primary.failure_artifact is None:
                raise ValueError("v26.105 Completion failure lacks a Rescue type")
    ...
    if budget_audit.status != "passed":
        ...
        terminal_failure_type = "provider_usage_semantics_contract_failure"
    ...
    provisional_raw = Exact16KRawExecution.model_construct(artifact_id="pending", **raw_values)
    raw = Exact16KRawExecution(
        artifact_id=exact_16k_raw_execution_id(provisional_raw),
        **raw_values,
    )
...
def score_exact_16k_raw_execution(
    *,
    raw: Exact16KRawExecution,
    prepared: Any,
    binding: RuntimeBinding,
...
def _next_transition(
    ...
    if not any(item.program_closed for item in results):
        return interpretation.completion_usable_low_closure_transition
...
def raw_lineage_audit(
    *,
    prepared: Any,
    results: Sequence[ThinkingRepairJobResult],
    raw_by_job: Mapping[str, Exact16KRawExecution],
    output_dir: Path,
) -> Exact16KRawLineageAudit:
    ...
    for result in results:
        ...
        if replayed.model_dump(mode="json") != raw.model_dump(mode="json"):
            raise ValueError(f"v26.105 Raw Execution replay changed: {result.job_id}")
        ...
        for descriptor in raw.provider_call_artifacts:
            ...
            reparsed = Exact16KRawProviderCall.model_validate(payload)
            if (
                sha256(provider_path) != descriptor.sha256
                or reparsed.provider_call_id not in raw.provider_call_ids
            ):
                raise ValueError("v26.105 Provider Artifact binding changed")
            private_hits += int(
                any(
    ...
    provisional = Exact16KRawLineageAudit.model_construct(audit_id="pending", **values)
    return Exact16KRawLineageAudit(
        audit_id=exact_16k_raw_lineage_audit_id(provisional),
        **values,
    )
...
def make_execution_report(
    *,
    prepared: Any,
    results: Sequence[ThinkingRepairJobResult],
    lineage: Exact16KRawLineageAudit,
    raw_by_job: Mapping[str, Exact16KRawExecution],
) -> Exact16KExecutionReport:
    ...
    exact_request_pass = all(
        ...
        and attempt.request_binding_certificate.request_max_tokens == 16384
    ...
    values = {
        ...
        "one_token_accounting_margin_call_count": sum(
            raw.provider_budget_audit.one_token_accounting_margin_count
            for raw in raw_by_job.values()
        ),
        "two_or_more_excess_token_call_count": sum(
            raw.provider_budget_audit.two_or_more_excess_token_count for raw in raw_by_job.values()
        ),
        "actual_provider_usage_charged_without_clipping": all(
        ...
        "exact_16k_request_binding_passed": exact_request_pass,
    ...
    provisional = Exact16KExecutionReport.model_construct(report_id="pending", **values)
    return Exact16KExecutionReport(
        report_id=exact_16k_execution_report_id(provisional),
        **values,
    )


def prepare_exact_16k_execution(
    *,
    runner_preflight_dir: Path,
    output_dir: Path,
    package_root: Path,
) -> PreparedInputs:
    report = Exact16KRunnerPreflightReport.model_validate(
        load_canonical_json(runner_preflight_dir / "report.json")
    )
    replay = Exact16KRunnerSourceReplayAudit.model_validate(
    ...
    provider_usage_semantics = ProviderUsageSemanticsContract.model_validate(
        load_canonical_json(runner_preflight_dir / "provider_usage_semantics_contract.json")
    )
    ...
    if (
        ...
        or report.provider_usage_semantics_contract_id != provider_usage_semantics.contract_id
        or report.execution_contract_id != execution_contract.contract_id
        or not report.exact_16k_execution_authorized
        or execution_contract.source_replay_audit_id != replay.audit_id
        or execution_contract.provider_usage_semantics_contract_id
        != provider_usage_semantics.contract_id
        or execution_contract.runner_implementation_source_files
        != report.implementation_source_files
    ):
        raise ValueError("v26.104 preflight binding changed before execution")
    ...
    if (
        ...
        or provider_usage_semantics.exact_request_completion_bound_tokens != EXACT_16K_MAX_TOKENS
        or provider_usage_semantics.maximum_accounting_admissible_completion_tokens
        != EXACT_16K_MAX_TOKENS + 1
        or provider_usage_semantics.rollout_upper_bound_tokens != 240000
        or provider_usage_semantics.contract_id != static.provider_usage_semantics.contract_id
    ):
        raise ValueError("v26.105 execution denominator differs from v26.103/v26.104")
    ...
    write_json_atomic(
        output_dir / "frozen_exact_16k_completion_contract.json",
        static.predecessor_contract.model_dump(mode="json"),
    )
    write_json_atomic(
        output_dir / "frozen_exact_16k_job_manifest.json",
...
def _run_one_job(
    *,
    ...
    client: Any | None,
    output_dir: Path,
) -> tuple[ThinkingRepairJobResult, Exact16KRawExecution]:
    binding = runtime_binding(prepared.static, job)
    raw = execute_exact_16k_job_raw(
        job=job,
        execution_contract=prepared.execution_contract,
        provider_usage_semantics=prepared.provider_usage_semantics,
        completion_protocol=prepared.static.completion_bound_protocol,
        binding=binding,
        client=client,
        output_dir=output_dir,
    )
    result = score_exact_16k_raw_execution(
        raw=raw,
        prepared=prepared,
        binding=binding,
        output_dir=output_dir,
...
def run_exact_16k_completion_calibration(
    *,
    runner_preflight_dir: Path,
    output_dir: Path,
    package_root: Path,
    workers: int,
    client_factory: Callable[[AgentModelConfig], Any] = Exact16KProspectiveThinkingJsonClient,
) -> Exact16KExecutionReport:
    ...
    checkpoint_path = output_dir / "exact_16k_job_results.checkpoint.jsonl"
    ...
    if pending and report_path.exists():
        raise ValueError("v26.105 completed report exists while Jobs remain pending")
    if not pending and report_path.exists():
        report = Exact16KExecutionReport.model_validate(load_canonical_json(report_path))
        if report.execution_contract_id != prepared.execution_contract.contract_id:
            raise ValueError("v26.105 completed report crosses execution Contracts")
        return report
    ...
    if len(results) != 32:
        raise ValueError("v26.105 execution denominator is incomplete")
    for job in jobs:
    ...
    write_json_atomic(
        output_dir / "exact_16k_job_results.json",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run the exact v26.105 Thinking 16K Completion calibration"
    )
    ...
    if args.prepare_only:
        ...
        return
    report = run_exact_16k_completion_calibration(
        runner_preflight_dir=args.runner_preflight_dir,
        output_dir=args.output_dir,
        package_root=args.package_root,
