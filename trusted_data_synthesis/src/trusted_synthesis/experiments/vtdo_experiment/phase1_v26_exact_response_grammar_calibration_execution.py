...
RUN_ID: Final = "finance_v26_114_exact_response_grammar_calibration_execution_v1_20260823"
...
IMPLEMENTATION_PATH: Final = (
    ...
    "phase1_v26_exact_response_grammar_calibration_execution.py"
)
POSTRUN_AUDIT_TRANSITION: Final = "exact_response_grammar_calibration_postrun_audit_only"
...
class ExecutionSourceReplayAudit(FrozenModel):
    ...
    predecessor_transitive_file_count: Literal[2041] = 2041
    predecessor_output_file_count: Literal[7] = 7
    implementation_file_count: Literal[1] = 1
    replayed_file_count: Literal[2049] = 2049
    replay_pass_count: Literal[2049] = 2049
    entries: tuple[ExecutionSourceReplayEntry, ...] = Field(min_length=2049, max_length=2049)
    ...
    schema_version: Literal["finance_v26_exact_grammar_execution_source_replay.v1"] = (
        "finance_v26_exact_grammar_execution_source_replay.v1"
    )
    ...
    def validate_audit(self) -> ExecutionSourceReplayAudit:
        if tuple(item.relative_path for item in self.entries) != tuple(
            ...
            raise ValueError("v26.114 source replay entries are not canonical")
        if len({item.relative_path for item in self.entries}) != 2049:
            raise ValueError("v26.114 source replay contains duplicate paths")
        if not all(item.passed for item in self.entries):
            raise ValueError("v26.114 source replay contains a failed file")
        if self.audit_id != execution_source_replay_audit_id(self):
            raise ValueError("v26.114 source replay identity changed")
        return self


class ExactGrammarJobResult(FrozenModel):
    ...
    rescue_success: bool
    rescue_recovered_then_later_rescuable_failure: bool
    completion_failure_counts: dict[str, int]
    model_failure_family_counts: dict[str, int]
    model_failure_subtype_counts: dict[str, int]
    semantic_proposal_response_payload_count: int = Field(ge=0, le=10)
    exact_abi_accepted_count: int = Field(ge=0, le=10)
    exact_abi_failure_count: int = Field(ge=0, le=2)
    wrong_state_binding_count: int = Field(ge=0, le=2)
    semantic_proposal_commit_count: int = Field(ge=0, le=9)
    abi_failure_counts_by_logical_request_index: dict[str, int]
    ...
    rollout_headroom_tokens: int = Field(ge=0, le=260000)
    ...
    schema_version: Literal["finance_v26_exact_grammar_job_result.v1"] = (
        "finance_v26_exact_grammar_job_result.v1"
    )

    @model_validator(mode="after")
    def validate_result(self) -> ExactGrammarJobResult:
        if self.reasoning_tokens_total > self.completion_tokens_total:
            raise ValueError("v26.114 reasoning Usage exceeds Completion Usage")
        if self.requested_path_adhered != (self.actual_route == self.requested_path_strategy_id):
            raise ValueError("v26.114 path adherence accounting changed")
        if self.independent_validity != (self.terminal_category == "model_valid_trajectory"):
            raise ValueError("v26.114 valid terminal accounting changed")
        if self.rescue_success and not self.rescue_attempt_count:
            raise ValueError("v26.114 Rescue success lacks a Rescue attempt")
        if self.result_id != exact_grammar_job_result_id(self):
            raise ValueError("v26.114 Job result identity changed")
        return self


class ExactGrammarCellSummary(FrozenModel):
    ...
    schema_version: Literal["finance_v26_exact_grammar_cell_summary.v1"] = (
        "finance_v26_exact_grammar_cell_summary.v1"
    )

    @model_validator(mode="after")
    def validate_summary(self) -> ExactGrammarCellSummary:
        if self.summary_id != exact_grammar_cell_summary_id(self):
            raise ValueError("v26.114 Cell summary identity changed")
        return self


class ExactGrammarRawLineageAudit(FrozenModel):
    audit_id: str = Field(min_length=1)
    runner_contract_id: str = EXPECTED_RUNNER_CONTRACT_ID
    ...
    stage_two_provider_call_count: Literal[0] = 0
    status: Literal["passed"] = "passed"
    schema_version: Literal["finance_v26_exact_grammar_raw_lineage.v1"] = (
        "finance_v26_exact_grammar_raw_lineage.v1"
    )

    @model_validator(mode="after")
    def validate_audit(self) -> ExactGrammarRawLineageAudit:
        if self.file_count != len(self.files):
            raise ValueError("v26.114 Raw Lineage file count changed")
        if self.exact_byte_replay_pass_count != self.file_count:
            raise ValueError("v26.114 Raw Lineage replay denominator changed")
        if self.unique_provider_artifact_id_count != self.provider_call_count:
            raise ValueError("v26.114 Provider identities are not unique")
        if self.audit_id != exact_grammar_raw_lineage_audit_id(self):
            raise ValueError("v26.114 Raw Lineage identity changed")
        return self
...
class PreexecutionValidityAudit(FrozenModel):
    ...
    exact_v2_prompt_parser_commit_path_computed: Literal[True] = True
    status: Literal["passed"] = "passed"
    schema_version: Literal["finance_v26_exact_grammar_preexecution_validity_audit.v1"] = (
        "finance_v26_exact_grammar_preexecution_validity_audit.v1"
    )
    ...
    def validate_audit(self) -> PreexecutionValidityAudit:
        if tuple(item.job_id for item in self.rows) != tuple(
            ...
            raise ValueError("v26.114 pre-execution validity rows are not canonical")
        if self.audit_id != preexecution_validity_audit_id(self):
            raise ValueError("v26.114 pre-execution validity identity changed")
        return self


class ExactGrammarExecutionReport(FrozenModel):
    report_id: str = Field(min_length=1)
    run_id: Literal["finance_v26_114_exact_response_grammar_calibration_execution_v1_20260823"] = (
        RUN_ID
    )
    ...
    model_failure_family_counts: dict[str, int]
    model_failure_subtype_counts: dict[str, int]
    semantic_proposal_response_payload_count: int = Field(ge=0)
    exact_abi_accepted_count: int = Field(ge=0)
    exact_abi_failure_count: int = Field(ge=0)
    wrong_state_binding_count: int = Field(ge=0)
    semantic_proposal_commit_count: int = Field(ge=0)
    observation_count: int = Field(ge=0)
    abi_failure_counts_by_logical_request_index: dict[str, int]
    no_rescue_job_count: int = Field(ge=0, le=32)
    rescue_recovered_job_count: int = Field(ge=0, le=32)
    rescue_recovered_then_later_rescuable_failure_job_count: int = Field(ge=0, le=32)
    ...
    cell_summaries: tuple[ExactGrammarCellSummary, ...] = Field(min_length=12, max_length=12)
    exact_model_passed: bool
    ...
    next_permitted_stage: Literal["exact_response_grammar_calibration_postrun_audit_only"] = (
        POSTRUN_AUDIT_TRANSITION
    )
    schema_version: Literal["finance_v26_exact_grammar_execution_report.v1"] = (
        "finance_v26_exact_grammar_execution_report.v1"
    )

    @model_validator(mode="after")
    def validate_report(self) -> ExactGrammarExecutionReport:
        if sum(self.terminal_counts.values()) != 32:
            raise ValueError("v26.114 terminal denominator changed")
        if self.report_id != exact_grammar_execution_report_id(self):
            raise ValueError("v26.114 execution report identity changed")
        return self
...
class PreparedExecution(BaseModel):
    ...
    preflight_report: ExactGrammarRunnerPreflightReport
...
def execution_source_replay_audit_id(value: ExecutionSourceReplayAudit) -> str:
    return _identity(value, "audit_id", "finance_v26_exact_grammar_execution_source_replay:")


def exact_grammar_job_result_id(value: ExactGrammarJobResult) -> str:
    return _identity(value, "result_id", "finance_v26_exact_grammar_job_result:")


def exact_grammar_cell_summary_id(value: ExactGrammarCellSummary) -> str:
    return _identity(value, "summary_id", "finance_v26_exact_grammar_cell_summary:")


def exact_grammar_raw_lineage_audit_id(value: ExactGrammarRawLineageAudit) -> str:
    return _identity(value, "audit_id", "finance_v26_exact_grammar_raw_lineage:")


def preexecution_validity_audit_id(value: PreexecutionValidityAudit) -> str:
    return _identity(value, "audit_id", "finance_v26_exact_grammar_preexecution_validity_audit:")


def exact_grammar_execution_report_id(value: ExactGrammarExecutionReport) -> str:
    return _identity(value, "report_id", "finance_v26_exact_grammar_execution_report:")
    ...
    *,
    ...
    for root in (implementation_root, package_root):
        ...
        if candidate.is_file() and sha256_file(candidate) == expected_sha256:
            return candidate
    raise ValueError(f"v26.114 cannot replay bound file: {relative_path}")
...
def build_execution_source_replay(
    *,
    ...
    report = ExactGrammarRunnerPreflightReport.model_validate(
    ...
    for item in predecessor.entries:
        ...
        entries[item.relative_path] = ExecutionSourceReplayEntry(
            ...
            source_kind="v26_113_transitive_source",
    ...
    if len(output_paths) != 7:
        raise ValueError("v26.114 expected exactly seven v26.113 outputs")
    ...
    entries[IMPLEMENTATION_PATH] = ExecutionSourceReplayEntry(
        ...
        source_kind="v26_114_implementation",
        expected_sha256=observed,
        observed_sha256=observed,
...
def prepare_two_stage_execution(
    *,
    ...
    if (
        ...
        raise ValueError("v26.114 static execution denominator changed")
    ...
    write_json_atomic(
        output_dir / "runner_contract.json",
        contract.model_dump(mode="json"),
    )
    write_json_atomic(
        output_dir / "frozen_stage_one_response_grammar.json",
...
def _completed_verification(
    *,
    ...
    if raw.completed_result is None:
        raise ValueError("v26.114 completed verification lacks a completed result")
    ...
    if set(checks) != set(VERIFICATION_CHECK_IDS):
        raise ValueError("v26.114 Verifier Gate vector changed")
    values: dict[str, Any] = {
...
def build_preexecution_validity_audit(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="v26_114_preexecution_validity_") as temporary:
        root = Path(temporary)
        for job in sorted(static.manifest.jobs, key=lambda item: item.job_id):
            ...
            if (
                ...
                raise ValueError(f"v26.114 pre-execution Independent Validity failed: {job.job_id}")
            rows.append(
...
def project_job_result(
    *,
    ...
    wrong_state = tuple(
        ...
        for item in semantic_attempts
        if item.model_failure_classification is not None
        and item.model_failure_classification.subtype
        == "semantic_proposal_binds_wrong_public_state"
    )
    abi_failure_positions = Counter(str(item.logical_request_index) for item in abi_failures)
    rescue_usable_position = next(
        (
            index
            for index, item in enumerate(raw.attempts)
            if item.phase == "rescue" and item.disposition == "usable"
        ),
        None,
    )
    later_rescuable_failure = bool(
        rescue_usable_position is not None
        and any(
            item.disposition == "completion_failure"
            or (
                item.model_failure_classification is not None
                and item.model_failure_classification.family
                in {"response_serialization_failure", "phase_control_failure", "prompt_echo"}
            )
            for item in raw.attempts[rescue_usable_position + 1 :]
        )
    )
    values: dict[str, Any] = {
        "job_id": raw.job.job_id,
        ...
        "rescue_recovered_then_later_rescuable_failure": later_rescuable_failure,
        ...
        "semantic_proposal_response_payload_count": sum(
            item.response_payload_present for item in semantic_attempts
        ),
        "exact_abi_accepted_count": sum(item.disposition == "usable" for item in semantic_attempts),
        "exact_abi_failure_count": len(abi_failures),
        "wrong_state_binding_count": len(wrong_state),
        "semantic_proposal_commit_count": len(raw.commits),
        "abi_failure_counts_by_logical_request_index": dict(sorted(abi_failure_positions.items())),
        ...
        "rollout_headroom_tokens": 260000 - raw.cumulative_provider_tokens,
    ...
    provisional = ExactGrammarJobResult.model_construct(result_id="pending", **values)
    return ExactGrammarJobResult(
        result_id=exact_grammar_job_result_id(provisional),
        **values,
    )
...
def _cell_summaries(
    results: Sequence[ExactGrammarJobResult],
) -> tuple[ExactGrammarCellSummary, ...]:
    cells: dict[tuple[str, str], list[ExactGrammarJobResult]] = defaultdict(list)
    for item in results:
        cells[(item.mechanism_id, item.requested_path_strategy_id)].append(item)
    summaries: list[ExactGrammarCellSummary] = []
    for (mechanism, path), rows in sorted(cells.items()):
        ...
        provisional = ExactGrammarCellSummary.model_construct(summary_id="pending", **values)
        summaries.append(
            ExactGrammarCellSummary(
                summary_id=exact_grammar_cell_summary_id(provisional),
                **values,
            )
        )
    ...
    *,
    ...
    for result in results:
        ...
        if replayed.model_dump(mode="json") != raw.model_dump(mode="json"):
            raise ValueError(f"v26.114 Raw replay changed: {result.job_id}")
        ...
        for descriptor in raw.provider_call_artifacts:
            ...
            if (
                ...
                raise ValueError("v26.114 Provider Artifact binding changed")
    ...
    provisional = ExactGrammarRawLineageAudit.model_construct(audit_id="pending", **values)
    return ExactGrammarRawLineageAudit(
        audit_id=exact_grammar_raw_lineage_audit_id(provisional),
        **values,
    )
...
def make_execution_report(
    *,
    prepared: PreparedExecution,
    results: Sequence[ExactGrammarJobResult],
    lineage: ExactGrammarRawLineageAudit,
    ...
    abi_positions = Counter(
        {
            key: sum(
                item.abi_failure_counts_by_logical_request_index.get(key, 0) for item in results
            )
            for key in {
                key for item in results for key in item.abi_failure_counts_by_logical_request_index
            }
        }
    )
    values: dict[str, Any] = {
        ...
        "model_failure_subtype_counts": dict(sorted(subtype_counts.items())),
        "semantic_proposal_response_payload_count": sum(
            item.semantic_proposal_response_payload_count for item in results
        ),
        "exact_abi_accepted_count": sum(item.exact_abi_accepted_count for item in results),
        "exact_abi_failure_count": sum(item.exact_abi_failure_count for item in results),
        "wrong_state_binding_count": sum(item.wrong_state_binding_count for item in results),
        "semantic_proposal_commit_count": sum(
            item.semantic_proposal_commit_count for item in results
        ),
        "observation_count": sum(item.observation_count for item in results),
        "abi_failure_counts_by_logical_request_index": dict(sorted(abi_positions.items())),
        "no_rescue_job_count": sum(item.rescue_attempt_count == 0 for item in results),
        "rescue_recovered_job_count": sum(item.rescue_success for item in results),
        "rescue_recovered_then_later_rescuable_failure_job_count": sum(
            item.rescue_recovered_then_later_rescuable_failure for item in results
        ),
    ...
    provisional = ExactGrammarExecutionReport.model_construct(report_id="pending", **values)
    return ExactGrammarExecutionReport(
        report_id=exact_grammar_execution_report_id(provisional),
        **values,
    )
...
def _run_one_job(
    *,
    ...
    output_dir: Path,
) -> tuple[ExactGrammarJobResult, TwoStageRawExecution]:
...
def _write_checkpoint(path: Path, rows: Sequence[ExactGrammarJobResult]) -> None:
...
def run_two_stage_calibration(
    *,
    ...
    if pending and report_path.exists():
        raise ValueError("v26.114 completed report exists while Jobs remain pending")
    if not pending and report_path.exists():
        report = ExactGrammarExecutionReport.model_validate(_load(report_path))
        if report.runner_contract_id != prepared.runner_contract.contract_id:
            raise ValueError("v26.114 completed report crosses Runner Contracts")
        return report
    ...
    for job in model_pending_jobs:
        ...
        if provider_dir.exists() and any(provider_dir.glob("call_*.json")):
            raise ValueError("orphan v26.114 Provider Artifacts require a fresh Recovery Contract")
    print(
        f"[v26.114] resuming {len(completed)}/32; "
    ...
    if len(results) != 32:
        raise ValueError("v26.114 execution denominator is incomplete")
    for job in jobs:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run the exact v26.114 two-stage semantic Proposal calibration"
    )
    parser.add_argument(
