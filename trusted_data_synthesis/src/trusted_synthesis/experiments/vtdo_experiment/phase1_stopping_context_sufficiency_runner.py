...
PREFIX = "stopping_context_sufficiency"
BASE_CONTRACT_NAME = "finance_stopping_shape_policy_contract.json"
WRAPPER_CONTRACT_NAME = "finance_stopping_context_sufficiency_contract.json"
RAW_AUDIT_NAME = "finance_stopping_context_sufficiency_raw_audit.json"
SHAPE_REPORT_NAME = "finance_stopping_context_sufficiency_shape_report.json"
MECHANISM_REPORT_NAME = "finance_stopping_context_sufficiency_mechanism_report.json"
REPORT_NAME = "finance_stopping_context_sufficiency_report.json"
REPORT_MARKDOWN_NAME = "finance_stopping_context_sufficiency_report.md"
MANIFEST_NAME = "finance_stopping_context_sufficiency_manifest.json"


def prepare_context_sufficiency_contract(
    *,
    protocol_path: Path,
    ...
    task_instances = {
        task_id: canonical_hash(
            ...
            prefix="finance_stopping_context_sufficiency_task_instance:",
        )
        for task_id, item in record_by_task.items()
    }
    rollout_tokens = {
        f"{binding.binding_id}|{replicate}": canonical_hash(
            ...
            prefix="finance_stopping_context_sufficiency_rollout:",
        )
    ...
    provisional_wrapper = FinanceStoppingContextSufficiencyContract.model_construct(
        contract_id="pending", **wrapper_values
    )
    wrapper = FinanceStoppingContextSufficiencyContract(
        contract_id=_artifact_id(
            provisional_wrapper,
            "contract_id",
            "finance_stopping_context_sufficiency_contract:",
        ),
        **wrapper_values,
    )
...
def run_context_sufficiency(
    *, contract_path: Path, output_dir: Path, workers: int
    ...
    if (output_dir / REPORT_NAME).exists():
        raise ValueError("v25.47 immutable final report already exists")
...
def finalize_context_sufficiency_run(
    *, contract_path: Path, output_dir: Path
    ...
    if missing:
        raise ValueError(f"v25.47 finalizer lacks expensive artifacts: {missing}")
...
def make_context_sufficiency_mechanism_report(
    wrapper: FinanceStoppingContextSufficiencyContract,
    ...
    pair_results = []
    period_values_by_pair: list[tuple[bool, ...]] = []
    definition_values_by_pair: list[tuple[bool, ...]] = []
    for pair in sorted(population.contextual_pairs, key=lambda item: item.pair_id):
        ...
        period_values_by_pair.append(tuple(period_values))
        definition_values_by_pair.append(tuple(definition_values))
    ...
    balanced_accuracy = 0.5 * (
        period_total / (PAIR_COUNT * REPLICAS) + definition_total / (PAIR_COUNT * REPLICAS)
    )
    policy_gain = balanced_accuracy - wrapper.flip_thresholds.constant_action_baseline
    bootstrap_lcb = _paired_branch_balanced_bootstrap_lcb(
        tuple(period_values_by_pair),
        tuple(definition_values_by_pair),
        replicates=wrapper.flip_thresholds.bootstrap_replicates,
        seed=wrapper.flip_thresholds.bootstrap_seed,
    )
    rejections = []
    ...
    if balanced_accuracy < wrapper.flip_thresholds.minimum_branch_balanced_first_action_accuracy:
        rejections.append("minimum_branch_balanced_first_action_accuracy")
    if policy_gain < wrapper.flip_thresholds.minimum_contextual_policy_gain:
        rejections.append("minimum_contextual_policy_gain")
    if bootstrap_lcb <= wrapper.flip_thresholds.minimum_branch_balanced_accuracy_bootstrap_lcb:
        rejections.append("minimum_branch_balanced_accuracy_bootstrap_lcb")
    values = {
        ...
        "branch_balanced_first_action_accuracy": balanced_accuracy,
        "constant_action_baseline": wrapper.flip_thresholds.constant_action_baseline,
        "contextual_policy_gain": policy_gain,
        "branch_balanced_accuracy_bootstrap_lcb95": bootstrap_lcb,
        "bootstrap_replicates": wrapper.flip_thresholds.bootstrap_replicates,
        "bootstrap_seed": wrapper.flip_thresholds.bootstrap_seed,
        "thresholds": wrapper.flip_thresholds,
        "passed": not rejections,
        "rejection_reasons": tuple(rejections),
    }
    provisional = FinanceStoppingContextSufficiencyMechanismReport.model_construct(
        report_id="pending", **values
    )
    return FinanceStoppingContextSufficiencyMechanismReport(
        report_id=_artifact_id(
            provisional,
            "report_id",
            "finance_stopping_context_sufficiency_mechanism_report:",
        ),
        **values,
    )


def _paired_branch_balanced_bootstrap_lcb(
    period_values_by_pair: tuple[tuple[bool, ...], ...],
    definition_values_by_pair: tuple[tuple[bool, ...], ...],
    *,
    replicates: int,
    seed: int,
) -> float:
    """One-sided 95% LCB, preserving pair and within-pair replica dependence."""

    if not (
        len(period_values_by_pair) == len(definition_values_by_pair) == PAIR_COUNT
        and all(len(values) == REPLICAS for values in period_values_by_pair)
        and all(len(values) == REPLICAS for values in definition_values_by_pair)
        and replicates >= 1
    ):
        raise ValueError("Context-sufficiency bootstrap denominator changed")
    rng = random.Random(seed)
    estimates: list[float] = []
    denominator = PAIR_COUNT * REPLICAS
    for _ in range(replicates):
        period_count = 0
        definition_count = 0
        for _pair_draw in range(PAIR_COUNT):
            pair_index = rng.randrange(PAIR_COUNT)
            for _replica_draw in range(REPLICAS):
                replica_index = rng.randrange(REPLICAS)
                period_count += int(period_values_by_pair[pair_index][replica_index])
                definition_count += int(definition_values_by_pair[pair_index][replica_index])
        estimates.append(0.5 * (period_count / denominator + definition_count / denominator))
    estimates.sort()
    lower_index = max(0, math.ceil(0.05 * replicates) - 1)
...
def _finalize_run(
    *,
    ...
    flip: FinanceStoppingContextSufficiencyMechanismReport | None = None
    if raw.shape_analysis_authorized:
        ...
        flip = make_context_sufficiency_mechanism_report(wrapper, population, records)
        _write_or_verify_json(output_dir / MECHANISM_REPORT_NAME, flip.model_dump(mode="json"))
...
def _make_overall_report(
    ...
    flip: FinanceStoppingContextSufficiencyMechanismReport | None,
) -> FinanceStoppingContextSufficiencyReport:
    all_shapes = bool(shape and shape.all_shapes_contract_passing)
    mechanism_passed = bool(flip and flip.passed)
    values = {
        "contract_id": wrapper.contract_id,
        ...
        "mechanism_report_id": flip.report_id if flip else None,
        ...
        "contextual_mechanism_fidelity_passing": mechanism_passed,
        "all_v25_47_gates_passing": bool(
            raw.shape_analysis_authorized and all_shapes and mechanism_passed
        ),
        "next_permitted_stage": (
            ...
            if not raw.shape_analysis_authorized
                ...
                if all_shapes and mechanism_passed
                else "contextual_shape_redesign_only"
            )
        ),
    }
    provisional = FinanceStoppingContextSufficiencyReport.model_construct(
        report_id="pending", **values
    )
    return FinanceStoppingContextSufficiencyReport(
        report_id=_artifact_id(
            provisional,
            "report_id",
            "finance_stopping_context_sufficiency_report:",
        ),
        **values,
    )
...
def _load_execution_inputs(
    ...
    wrapper = FinanceStoppingContextSufficiencyContract.model_validate_json(
    ...
    if wrapper.implementation_manifest != _execution_implementation_manifest():
        raise ValueError("v25.47 execution implementation changed after freeze")
    protocol = FinanceStoppingContextSufficiencyProtocol.model_validate_json(
    ...
    if base.implementation_manifest != _shape_execution_implementation_manifest():
        raise ValueError("v25.47 frozen Shape runner implementation changed")
    if not (
        ...
        raise ValueError("v25.47 wrapper, base Contract, and population differ")
...
def _verify_population(
    ...
    if not (
        ...
        and population.next_permitted_stage == "flash_context_sufficiency_development"
        and population.implementation_manifest == _population_implementation_manifest()
    ):
        raise ValueError("v25.47 population is not execution-ready")
...
def _execution_implementation_manifest() -> dict[str, str]:
    ...
    paths = (
        ...
        "src/trusted_synthesis/experiments/vtdo_experiment/phase1_stopping_context_sufficiency_runner.py",
        "src/trusted_synthesis/experiments/vtdo_experiment/phase1_stopping_context_sufficiency_finalize.py",
    )
...
def _make_manifest(
    *,
    wrapper: FinanceStoppingContextSufficiencyContract,
    report: FinanceStoppingContextSufficiencyReport,
    raw: InstrumentResetRawAudit,
    shape: FinanceStoppingShapePolicyReport | None,
    flip: FinanceStoppingContextSufficiencyMechanismReport | None,
    output_dir: Path,
    ...
    names = (
        ...
        *((SHAPE_REPORT_NAME, MECHANISM_REPORT_NAME) if shape and flip else ()),
    )
    return {
        "schema_version": "finance_stopping_context_sufficiency_manifest.v1",
        "contract_id": wrapper.contract_id,
...
def _write_or_verify_json(path: Path, value: Mapping[str, Any] | dict[str, Any]) -> None:
    if path.exists():
        ...
        if observed != value:
            raise ValueError(f"v25.47 immutable JSON differs: {path}")
        return
    path.write_text(
...
def _write_or_verify_jsonl(path: Path, values: Any) -> None:
    ...
    if path.exists():
        ...
        if observed != rows:
            raise ValueError(f"v25.47 immutable JSONL differs: {path}")
        return
    path.write_text(
...
def _write_or_verify_text(path: Path, value: str) -> None:
    if path.exists():
        if path.read_text(encoding="utf-8") != value:
            raise ValueError(f"v25.47 immutable text differs: {path}")
        return
...
def _render_report(
    ...
    lines = [
        "# Finance v25.47 Contextual Counterfactual Development",
        "",
        ...
        "- Contextual mechanism fidelity passing: "
        f"**{report.contextual_mechanism_fidelity_passing}**",
        f"- All v25.47 gates passing: **{report.all_v25_47_gates_passing}**",
    ...
    if flip is not None:
        lines.extend(
            [
                "## Contextual mechanism fidelity",
                "",
                f"- Dual-correct consistency: **{flip.contextual_flip_consistency:.4f}**",
                f"- Informative pairs: **{flip.informative_pair_count}/4**",
                "- Branch-balanced first-action accuracy: "
                f"**{flip.branch_balanced_first_action_accuracy:.4f}**",
                f"- Constant-action baseline: **{flip.constant_action_baseline:.4f}**",
                f"- Contextual policy gain: **{flip.contextual_policy_gain:.4f}**",
                "- Paired hierarchical bootstrap LCB95: "
                f"**{flip.branch_balanced_accuracy_bootstrap_lcb95:.4f}**",
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Prepare, run, or finalize Finance v25.47")
...
def main() -> None:
    ...
    if args.command == "prepare":
        prepare_context_sufficiency_contract(
