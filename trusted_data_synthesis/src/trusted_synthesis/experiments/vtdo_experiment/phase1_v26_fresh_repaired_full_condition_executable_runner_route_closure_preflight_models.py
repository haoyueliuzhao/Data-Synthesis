...
SCHEMA_VERSION: Final = "fresh_repaired_full_condition_executable_runner_route_closure_preflight.v1"
...
class ExternalRouteClosureAuthorization(FrozenModel):
    authorization_id: str = Field(min_length=1)
    review_sha256: Literal["4777e6e354b5bd114dcfa1fc549bb419be1ea5daed58e8e64ebaf263ab35b2f1"]
    review_byte_count: Literal[13410] = 13_410
    review_audit_result: Literal["VALID_NEGATIVE_INDEPENDENT_AUDIT"] = (
    ...
    operator_directive: Literal["参照审计执行v26.208，你刚才缺漏了"] = (
        "参照审计执行v26.208，你刚才缺漏了"
    )
    operator_directive_sha256: Literal[
        "74a283edbb4efd8e0810f0b2f6295bba7d9a47270a48ded67f0d867473d6c07c"
    ]
    operator_directive_byte_count: Literal[46] = 46
    ...
    def validate_authorization(self) -> ExternalRouteClosureAuthorization:
        ...
        if (
            ...
            raise ValueError("v26.208 operator directive bytes differ")
        if self.authorization_id != identity(
            self,
            "authorization_id",
            "finance_v26_208_external_route_closure_authorization:",
        ):
            raise ValueError("v26.208 external route-closure Authorization identity differs")
        return self


class V207PredecessorFreeze(FrozenModel):
    ...
    @model_validator(mode="after")
    def validate_freeze(self) -> V207PredecessorFreeze:
        if self.freeze_id != identity(
            self,
            "freeze_id",
            "finance_v26_208_v207_predecessor_freeze:",
        ):
            raise ValueError("v26.208 v26.207 predecessor Freeze identity differs")
        return self
...
class ImplementationBinding(FrozenModel):
    ...
    def validate_binding(self) -> ImplementationBinding:
        ...
        if paths != tuple(sorted(set(paths))):
            raise ValueError("v26.208 implementation file vector differs")
        if self.implementation_id != identity(
            self,
            "implementation_id",
            "fresh_repaired_executable_route_implementation_binding:",
        ):
            raise ValueError("v26.208 Implementation Binding identity differs")
        return self
...
class ExecutableRunnerPackage(FrozenModel):
    ...
    def validate_package(self) -> ExecutableRunnerPackage:
        if len(self.schedule_ids) != len(self.component_keys):
            raise ValueError("v26.208 Package Schedule/Component denominator differs")
        if self.package_id != identity(
            self,
            "package_id",
            "fresh_repaired_executable_full_condition_runner_package:",
        ):
            raise ValueError("v26.208 executable Runner Package identity differs")
        return self
...
class ExecutableRunnerPackageCatalog(FrozenModel):
    ...
    def validate_catalog(self) -> ExecutableRunnerPackageCatalog:
        if len({item.package_id for item in self.packages}) != 32:
            raise ValueError("v26.208 executable Package denominator differs")
        if self.source_v206_package_ids != tuple(
            sorted(item.source_v206_package_id for item in self.packages)
        ):
            raise ValueError("v26.208 source Package set differs")
        if self.catalog_id != identity(
            self,
            "catalog_id",
            "fresh_repaired_executable_full_condition_package_catalog:",
        ):
            raise ValueError("v26.208 Package Catalog identity differs")
        return self
...
class ExecutableDevelopmentJob(FrozenModel):
    ...
    def validate_job(self) -> ExecutableDevelopmentJob:
        ...
        expected = (
            canonical_hash(parent, prefix="fresh_repaired_executable_raw_namespace:"),
            canonical_hash(parent, prefix="fresh_repaired_executable_result_namespace:"),
            canonical_hash(parent, prefix="fresh_repaired_executable_trace_namespace:"),
            canonical_hash(parent, prefix="fresh_repaired_executable_outcome_namespace:"),
            canonical_hash(parent, prefix="fresh_repaired_executable_deterministic_seed:"),
        )
        actual = (
            self.raw_namespace,
        ...
        if actual != expected:
            raise ValueError("v26.208 Job namespace parent differs")
        if self.job_id != identity(
            self,
            "job_id",
            "fresh_repaired_executable_full_condition_development_job:",
        ):
            raise ValueError("v26.208 Development Job identity differs")
        return self
...
class ExecutableDevelopmentManifest(FrozenModel):
    ...
    def validate_manifest(self) -> ExecutableDevelopmentManifest:
        ...
        if len(set(ids)) != 192 or self.expected_job_ids != tuple(sorted(ids)):
            raise ValueError("v26.208 Manifest Job set differs")
        if self.source_v206_job_ids != tuple(sorted(item.source_v206_job_id for item in self.jobs)):
            raise ValueError("v26.208 source Job set differs")
        if len({(item.package_id, item.replica_index) for item in self.jobs}) != 192:
            raise ValueError("v26.208 Package x Replica denominator differs")
        namespaces = tuple(
        ...
        if any(len(values) != 192 for values in namespaces):
            raise ValueError("v26.208 evidence namespaces are not unique")
        if self.manifest_id != identity(
            self,
            "manifest_id",
            "fresh_repaired_executable_full_condition_manifest:",
        ):
            raise ValueError("v26.208 Manifest identity differs")
        return self
...
class ExecutableRunnerContract(FrozenModel):
    ...
    def validate_runner(self) -> ExecutableRunnerContract:
        if self.runner_id != identity(
            self,
            "runner_id",
            "fresh_repaired_executable_full_condition_runner:",
        ):
            raise ValueError("v26.208 executable Runner identity differs")
        return self
...
class ExecutableExecutionContract(FrozenModel):
    ...
    def validate_execution(self) -> ExecutableExecutionContract:
        if self.contract_id != identity(
            self,
            "contract_id",
            "fresh_repaired_executable_full_condition_execution_contract:",
        ):
            raise ValueError("v26.208 Execution Contract identity differs")
        return self
    ...
    def validate_certificate(self) -> ValidatedRequestCertificate:
        if self.phase == "final":
            if self.candidate_action_ids:
                raise ValueError("v26.208 Final certificate carries Action candidates")
        elif not self.candidate_action_ids or self.model_visible_grammar_id_key_count != 0:
            raise ValueError("v26.208 Action certificate lacks Candidates or exposes grammar_id")
        if self.certificate_id != identity(
            self,
            "certificate_id",
            "fresh_repaired_executable_request_validation_certificate:",
        ):
            raise ValueError("v26.208 Request Certificate identity differs")
        return self
...
class PreTransportReceipt(FrozenModel):
    ...
    def validate_receipt(self) -> PreTransportReceipt:
        if self.receipt_id != identity(
            self,
            "receipt_id",
            "fresh_repaired_executable_pre_transport_receipt:",
        ):
            raise ValueError("v26.208 pre-transport Receipt identity differs")
        return self
...
class ExecutableInvocationRecord(FrozenModel):
    ...
    def validate_record(self) -> ExecutableInvocationRecord:
        if self.typed_terminal is None and self.event_sequence != EVENT_SEQUENCE:
            raise ValueError("v26.208 successful invocation order differs")
        if self.phase == "final":
            if self.candidate_action_ids or self.selected_action_id is not None:
                raise ValueError("v26.208 Final invocation carries Action fields")
            if self.action_accepted is not None:
                raise ValueError("v26.208 Final invocation carries Action acceptance")
        elif not self.candidate_action_ids:
            raise ValueError("v26.208 Action invocation lacks Candidate set")
        if self.invocation_id != identity(
            self,
            "invocation_id",
            "fresh_repaired_executable_invocation_record:",
        ):
            raise ValueError("v26.208 Invocation identity differs")
        return self
...
class ExecutableInvocationCensus(FrozenModel):
    ...
    def validate_census(self) -> ExecutableInvocationCensus:
        ...
        if any(len({getattr(item, field) for item in self.rows}) != 792 for field in fields):
            raise ValueError("v26.208 executable invocation identity set differs")
        counts = {
        ...
        if counts != {
            ...
            raise ValueError("v26.208 invocation phase denominator differs")
        if any(
            item.event_sequence != EVENT_SEQUENCE or item.typed_terminal is not None
            for item in self.rows
        ):
            raise ValueError("v26.208 denominator contains a failed or bypassed invocation")
        if self.census_id != identity(
            self,
            "census_id",
            "finance_v26_208_executable_invocation_census:",
        ):
            raise ValueError("v26.208 Invocation Census identity differs")
        return self
...
class ExecutableJobControlRow(FrozenModel):
    ...
    def validate_row(self) -> ExecutableJobControlRow:
        if self.action_and_correction_count != (
            ...
            raise ValueError("v26.208 per-Job invocation geometry differs")
        ...
        if (self.raw_id, self.result_id, self.trace_id, self.outcome_id) != (
            ...
            raise ValueError("v26.208 Raw -> Result -> Trace -> Outcome chain differs")
        if self.row_id != identity(
            self,
            "row_id",
            "finance_v26_208_executable_job_control_row:",
        ):
            raise ValueError("v26.208 Job Control Row identity differs")
        return self
...
class FullConditionExecutionControlAudit(FrozenModel):
    ...
    def validate_audit(self) -> FullConditionExecutionControlAudit:
        if len({item.job_id for item in self.rows}) != 192:
            raise ValueError("v26.208 execution Job denominator differs")
        layer_ids = {
            value
            for item in self.rows
        ...
        if len(layer_ids) != 768:
            raise ValueError("v26.208 evidence-layer identity denominator differs")
        if sum(item.correction_count for item in self.rows) != 120:
            raise ValueError("v26.208 Correction denominator differs")
        if self.audit_id != identity(
            self,
            "audit_id",
            "finance_v26_208_full_condition_execution_control_audit:",
        ):
            raise ValueError("v26.208 Execution Control Audit identity differs")
        return self
...
class SourceAndDynamicNoBypassAudit(FrozenModel):
    ...
    def validate_audit(self) -> SourceAndDynamicNoBypassAudit:
        if (
            ...
            raise ValueError("v26.208 dynamic route partition differs")
        if self.audit_id != identity(
            self,
            "audit_id",
            "finance_v26_208_source_dynamic_no_bypass_audit:",
        ):
            raise ValueError("v26.208 source/dynamic no-bypass Audit identity differs")
        return self
...
class TypedFailureControl(FrozenModel):
    ...
    def validate_control(self) -> TypedFailureControl:
        if self.expected_terminal != self.observed_terminal:
            raise ValueError("v26.208 Failure Control terminal differs")
        if self.control_id != identity(
            self,
            "control_id",
            "finance_v26_208_typed_failure_control:",
        ):
            raise ValueError("v26.208 Failure Control identity differs")
        return self
...
class TypedFailureControlAudit(FrozenModel):
    ...
    def validate_audit(self) -> TypedFailureControlAudit:
        if len({item.control_name for item in self.controls}) != 5:
            raise ValueError("v26.208 Failure Control denominator differs")
        if self.audit_id != identity(
            self,
            "audit_id",
            "finance_v26_208_typed_failure_control_audit:",
        ):
            raise ValueError("v26.208 Failure Control Audit identity differs")
        return self
...
class DynamicNonReferenceBranchAudit(FrozenModel):
    ...
    diagnostic_transport_dispatch_count: Literal[2] = 2
    ...
    def validate_audit(self) -> DynamicNonReferenceBranchAudit:
        if (
            ...
            raise ValueError("v26.208 dynamic nonreference branch witness differs")
        if self.audit_id != identity(
            self,
            "audit_id",
            "finance_v26_208_dynamic_nonreference_branch_audit:",
        ):
            raise ValueError("v26.208 Dynamic Branch Audit identity differs")
        return self
...
class EstimandResourceBoundaryAudit(FrozenModel):
    ...
    def validate_audit(self) -> EstimandResourceBoundaryAudit:
        if self.audit_id != identity(
            self,
            "audit_id",
            "finance_v26_208_estimand_resource_boundary_audit:",
        ):
            raise ValueError("v26.208 Estimand/Resource Boundary Audit identity differs")
        return self


class RouteClosureGateAudit(FrozenModel):
    audit_id: str = Field(min_length=1)
    predecessor_freeze_id: str = Field(min_length=1)
    manifest_id: str = Field(min_length=1)
    execution_contract_id: str = Field(min_length=1)
    ...
    r0_exact_v207_authority_freeze_and_semantic_condition_passed: Literal[True] = True
    r1_fresh_executable_identity_closure_passed: Literal[True] = True
    r2_source_and_dynamic_no_bypass_closure_passed: Literal[True] = True
    ...
    @model_validator(mode="after")
    def validate_audit(self) -> RouteClosureGateAudit:
        if self.audit_id != identity(
            self,
            "audit_id",
            "finance_v26_208_route_closure_gate_audit:",
        ):
            raise ValueError("v26.208 Route Closure Gate Audit identity differs")
        return self
...
class ProspectiveTransition(FrozenModel):
    ...
    def validate_transition(self) -> ProspectiveTransition:
        if self.transition_id != identity(
            self,
            "transition_id",
            "finance_v26_208_transition:",
        ):
            raise ValueError("v26.208 Transition identity differs")
        return self
...
class SourceIdentity(FrozenModel):
    ...
    def validate_source(self) -> SourceIdentity:
        if self.implementation_files != tuple(sorted(set(self.implementation_files))):
            raise ValueError("v26.208 source file vector differs")
        if self.source_identity_id != identity(
            self,
            "source_identity_id",
            "finance_v26_208_source_identity:",
        ):
            raise ValueError("v26.208 Source Identity differs")
        return self


class RouteClosurePreflightReport(FrozenModel):
    ...
    @model_validator(mode="after")
    def validate_report(self) -> RouteClosurePreflightReport:
        if self.report_id != identity(
            self,
            "report_id",
            "finance_v26_208_route_closure_preflight_report:",
        ):
            raise ValueError("v26.208 Report identity differs")
        return self
...
class ArtifactManifest(FrozenModel):
    ...
    def validate_manifest(self) -> ArtifactManifest:
        ...
        if paths != tuple(sorted(set(paths))):
            raise ValueError("v26.208 Artifact Manifest path set differs")
        if self.file_count != len(self.members):
            raise ValueError("v26.208 Artifact Manifest file count differs")
        if self.total_byte_count != sum(item.byte_count for item in self.members):
            raise ValueError("v26.208 Artifact Manifest byte count differs")
        expected_root = canonical_hash(
            tuple(item.model_dump(mode="json") for item in self.members),
            prefix="finance_v26_208_artifact_root:",
        )
        if self.artifact_root != expected_root:
            raise ValueError("v26.208 Artifact Root differs")
        if self.manifest_id != identity(
            self,
            "manifest_id",
            "finance_v26_208_artifact_manifest:",
        ):
            raise ValueError("v26.208 Artifact Manifest identity differs")
        return self
...
def artifact_manifest(run_id: str, payloads: dict[str, bytes]) -> ArtifactManifest:
    ...
    return cast(
        ...
        make_identity(
            ...
            field="manifest_id",
            prefix="finance_v26_208_artifact_manifest:",
        ),
    )
