"""Pre-response frozen retention rules instantiated against each new bound task.

No role aliases or modified event copies are used. Out-of-domain events remain
undetermined; actual effects, rejected proposals and declared supports stay distinct.
"""
...
from . import support as support_links
...
def _checked_support(entry):
    proof = entry["support_proof"]
    identity(proof, "cross_binding_support")
    require(
        proof["qualified"]
        ...
        and proof["base_anchor_id"] == entry["base_anchor"]["id"],
        "cross_binding.support_binding",
    )
    require(
        equal(entry["graph"]["nodes"], entry["finite_projection"]["nodes"]),
...
def _bound_trace(entry, role):
    ...
    saved = entry["support_proof"]["trace"][role]
...
def support_transition(entry, index):
    ...
    disclosed = context["evidence"]["disclosed_total"]["id"]
...
def project_entry(entry, condition, rule, contract):
    """Apply the entire prospectively frozen rule to one new registered outcome."""
    from .rules import quotient_rule

    require(rule == quotient_rule(), "cross_binding.projection_rule_drift")
    require(
        condition["rule_id"] == contract["rule_id"] == rule["id"],
        "cross_binding.projection_contract_rule",
    )
    reg, q, session = (entry[k] for k in ("registration", "qualification", "session"))
    identity(reg, "session_registration")
    identity(q, "qualification")
    require(
        q["registration_id"] == reg["id"]
        and q["session_id"] == (session["id"] if session else None)
        and reg["run_condition_id"] == condition["id"],
        "cross_binding.projection_source_binding",
    )
    audit = q.get("domain_audit")
    graph = audit.get("actual_decision_graph") if isinstance(audit, dict) else None
    metadata = {
        key: reg[key]
        for key in (
            "task_group",
    ...
    metadata.update(
        ...
        old_projection_supported=bool(audit and audit.get("projection_supported")),
    ...
    anchor = record(
        ...
        **metadata,
        finite_projection=audit.get("finite_projection") if audit else None,
    )
    proof = support_links.actual_support(entry, anchor)
    if q["qualified"] is not True:
        return record(
            ...
            **metadata,
            ...
            source_non_accept_ledger=graph["non_accept_event_ledger"] if graph else [],
            actual_support=proof,
            base_anchor=anchor,
            reason="unqualified registered outcome stays in its original denominator",
        )
    require(session is not None and graph is not None, "cross_binding.qualified_graph_missing")
    assert session is not None and graph is not None and audit is not None
    source_entry = {
        **entry,
        "audit": audit,
        "graph": graph,
    ...
    baseline = prior.project_entry(source_entry, rule["base_interpretations"], condition["id"])
    working = {
        **source_entry,
        "old_projection": baseline,
        "finite_projection": audit["finite_projection"],
        "support_proof": proof,
        "base_anchor": anchor,
    }
    ledger = copy.deepcopy(baseline["interpretation_ledger"])
    events, producers = session["events"], _producers(working)
    episodes, details, errors = [], [], []
    # Preserve proved episodes even when another episode made the baseline undetermined.
    for row in ledger:
        if row["disposition"] == "retained_behavior_relation":
            index, successor = row["sequence"], row["nearest_admitted_successor_sequence"]
            relation = prior.bind_retained_order(
                source_entry,
                prior.retained_proposal(events[index], events[successor], producers),
                events[index],
                successor,
                producers,
            )
            episodes.append((index, relation))
    grounding = None
    for row in ledger:
        if row["disposition"] != "undetermined":
            continue
        index, previous = row["sequence"], copy.deepcopy(row)
        try:
            require(
                row["source_event_sha256"] == sha(canonical_json_bytes(events[index])),
                "cross_binding.original_event_changed",
            )
            kind = (events[index].get("parsed") or {}).get("kind")
            if kind == "action":
                relation, evidence = support_transition(working, index)
                episodes.append((index, relation))
                detail = record(
                    "cross_binding_event_interpretation",
                    source_sequence=index,
                    source_submission_id=events[index]["submission"]["id"],
                    baseline_interpretation=previous,
                    evidence=evidence,
                    interpretation_type="retained_support_choice_transition",
                )
            elif kind == "final":
                if grounding is None:
                    relation, grounding = grounding_assertions(working)
                    episodes.append((grounding["first_sequence"], relation))
                ...
                raise ProtocolError("cross_binding.out_of_frozen_event_domain")
            details.append(detail)
            row.update(
                baseline_disposition=previous["disposition"],
                baseline_reason=previous.get("reason"),
                disposition="retained_behavior_relation",
                reason=None,
                interpretation={"rule_id": rule["id"], "detail_id": detail["id"]},
            )
        except (
            ProtocolError,
            KeyError,
            ...
            errors.append({"sequence": index, "reason": str(error) or type(error).__name__})
    rejected = [e for e in events if not e["receipt"]["admitted"]]
    errors.extend(error for error in baseline["errors"] if "sequence" not in error)
    supported = (
        not errors
        and len(ledger) == len(rejected)
        and all(row["disposition"] != "undetermined" for row in ledger)
    )
    retained: list[dict[str, Any]] = []
    for _, relation in sorted(episodes, key=lambda pair: pair[0]):
        if not retained or not equal(retained[-1], relation):
            retained.append(relation)
    behavior = (
        {**copy.deepcopy(audit["finite_projection"]), "retained_interactions": retained}
        if supported
        else None
    )
    if baseline["supported"]:
        require(equal(behavior, baseline["behavior_projection"]), "cross_binding.base_semantics")
    return record(
        "panel_quotient_projection",
        **metadata,
        ...
        actual_support=proof,
        base_anchor=anchor,
        base_rule_interpretation=baseline,
        source_non_accept_ledger=copy.deepcopy(graph["non_accept_event_ledger"]),
        errors=errors,
        complete_source_event_count=len(events),
        complete_source_events_sha256=sha(canonical_json_bytes(events)),
        base_nodes_and_final_unchanged=behavior is not None
        and equal({key: behavior[key] for key in ("nodes", "final")}, audit["finite_projection"]),
        original_raw_events_or_qualification_modified=False,
        actual_uses_edges_from_erroneous_assertions_added=False,
        new_outcome_specific_rule_extension=False,
    )
