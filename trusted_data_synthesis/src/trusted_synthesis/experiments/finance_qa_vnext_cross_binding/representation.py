"""New cross-binding exact exports, preserving real profile prompts and every admitted unit."""
...
def validate_profile_bindings(rows, entries, condition):
    ...
    require(
        ...
        and condition["registered_session_count"] == len(entries) == len(tasks) * 4
        ...
        "cross_representation.frozen_population",
    )
    require(
        ...
        "cross_representation.frozen_profile_prompts",
    )
    require(
        Counter(
        ...
        == Counter({(task, name): 2 for task in tasks for name in PROFILES}),
        "cross_representation.per_task_profile_denominators",
    )
    ...
    for entry in entries:
        ...
        require(
            ...
            "cross_representation.new_task_profile_parents",
        )
        require(panel._eligible(q) or not export["rows"], "cross_representation.failed_positive")
        if session is not None:
            require(
                session["callback_binding"]["model_configuration_id"] == configs[profile]["id"],
                "cross_representation.actual_callback_configuration",
            )
        ...
        for row in export["rows"]:
            ...
            require(session is not None, "cross_representation.missing_original_session")
            ...
            require(
                ...
                "cross_representation.original_profile_request_and_target",
            )
    ...
    require(
        ...
        "cross_representation.complete_original_exports",
    )
    return record(
        "cross_binding_representation_profile_checks",
...
def analyze_representation(rows, entries, binding, policy, condition):
    ...
    require(
        ...
        "cross_representation.frozen_policy",
    )
    before = canonical_json_bytes(rows)
    result = panel.analyze_representation(rows, entries, binding, policy, condition["id"])
    require(canonical_json_bytes(rows) == before, "cross_representation.rows_mutated")
    ...
    require(
        ...
        "cross_representation.new_data_binding",
    )
    linked = record(
        "cross_binding_representation_binding",
        generation_condition_id=condition["id"],
    ...
    return {**result, "cross_binding": linked, "profile_checks": checks}
