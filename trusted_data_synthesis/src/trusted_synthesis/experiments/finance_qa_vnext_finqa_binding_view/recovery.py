...
def method_probes(task):
    ...
    require(task["key"] == "J2" and target["op"] == "subtract", "binding_metrics.J2_target_shape")
    return {
        "target_first_term": target["args"][0],
        "target_second_term": target["args"][1],
        "complete_target_difference": target,
    }


def recovery_trace(events, requests, result, task):
    require(task["key"] in {"E1", "J2"}, "binding_metrics.two_registered_tasks")
    target = task["target"]
