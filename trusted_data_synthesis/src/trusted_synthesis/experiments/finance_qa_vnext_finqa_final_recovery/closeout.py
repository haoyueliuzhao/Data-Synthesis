...
def closeout(root, reviews):
    ...
    for h in ("E", "F"):
        for feedback in ("B", "R"):
            rows = [
                ...
                if r["condition"] == h and r["feedback_condition"] == feedback
            ]
            require(
                len(rows) == 2 and aggregate(rows) == summary["by_cell"][h + "_" + feedback],
    ...
    report = record(
        "e1_recovery_closeout",
        registered_sessions=8,
        by_cell=summary["by_cell"],
        by_feedback=summary["by_feedback"],
    ...
    seal_directory(store, kind="e1_recovery_closeout_manifest", report_id=report["id"])
    return report
