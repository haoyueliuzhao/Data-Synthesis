...
from trusted_synthesis.experiments.finance_qa_vnext_finqa_difficulty.runtime import parse
...
from .metrics import recovery_trace
from .runtime import PRESENTATION_VERSION, FinalRecoveryRuntime, verify_feedback
...
def verify_session(panel, registration, directory, *, model_required=True):
    ...
    for index in range(result["submissions"]):
        ...
        verify_feedback(request, registration["feedback_condition"])
        events.append(event)
    ...
    for attempt in ledger["attempts"]:
        ...
        review_http_exposures += verify_feedback(request, registration["feedback_condition"])
