...
class QualifiedCompletedResult(FrozenModel):
    ...
    schema_version: Literal["finance_v26_qualified_capability_completed_result.v1"] = (
        "finance_v26_qualified_capability_completed_result.v1"
    )
    ...
    def validate_result(self) -> QualifiedCompletedResult:
        if self.result_id != _identity(
            self,
            "result_id",
            "finance_v26_qualified_capability_completed_result:",
        ):
            raise ValueError("qualified Capability completed-result identity changed")
        return self
...
class FreshCapabilityRawExecution(FrozenModel):
    ...
    schema_version: Literal["finance_v26_fresh_capability_raw_execution.v1"] = (
        "finance_v26_fresh_capability_raw_execution.v1"
    )

    @model_validator(mode="after")
    def validate_raw(self) -> FreshCapabilityRawExecution:
        if (
            ...
            raise ValueError("fresh Capability Raw denominator changed")
        if self.model_endpoint_observed and not (
            ...
            raise ValueError("ineligible Fresh Capability Raw claims a model endpoint")
        if (
        ...
        if self.artifact_id != _identity(
            self,
            "artifact_id",
            "finance_v26_fresh_capability_raw_execution:",
        ):
            raise ValueError("fresh Capability Raw identity changed")
        return self


@dataclass(frozen=True)
class FreshCapabilityRuntimeBinding:
    package: Any
    record: Any
...
def _presentation_salt(
    *,
    binding: FreshCapabilityRuntimeBinding,
    ...
    return canonical_hash(
        {
            ...
            "path_strategy_id": "structured_direct",
        },
        prefix="finance_v26_fresh_capability_candidate_presentation:",
    )
...
def execute_fresh_capability_job_raw(
    *,
    job: Any,
    runner_contract: Any,
    ...
    client: Any | None,
    output_dir: Path,
) -> FreshCapabilityRawExecution:
    raw_path = _raw_path(output_dir, job)
    if raw_path.exists():
        raw = FreshCapabilityRawExecution.model_validate_json(raw_path.read_text(encoding="utf-8"))
        if (
            raw.runner_contract_id != runner_contract.contract_id
            or raw.job_payload != job.model_dump(mode="json")
        ):
            raise ValueError("fresh Capability Raw recovery crosses identities")
        for descriptor in (
            ...
            *raw.transport_invocation_artifacts,
            ...
            if (
                ...
                raise ValueError("fresh Capability Raw recovery bytes changed")
        return raw
    ...
    if client is None:
        raise ValueError("pending fresh Capability Job has no Stage 1 client")
    ledger = _QualifiedJournal(
        client,
    ...
    for _ in range(resource_contract.maximum_primary_stage_one_requests - 1):
        ...
        if decoded_state != state or prompt_runner._sensitive_key_paths(  # noqa: SLF001
            ...
            raise ValueError("fresh Capability Prompt changed state or safe Key surface")
        ...
        if commit is None:
            raise ValueError("accepted fresh Capability action lacks a Commit")
        commits.append(
    ...
    provisional_raw = FreshCapabilityRawExecution.model_construct(
        artifact_id="pending", **raw_values
    )
    raw = FreshCapabilityRawExecution(
    ...
    persisted = FreshCapabilityRawExecution.model_validate_json(
        raw_path.read_text(encoding="utf-8")
    )
    if persisted.artifact_id != raw.artifact_id:
        raise ValueError("fresh Capability Raw persistence changed its identity")
    return persisted
