...
from trusted_synthesis.experiments.vtdo_experiment import (
    phase1_v26_fresh_repaired_full_condition_final_request_contract_continuity_repair_preflight as subject,
)
from trusted_synthesis.experiments.vtdo_experiment import (
...
ATTACHED_REVIEW = Path(
    "/home/zhuxinrui/.codex/attachments/43e20d8d-a057-4f0b-ad8c-d5a567ccc93b/pasted-text.txt"
)
...
def _review_path() -> Path:
    ...
    pytest.skip("exact v26.209 review input is unavailable")
...
def test_modified_external_review_rejects(tmp_path: Path) -> None:
    ...
    with pytest.raises(subject.V209Error, match="external review bytes differ"):
        subject._authorization(path)


def test_v208_v207_and_v206_parent_chain_is_frozen() -> None:
    authorization, _, _ = subject._authorization(_review_path())
    parents = subject._predecessor_freeze(
        repository_root=REPOSITORY_ROOT,
        authorization_id=authorization.authorization_id,
    )
    assert parents.freeze.postreview_scope.endswith("FAILED_AT_FROZEN_FINAL_REQUEST_CONTINUITY")
    assert parents.freeze.first_blocker == "frozen_final_provider_request_envelope_not_preserved"
    ...
    assert len(parents.source_callsites.rows) == 792
...
def test_source_has_one_shared_transport_route() -> None:
    ...
    for name in ("invoke_action", "invoke_correction", "invoke_final"):
        wrapper = subject._method(tree, "FinalContinuityRepairedFullConditionRunner", name)
...
def test_formal_final_request_continuity_counts() -> None:
    ...
    continuity = models.FrozenRequestContinuityAudit.model_validate_json(
        (FORMAL_ROOT / "frozen_request_continuity_audit.json").read_bytes()
    )
    ...
    assert continuity.total_registered_message_match_count == 792
...
def test_formal_failure_and_dynamic_branch_controls() -> None:
    ...
    assert dynamic.dynamic_final_contract_envelope_valid
    assert dynamic.dynamic_final_request_differs_from_reference
    assert dynamic.diagnostic_final_dispatch_count == 1


def test_formal_gate_and_transition_keep_online_blocked() -> None:
    if not FORMAL_ROOT.is_dir():
        pytest.skip("v26.209 formal directory has not been materialized")
    gate = models.FinalRequestContinuityGateAudit.model_validate_json(
        (FORMAL_ROOT / "final_request_continuity_gate_audit.json").read_bytes()
    )
    transition = models.ProspectiveTransition.model_validate_json(
        (FORMAL_ROOT / "prospective_transition.json").read_bytes()
    )
    report = models.FinalRequestContinuityPreflightReport.model_validate_json(
