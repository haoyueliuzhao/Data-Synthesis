...
from trusted_synthesis.experiments.vtdo_experiment.phase1_v26_thinking_8k_completion_calibration_execution import (  # noqa: E501
    prepare_exact_8k_execution,
)
from trusted_synthesis.experiments.vtdo_experiment.phase1_v26_thinking_8k_completion_calibration_execution_preflight import (  # noqa: E501
    DestructivePreflightAudit,
    Exact8KClientBindingAudit,
    PrecallRecoveryAudit,
    RunnerFixtureAudit,
    build_exact_8k_runner_preflight,
)
...
) -> tuple[Path, Exact8KRunnerPreflightReport]:
    output = tmp_path_factory.mktemp("v26_100_exact_8k_runner")
...
def test_v26_100_replays_v26_99_and_all_runner_sources_before_client(
    built: tuple[Path, Exact8KRunnerPreflightReport],
) -> None:
    output, report = built
    replay = Exact8KRunnerSourceReplayAudit.model_validate_json(
    ...
    assert replay.replayed_file_count == replay.replay_pass_count == 770
    assert counts == {
...
def test_v26_100_binds_actual_client_request_to_exact_8k(
    built: tuple[Path, Exact8KRunnerPreflightReport],
) -> None:
    output, _ = built
    client = Exact8KClientBindingAudit.model_validate_json(
        (output / "client_request_binding_audit.json").read_text()
    )
    contract = Exact8KExecutionContract.model_validate_json(
        (output / "execution_contract.json").read_text()
    )
    budget = ProviderTokenBudgetContract.model_validate_json(
        (output / "provider_token_budget_contract.json").read_text()
    )

    assert client.model_config_id == contract.model_config_id == EXACT_8K_MODEL_CONFIG_ID
    assert client.thinking_binding_id == contract.thinking_binding_id
    assert client.thinking_binding_id == EXACT_8K_THINKING_BINDING_ID
    assert client.representative_certificate.request_max_tokens == 8192
    ...
    assert budget.maximum_output_tokens == contract.completion_upper_bound_tokens == 8192
    assert budget.maximum_total_tokens == contract.rollout_upper_bound_tokens == 160000
    assert contract.model_discovery_calls_per_job == 0
    assert not contract.automatic_16k_escalation_allowed


def test_v26_100_direct_fixtures_close_every_provider_call_pre_call(
    built: tuple[Path, Exact8KRunnerPreflightReport],
) -> None:
    output, _ = built
    ...
    assert all(item.all_provider_calls_exact_8k_request_bound for item in fixture.direct_rows)


def test_v26_100_rescue_and_interpretation_controls_are_frozen(
    built: tuple[Path, Exact8KRunnerPreflightReport],
) -> None:
    output, _ = built
    fixture = RunnerFixtureAudit.model_validate_json(
    ...
    assert all(item.all_rescue_calls_exact_8k_request_bound for item in fixture.rescue_rows)
    ...
    assert fixture.length_failure_transition_control == "fresh_16k_completion_preflight_only"
...
def test_v26_100_off_compiler_budget_and_recovery_controls_fail_closed(
    built: tuple[Path, Exact8KRunnerPreflightReport],
) -> None:
    output, _ = built
    ...
    assert audit.off_compiler_rescue_exact_8k_request_bound


def test_v26_100_destructive_controls_and_transition_are_narrow(
    built: tuple[Path, Exact8KRunnerPreflightReport],
) -> None:
    output, report = built
    ...
    assert report.exact_8k_execution_authorized
    assert not report.fallback_16k_execution_authorized
    ...
    assert report.next_permitted_stage == "thinking_8k_completion_calibration_execution_only"


def test_v26_100_online_prepare_replays_without_client(
    built: tuple[Path, Exact8KRunnerPreflightReport],
    tmp_path: Path,
) -> None:
    ...
    assert prepared.static.agent_model_config.max_output_tokens == 8192
    assert not (tmp_path / "prepared" / "report.json").exists()
    assert not (tmp_path / "prepared" / "raw_provider_calls").exists()


def test_v26_100_dual_build_is_byte_identical_and_privacy_redacted(
    built: tuple[Path, Exact8KRunnerPreflightReport],
    tmp_path: Path,
) -> None:
    formal, formal_report = built
    independent = tmp_path / "independent"
    independent_report = build_exact_8k_runner_preflight(
        run_id=RUNNER_PREFLIGHT_RUN_ID,
