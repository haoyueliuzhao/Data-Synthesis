...
from trusted_synthesis.experiments.finance_qa_vnext_fixed_kernel_value import evaluation as analysis
from trusted_synthesis.experiments.finance_qa_vnext_fixed_kernel_value.protocol import record

BINDING = {key: "synthetic:" + key for key in analysis.BINDING_FIELDS}
ARMS, SEEDS = analysis.original_design.ARMS, analysis.original_design.SEEDS
...
def test_actual_selection_rejects_unbound_incomplete_or_reclassified_reports(defect):
    ...
    if defect == "missing_train":
        ...
        train[0] = analysis.original_design.select_direction(
...
def test_protocol_is_fixed_before_results_and_no_randomness_or_denominator_tuning():
    value = analysis.analysis_policy()
