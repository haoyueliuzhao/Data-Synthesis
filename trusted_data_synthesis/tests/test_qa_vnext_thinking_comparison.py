...
from trusted_synthesis.experiments.finance_qa_vnext_thinking_comparison.online.calculator import (
    CalculationError,
    calculate,
)
...
from trusted_synthesis.experiments.finance_qa_vnext_thinking_comparison.plan import WORKER_PYTHON
from trusted_synthesis.experiments.finance_qa_vnext_thinking_comparison.stage import assert_public

WORKER = (
    Path(__file__).resolve().parents[1]
    / "src/trusted_synthesis/experiments/finance_qa_vnext_thinking_comparison/online/worker.py"
)
...
def test_cost_missing_provider_usage_is_not_zero_or_removed(tmp_path):
    ...
    (online / "launch.json").write_bytes(
        encode({"registrations": [{"label": "control", "task_key": "control", "arm": "F"}]})
    )
...
def envelope(content, private="PRIVATE_SYNTHETIC_REASONING", finish="stop"):
    return {
        "id": "control:thinking",
        ...
        "ignored_private_extension": private,
    }


def test_thinking_both_models_equal_except_identity_and_public_system_unchanged():
    ...
    first = request_body([], "deepseek-v4-flash")
    second = request_body([], "deepseek-v4-pro")
    assert SYSTEM == old_system
    assert first.pop("model") != second.pop("model") and first == second
    assert first["thinking"] == {"type": "enabled"} and first["reasoning_effort"] == "high"
    assert first["max_tokens"] == 16384
    assert not set(first) & {"temperature", "top_p", "tools", "reasoning_content"}


def test_projection_retains_exact_public_content_and_telemetry_only():
    from trusted_synthesis.experiments.finance_qa_vnext_thinking_comparison.online import projection

    project_response = projection.project_response

    raw_public = ' { "final" : { "value" : 0.1234500 } } '
    result = project_response(encode(envelope(raw_public)))
    assert "PRIVATE_SYNTHETIC_REASONING" not in encode(result).decode()
    assert result["choices"][0]["message"]["content"] == raw_public
    assert result["reasoning_telemetry"]["nonempty"] is True
    assert result["usage"]["completion_tokens_details"]["reasoning_tokens"] == 8
    assert result["usage"]["completion_tokens"] == 12
    assert not result["is_original_http_response"]


def test_malformed_response_or_error_does_not_persist_private_body():
    from trusted_synthesis.experiments.finance_qa_vnext_thinking_comparison.online import projection

    project_response = projection.project_response

    for raw in (
        b"PRIVATE_SYNTHETIC_REASONING not json",
        b'{"error":{"message":"PRIVATE_SYNTHETIC_REASONING"}}',
    ):
...
def test_real_worker_projects_before_persistence_and_excludes_private_history(tmp_path):
    output, result = run_isolated(
        tmp_path,
        [
            {
                "_control_envelope": envelope(
                    '{"tool":"calculate","arguments":{"expression":"2+3"}}'
                )
            },
            {"_control_envelope": envelope('{"final":{"value":"5"}}')},
        ],
    )
    assert result["terminal"] == "model_final" and result["model_requests"] == 2
    assert not list(output.rglob("*_http_response.body"))
    assert len(list(output.rglob("*_response_projection.json"))) == 2
    assert all(
        b"PRIVATE_SYNTHETIC_REASONING" not in p.read_bytes()
        for p in output.rglob("*")
        if p.is_file()
    )
    second = json.loads((output / "turns/001_http_request.body").read_text())
    assert "reasoning_content" not in json.dumps(second)
    assert result["requested_model"] == "deepseek-v4-flash"


def test_length_is_terminal_resource_truncation_not_bad_formula_or_json(tmp_path):
    output, result = run_isolated(
        tmp_path,
        [
            {"_control_envelope": envelope('{"final":', finish="length")},
            {"final": {"value": "999"}},
        ],
    )
    assert result["terminal"] == "generation_truncated_length"
    assert result["model_requests"] == 1 and result["tool_calls"] == 0 and result["events"] == []
    assert (output / "turns/000_assistant.raw").read_text() == '{"final":'
    assert not list(output.rglob("*_http_response.body"))
    audit = audit_session(
        output,
        json.loads((tmp_path / "public.json").read_text()),
    ...
    assert audit["first_final_index"] is None and audit["calculations"] == []


def test_v2_precision_floor_fixes_tail_without_fixing_wrong_units_or_numbers():
    private = control_private("997/70")
    assert answer_score("14.242857142857142", "%", private)["task_answer_status"] == "PASS"
    assert answer_score("14.23", "%", private)["task_answer_status"] == "FAIL"
    assert answer_score("0.14242857142857142", "%", private)["task_answer_status"] == "FAIL"
    assert (
        answer_score("14.242857142857142", "USD million", private)["task_answer_status"] == "FAIL"
    )
    private = control_private("1/3")
    assert answer_score("0.333333333333", "%", private)["numeric_correct"] is True
    assert answer_score("333333333333/1000000000000", "%", private)["numeric_correct"] is False


def test_multi_field_precision_is_compatible_but_wrong_scale_or_unit_is_not():
    from trusted_synthesis.experiments.finance_qa_vnext_thinking_comparison.evaluate import (
        display_compatible,
    )

    assert display_compatible("16.416815660386008", "%", "16.4", "percent")
    assert display_compatible("997/70", "%", "14.24", "%")
    assert not display_compatible("14.24", "%", "0.1424", "%")
    assert not display_compatible("14.24", "%", "14.24", "USD million")


def test_fixed_paired_24_population_and_768_request_cap():
    from trusted_synthesis.experiments.finance_qa_vnext_thinking_comparison.plan import (
        LABELS,
        TASKS,
        condition,
        registrations,
    ...
    rows = registrations({key: {"task_id": key, "id": key} for key in TASKS})
    assert sum(r["response_budget"] for r in rows) == 768
    assert condition()["maximum_reserved_token_allowance"] == 88866816
    for key in TASKS:
        assert len([r for r in rows if r["task_key"] == key and r["arm"] == "F"]) == 2
        assert len([r for r in rows if r["task_key"] == key and r["arm"] == "P"]) == 2


def test_all_twelve_old_sessions_side_table_only_no_historical_overwrite():
    from trusted_synthesis.experiments.finance_qa_vnext_thinking_comparison.evaluate import (
        historical_side_table,
    )

    root = Path(__file__).resolve().parents[2]
    result = historical_side_table(root)
    assert len(result["rows"]) == 12
    assert result["old_strict_counts"] == {"PASS": 10, "FAIL": 2}
    assert result["revised_counts"] == {"PASS": 12}
    assert result["provider_calls"] == 0 and result["old_records_unchanged"]


def test_blinded_packet_is_durable_json_and_omits_condition_metadata():
    from trusted_synthesis.canonical_json import canonical_json_bytes
    from trusted_synthesis.experiments.finance_qa_vnext_thinking_comparison.evaluate import (
        blinded_packet,
    )

    packet = blinded_packet(
        {
            "label": "F_C2_01",
    ...
    assert b"deepseek" not in raw and b"F_C2_01" not in raw and b"123" not in raw
    assert packet["raw_messages"] == {"0": '{"final":{"value":"13"}}'}
