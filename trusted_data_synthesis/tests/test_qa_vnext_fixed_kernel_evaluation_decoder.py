...
from trusted_synthesis.experiments.finance_qa_vnext_fixed_kernel_value import evaluation
from trusted_synthesis.experiments.finance_qa_vnext_fixed_kernel_value import evaluation as decoder
from trusted_synthesis.experiments.finance_qa_vnext_fixed_kernel_value.evaluation import (
    BINDING_FIELDS,
    read_json,
    record,
    sha,
...
def make_decoder(path, *, length=12, generated=None, error=None, pool="A"):
    ...
    identity = record(
        "model_identity",
        **{
            key: decoder.SURFACE_MANIFEST_ID if key == "surface_manifest_id" else "synthetic-" + key
            for key in BINDING_FIELDS
            if key not in {"training_configuration_id", "decoder_config_id"}
        },
...
def run_public(tmp_path, public_panel, **options):
    ...
    report = evaluation.generate(
        tmp_path,
        "generation",
        surface_directory=evaluation.SURFACE_DIRECTORY,
...
def test_production_loader_mock_failure_keeps_attempt_and_partial_usage_unknown(
    ...
    from trusted_synthesis.experiments.finance_qa_vnext_fixed_kernel_value import training as train
