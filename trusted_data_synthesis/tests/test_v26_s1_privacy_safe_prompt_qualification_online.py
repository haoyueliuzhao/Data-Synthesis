...
from trusted_synthesis.experiments.vtdo_experiment import (
    phase1_v26_s1_privacy_safe_prompt_qualification_online as online,
)
from trusted_synthesis.experiments.vtdo_experiment import (
...
def prepared(tmp_path_factory: pytest.TempPathFactory) -> online.PreparedExecution:
    output_dir = tmp_path_factory.mktemp("v26_138_prepare")
...
def test_v26_138_exact_scripted_denominator_and_zero_call_resume(
    prepared: online.PreparedExecution,
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    assert prepared.source_replay.replayed_file_count == 3_901
    assert prepared.preexecution_binding.byte_identical_preflight_output_count == 16
    ...
    report = online.run_privacy_safe_s1_qualification(
        preflight_dir=PREFLIGHT_DIR,
        output_dir=output_dir,
        package_root=PACKAGE_ROOT,
        implementation_root=PACKAGE_ROOT,
    ...
    assert report.clean_prompt_privacy_noncompliance_job_count == 0
    assert report.classifier_sensitive_action_prompt_key_count == 0
    assert report.historical_entry_result_pooled_count == 0
    assert report.formal_v26_134_qualification_remains_failed
    assert report.pass_requires_independent_postrun_audit
    assert not report.role_execution_authorized
    ...
    def _forbidden_client(*_args: Any, **_kwargs: Any) -> Any:
        raise AssertionError("completed v26.138 replay constructed a client")

    recovered = online.run_privacy_safe_s1_qualification(
        preflight_dir=PREFLIGHT_DIR,
        output_dir=output_dir,
        package_root=PACKAGE_ROOT,
        implementation_root=PACKAGE_ROOT,
...
def test_v26_138_second_detour_is_measurement_support_exit(
    prepared: online.PreparedExecution,
    tmp_path: Path,
) -> None:
    job = next(
        ...
        if item.source_engineering_path_id == preflight.runner_base.EXPECTED_DETOUR_PATH_ID
    )
    ...
    def _detour_factory(config: Any, _job: Any, binding: Any) -> _OnlineScriptedClient:
        return _OnlineScriptedClient(
            preflight.runner_base.ScriptedS1QualificationClient(
                config,
                final_answer=binding.compiler_trajectory.final_answer,
                force_action_id=preflight.runner_base.EXPECTED_DETOUR_ACTION_ID,
                force_action_uses=2,
            )
        )
...
def test_v26_138_clean_prompt_privacy_rejection_is_model_noncompliance(
    prepared: online.PreparedExecution,
    tmp_path: Path,
) -> None:
    job = prepared.manifest.jobs[0]

    def _privacy_factory(config: Any, _job: Any, binding: Any) -> _OnlineScriptedClient:
        return _OnlineScriptedClient(
            preflight.runner_base.ScriptedS1QualificationClient(
                config,
                final_answer=binding.compiler_trajectory.final_answer,
    ...
    result, raw, _ = online._run_one_job(  # noqa: SLF001
        job=job,
        prepared=prepared,
        client_factory=_privacy_factory,
        output_dir=tmp_path / "privacy",
    )
    assert raw.privacy_rejected_payload_count == 1
    assert result.privacy_gate_failure
    assert result.terminal_category == "model_invalid_trajectory"
    assert not result.instrument_failure
