...
def refresh_atomic_facts(db: DBProtocol, config: dict[str, Any], output_dir: str | None = None, batch_size: int = 5000) -> dict[str, Any]:
    ...
    for table in ["derived_facts", "fact_quality_checks", "standardized_facts", "atomic_facts", "source_documents"]:
...
def _load_context(db: DBProtocol) -> dict[str, Any]:
    metric_rows = [dict(row) for row in db.fetchall("SELECT * FROM metrics WHERE COALESCE(is_active, 1) = 1")]
    metric_alias_rows = [dict(row) for row in db.fetchall("SELECT * FROM metric_alias_map WHERE COALESCE(is_active, 1) = 1")]
    entity_alias_rows = [dict(row) for row in db.fetchall("SELECT * FROM entity_alias_map WHERE COALESCE(is_active, 1) = 1")]
    source_entity_rows = [dict(row) for row in db.fetchall("SELECT source_id, source_code, raw_metadata FROM source_entities")]
    try:
        series_map_rows = [dict(row) for row in db.fetchall("SELECT * FROM source_series_entity_map WHERE COALESCE(is_active, 1) = 1")]
    except Exception:
        series_map_rows = []
    ...
    return {"metrics": metrics, "metric_aliases": metric_aliases, "entity_aliases": entity_aliases, "source_metadata": _source_metadata_context(source_entity_rows), "series_entity_map": _series_entity_map_context(series_map_rows)}



def _series_entity_map_context(rows: list[dict[str, Any]]) -> dict[str, dict[str, dict[str, Any]]]:
...
def _iter_atomic_facts(db: DBProtocol, context: dict[str, Any], report: dict[str, Any]) -> Iterable[dict[str, Any]]:
    rows = db.fetchall(
        ...
        """,
        ("sec_companyfacts_json", "fred_observation", "wb_observation", "imf_sdmx_response"),
    )
    for row in rows:
        record = dict(row)
        ...
        for concept, concept_payload in namespace_facts.items():
            ...
            for unit_name, items in units.items():
                ...
                for item in items:
                    ...
                    fiscal_quarter, period_role = _sec_period_label(metric, item, period_start, period_end)
                    selection_key = _sec_selection_key(entity_id, metric_id, unit_name, item, metric, period_start, period_end, fiscal_quarter, period_role)
                    fact = _fact(
                        entity_id=entity_id,
                        metric_id=metric_id,
                        value=value,
                        ...
                        verification_status="single_source",
                        tolerance=None,
                        notes=_compact_notes({"label": label, "form": item.get("form"), "accn": item.get("accn"), "frame": item.get("frame"), "period_role": period_role}),
                        stable_parts=["sec_companyfacts", entity_id, metric_id, item.get("accn"), full_concept, unit_name, item.get("fy"), fiscal_quarter, period_start, period_end, item.get("val"), item.get("frame"), item.get("form"), item.get("filed")],
                    )
                    candidates.append({"fact": fact, "item": item, "metric": metric, "selection_key": selection_key, "period_role": period_role})
...
def _select_sec_companyfacts(candidates: list[dict[str, Any]], report: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for group in grouped.values():
        ...
        winner = group[0]
        if len(group) > 1:
            report["skipped_counts"]["sec_duplicate_candidates_suppressed"] += len(group) - 1
...
def _annotate_sec_selection(candidate: dict[str, Any], candidate_count: int, amended: bool = False) -> None:
    fact = candidate["fact"]
...
def _sec_selection_key(entity_id: str, metric_id: str, unit_name: str, item: dict[str, Any], metric: dict[str, Any], period_start: str | None, period_end: str | None, fiscal_quarter: str | None, period_role: str) -> tuple[Any, ...]:
    period_type = metric.get("period_type")
    if period_type == "point_in_time":
        return ("sec_instant", entity_id, metric_id, unit_name, period_end, item.get("fy"), fiscal_quarter)
    return ("sec_duration", entity_id, metric_id, unit_name, period_start, period_end, item.get("fy"), fiscal_quarter, period_role)


def _sec_period_label(metric: dict[str, Any], item: dict[str, Any], period_start: str | None, period_end: str | None) -> tuple[str | None, str]:
    fp = item.get("fp")
...
def _sec_candidate_score(candidate: dict[str, Any]) -> tuple[int, int, int, int, str, str]:
    item = candidate["item"]
    ...
    return (
        ...
        _sec_frame_score(item.get("frame"), item.get("fy"), item.get("fp"), period_role),
...
def _sec_form_score(form: str, period_role: str | None) -> int:
    ...
    if period_role in {"annual_flow", "instant"}:
        scores = {"10-K": 100, "20-F": 95, "40-F": 95, "10-K/A": 75, "20-F/A": 70, "40-F/A": 70, "10-Q": 40, "10-Q/A": 30}
    elif period_role == "quarter_flow":
        scores = {"10-Q": 100, "10-Q/A": 75, "10-K": 20, "10-K/A": 15}
    else:
        scores = {"10-Q": 70, "10-Q/A": 55, "10-K": 50, "10-K/A": 40, "20-F": 45, "40-F": 45}
...
def _sec_duration_score(metric: dict[str, Any], item: dict[str, Any], period_role: str | None) -> int:
    if metric.get("period_type") == "point_in_time":
        return 20
    duration = _days_between(_date_or_none(item.get("start")), _date_or_none(item.get("end")))
    if duration is None:
        return 0
...
def _extract_fred_observation(record: dict[str, Any], context: dict[str, Any], report: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if value is None:
        ...
        return None
    series_map = context.get("series_entity_map", {}).get("fred_observations", {}).get(str(series_id), {})
    entity_id = series_map.get("instrument_entity_id") or series_map.get("applies_to_entity_id") or FX_ENTITY_BY_FRED_SERIES.get(series_id) or "USA_COUNTRY"
    metric = context["metrics"].get(metric_id, {})
    ...
    fact = _fact(
        ...
        verification_status="single_source",
        tolerance=None,
        notes=_compact_notes({"realtime_start": payload.get("realtime_start"), "realtime_end": payload.get("realtime_end"), "frequency": series_metadata.get("frequency") if isinstance(series_metadata, dict) else None, "units_short": series_metadata.get("units_short") if isinstance(series_metadata, dict) else None, "series_entity_id": series_map.get("series_entity_id"), "applies_to_entity_id": series_map.get("applies_to_entity_id"), "instrument_entity_id": series_map.get("instrument_entity_id")}),
        stable_parts=["fred_observations", series_id, entity_id, metric_id, payload.get("date"), payload.get("realtime_start"), payload.get("realtime_end"), payload.get("value")],
    )
    _count_fact(report, fact)
    return fact
...
def _extract_worldbank_observation(record: dict[str, Any], context: dict[str, Any], report: dict[str, Any]) -> dict[str, Any] | None:
    ...
    fact = _fact(
        ...
        stable_parts=["worldbank_indicators", country, indicator, entity_id, metric_id, year, payload.get("value")],
    )
    _count_fact(report, fact)
    return fact
    ...
    for country_code, observations in indicator_values.items():
        ...
        for year_text, raw_value in observations.items():
            ...
            fact = _fact(
                ...
                stable_parts=["imf_sdmx", country_code, concept, entity_id, metric_id, year, raw_value],
            )
            _count_fact(report, fact)
            yield fact


def _refresh_source_documents(db: DBProtocol, context: dict[str, Any], build_id: str, report: dict[str, Any]) -> int:
    rows = db.fetchall(
        """
...
def _source_document_for_sec_filing(record: dict[str, Any], context: dict[str, Any], build_id: str, report: dict[str, Any]) -> dict[str, Any] | None:
    payload = _json_value(record.get("record_json"))
    if not isinstance(payload, dict):
    ...
    stable_document_id = _stable_document_id("sec_filings", entity_id, form, payload.get("accessionNumber"), payload.get("primaryDocument"), record.get("raw_object_id"))
    return {
        ...
        "notes": _compact_notes({"accessionNumber": payload.get("accessionNumber"), "primaryDocument": payload.get("primaryDocument"), "ticker": payload.get("ticker")}),
    }
...
def _source_document_for_cn_disclosure(record: dict[str, Any], context: dict[str, Any], build_id: str, report: dict[str, Any]) -> dict[str, Any] | None:
    ...
    return {
        ...
        "raw_object_id": record.get("raw_object_id"),
        "document_status": record.get("validation_status") or "indexed",
        "notes": _compact_notes({"announcement_id": payload.get("announcement_id"), "title": payload.get("title"), "company_name": payload.get("company_name")}),
    }


def _insert_source_document(db: DBProtocol, document: dict[str, Any]) -> None:
    columns = [
        "document_id", "stable_document_id", "build_id", "is_active", "superseded_by",
        "entity_id", "source_id", "form_type", "report_type", "period_end", "filing_date",
        "storage_uri", "original_url", "raw_object_id", "document_status", "notes",
    ]
    db.execute(
        f"""
...
def _compact_notes(values: dict[str, Any]) -> str | None:
    ...
    for key, value in values.items():
        if value is None or value == "":
            continue
        clean[key] = value
    return json.dumps(clean, ensure_ascii=False, sort_keys=True, default=str) if clean else None
