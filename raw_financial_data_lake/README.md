...
The non-active production smoke build `analysis_build_e7b50427d67ca3b53c570f70` generated 15 candidates and 65 signals across all three MVP patterns; 15/15 samples passed independent verification and the build gate. Peer scopes use complete 5-30 member SEC SIC major groups traced from saved SEC submissions, without truncating the entity set. Candidate and Evidence Bundle freeze the scope membership and policy hashes; the final verifier independently reconstructs the eligible universe from the pinned KG and requires exact Candidate, Bundle, and per-Signal-role entity coverage.
