...
图模式先生成 `canonical_semantics` 和 canonical Operation Plan，再选择受控模板。QA generator 4.6.0 将 LLM 接口收敛为 **Sentence Plan + Deterministic Semantic Rendering**：`question_generation.mode=controlled_llm` 时，verbalizer 只发送 canonical question、有限枚举的风格 schema 和候选数量，不发送答案，也不发送 `slot_map / operator_id / constraints` 让模型自我声明语义。LLM 只能返回：
...
默认模式仍是 `controlled_template`，因此离线构建完全可复现。启用 LLM 只增加枚举化语言风格，不改变问题语义。
