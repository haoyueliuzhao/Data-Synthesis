...
Surface Variation V3 扩展了严格同义的财务指标别名、财政期/自然期写法、公司简称和 scope 上下文化表达。默认生产策略由程序按 stable candidate ID 从白名单中确定性选择表达，并在存在候选时保证至少一个非 canonical slot；LLM 只负责受保护 placeholder 模板的句法润色。可选的 `llm_selects_variants` 模式仍保留用于兼容性试验，但不作为跨模型默认路径。Build Gate 另外审计平均非 canonical slot 数、重试率和 style 覆盖，Diversity Report 报告语言骨架、风格、实现来源及逐 slot 变体使用情况。
...
## Surface Variation V3 与受控 LLM 改写

最终问题生成现在使用“本地白名单反标准化 + Protected Rewrite + 独立语义重解析”。本地层稳定选择严格同义指标、实体简称、财政/自然年和季度累计表达；LLM 只改写占位符周围的句法，不接收答案，也不能创建实体、指标、期间、阈值或结论。
