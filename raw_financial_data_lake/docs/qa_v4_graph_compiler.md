...
`variants` 是单次请求中的候选 Sentence Plan 数，不是每个 Candidate 的导出样本数。QA build 现在独立汇总 `llm_request_count`、HTTP/JSON/Plan/controlled-generation 成功率、fallback、时延、token、费用估算和生成方式分布，并把这些指标写入 build gate/report。即使 fallback 后 QA verifier 仍然 100% 通过，API 成功率或 fallback 门控不达标时 build 也不能 ready。
